...
let boPanelZ              = localStorage.getItem('boPanelZ') !== null ? parseInt(localStorage.getItem('boPanelZ')) : 200; // BOパネルの重なり順(z-index。マイナス可)
let boPanelWidth          = parseInt(localStorage.getItem('boPanelWidth'))          || 570; // BOパネルの横幅(px)
...
let rankingTopN      = parseInt(localStorage.getItem('rankingTopN')) || 3; // ランキングパネルに出す人数（ダメージ欄）
let rankingMpTopN    = parseInt(localStorage.getItem('rankingMpTopN')) || rankingTopN; // MP欄の行数（未設定ならダメージ欄と同じ）

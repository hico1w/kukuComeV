...
### 下集合：キャラ大量時の右端重なりバグ修正

- `gatherCharactersBottom()` を複数行折り返し対応に書き直し
- 1行に収まらない場合、`perRow = floor((effectiveW + GAP) / (avgW + GAP))` で行あたり最大人数を計算し自動で複数行に分割
- 行は下から積み上げ（一番下の行がステージ最下部）
- 従来の `clampToStage` によるX座標強制クランプを廃止し、重なりではなく折り返しで対応

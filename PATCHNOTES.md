...
### feat: DINO — デバッグ用の無敵モード（`mutekiman`）

- ゲーム画面で **`mutekiman` とタイプすると無敵**になり、もう一度打つと解除。ヘッダーに緑の「無敵」バッジが出る
- 当たり判定はそのままで、`o.deadly` の衝突時に `die()` を呼ばず素通りさせる（カニの弾き飛ばしは無敵中も従来どおり動く）
- `dinobot` と同じキー入力バッファを共用にした。長さが違うので `endsWith` で両方を判定する。名前入力欄にフォーカスがあるときは拾わない
- **確認**: 無敵にして26秒間まったく操作せずに放置しても死なずスコア1176まで到達、解除後は放置で通常どおり死ぬことを確認

---

## v2.910.0 — 2026-09-06

### feat: DINO — ジャンプ中にもう一度押すと急降下して着地

- **`cloudflare/pages/dino.html`**: `jump()` に空中時の分岐を追加。ジャンプ中にジャンプ操作をすると `state.vy` を `CFG.dropV = 2600` にして一気に落とす
- 実測: 高さ 145px から **0.067秒**で着地（通常のジャンプは離陸から着地まで 1.00秒）
- 連打しても状態が壊れないことを確認
- なお、これは「跳ぶのを途中でやめられる」だけで操作の選択肢が増える方向なので、**避けられない配置が生まれることはない**（障害物の間隔計算は従来どおり通常のジャンプ軌道を前提にしており、そのままで安全側）

---

## v2.909.0 — 2026-09-06

### feat: DINO に歌詞の背景とボット操作を追加、配置の余裕も調整

#### 難易度の余裕

人の操作はボットほど正確ではないので、判定に余裕を持たせた。

- 重なり区間を前後に広げる `PAD` を 0.04秒 → **0.10秒**
- 障害物を抜けた時点で残る選択肢 `MIN_CHOICES` を 7 → **14**（判定は0.01秒刻みなので、ジャンプ開始を0.14秒ぶん自由に選べる）
- **フグ同士は重なってよい**ことにした（`minVisualGap` で幅の0.5倍まで許可）。くっつくと当たり判定の区間がひとつにまとまるので、むしろ避けやすくなる
- 重なった場合の判定も修正。直前が抜けきる前に次が重なり始めるケースを、通しの区間として見るようにした
- **距離の計算を近似から正確な形状ベースに変更。** 「半分ずつ」で見積もっていたため、幅の違う種類の組み合わせでズレて詰みを通していた（本番で13件検出）。enter/exit をそれぞれ実寸から出すようにした
- 結果: **1,754回の判定で避けられない配置は0件**、隣り合う距離は平均267px

#### 歌詞の背景（スコア5000超）

- スコアが 5000 を超えると、ページ背景に歌詞が浮かび上がる（2.2秒かけてフェードイン）
- **盤面の裏には文字を置かない。** 盤面の上下左右に4つの領域を作り、そこだけに敷き詰める。加えてレイヤー全体を**盤面をくり抜いた形に `clip-path` で切り抜く**（領域の置き方だけだと画面幅によって端でかすかにかぶるため）
- 文字色は背景 `#0b0d12` に対して `#1b2130`〜`#2c374e`。強調したい語だけ大きくし、いちばん大きいものは Cormorant Garamond で見せる
- 細い左右の帯には短い語だけを配る（長い行は縁で切れて見えるため）
- **検証**: 1440x900 / 1100x1000 / 900x800 / 390x844 の4サイズで、盤面を非表示にしてその領域を実際にスクリーンショットし、**背景色以外の画素が1つも無い**ことを確認

#### ボット操作（見本用）

- ゲーム画面で **`dinobot` とタイプするとボットが操作を代わる**。もう一度打つと解除。ヘッダーに `BOT` バッジが出る
- 待機中に有効化すると自動で開始する
- ゲーム本体と同じ「到達可能な状態」の先読みを使い、走ったままで抜けられるうちは待ち、待てなくなったら跳ぶ
- **限界**: このボットは最適ではない。「1回のジャンプで2つ処理すべき場面」を取りこぼすため、45秒あたり4回ほど死ぬ。配置の正しさは上記の総当たり判定で保証しており、ボットの死亡は配置の問題ではない

---

## v2.908.0 — 2026-09-06

### change: DINO の敵の密度を上げ、避けられない配置の判定を作り直した

障害物の間隔を手決めの倍率で決めるのをやめ、**ジャンプの軌道から「本当に避けられるか」を計算**して詰められるだけ詰めるようにした。隣り合う障害物の平均距離は **705px → 242px**（約3倍の密度）。

#### 考え方

- 各障害物には「その高さにいると当たる帯」がある。当たり判定の箱から逆算するので、絵のサイズを変えても追従する
  - フグ … `y < 63.1px` で当たる → 跳び越せる
  - 低い星 … `y < 81.6px` で当たる → 跳び越せる
  - 高い星 … `54.2 〜 195.6px` で当たる。**頂点150pxでも越えられない**ので走って抜ける
  - カニ … 帯なし＝**どこに置いてもいい**
- プレイヤーの状態は「地上」か「ジャンプ中の位相」しかないので、取りうる状態の集合を持ち回れば、何個続いても詰みかどうかを厳密に判定できる
- **障害物は「一瞬」ではなく、絵の幅ぶんの時間ずっと当たり判定を持つ**（フグで約0.24秒）。その区間のあいだずっと安全でいられるかで判定する

#### 直したバグ

- **間隔の適用先が1つズレていた。** `spawn()` が設定する `nextGap` は「今回→次」の距離なのに、「直前→今回」の組み合わせで計算した値を入れていた。**次に出す種類を先に決めてから**間隔を求めるよう組み直した
- **時間刻みが粗すぎた。** 0.02秒刻みでは「1回のジャンプで2つ跳び越せる限界(0.762秒)」を 0.008秒だけ超える配置を見分けられず、詰みを通していた。刻みを 0.01秒にし、重なり区間を前後 0.04秒ぶん広げて厳しめに見るようにした
- **判定を窓で区切っていた。** 直近4件だけを見ていたため窓をまたいだ詰みを見逃していた。状態集合をそのまま引き継ぐ方式に変更
- 検証用の `solvable()` で、列の先頭が「跳ぶ必要のある障害物」だと常に詰み扱いになっていた（跳ぶ準備期間がない状態から始めていたため）。入口の状態を自由に選べるよう修正

#### 出現の調整

- カニは置き場所の制約がないので比率を上げた（22% → 24%）。高い星は 10% → 14%
- 間隔のばらつきを 0〜420px から 0〜170px に縮小

#### 検証

実際にゲームを走らせながら、画面上の障害物の**実座標**から重なり区間を出し、「どんな操作をしても避けられない配置か」を全状態の総当たりで判定した。

- **1,795回の判定で、避けられない配置は 0件**（同時に画面へ出た障害物は最大5個）
- 隣り合う距離: 平均 242px / 最小 120px

なお、最適に動くボットを書いて実プレイでも確かめようとしたが、**ボット側のヒューリスティックが最適解を選べず**（1回のジャンプで2つ処理すべき場面を取りこぼす）、こちらは検証手段として成立しなかった。配置の保証は上記の総当たり判定によるもので、ボットの死亡は配置の問題ではない。

---

## v2.907.0 — 2026-09-06

### feat: DINO を遊びやすく調整＋ハイスコアランキングを追加

#### ジャンプと難易度

- **ジャンプを高く・長く**した。`gravity 1900 → 1200` / `jumpV 620 → 600` で、**頂点 101px → 150px、滞空 0.65秒 → 1.00秒**
- **速度上昇を「1秒あたり +0.5%」に変更**。従来の「毎秒+9px」の直線加算をやめ、`speed *= 1.005 ^ dt` にした。検算用に `state.gameT`（プレイ中の経過秒）を持たせ、`speed === speed0 * 1.005^gameT` が誤差 0.0000% で一致することを実測で確認
- **絶対に避けられない配置が出ないようにした。** 障害物の間隔を「時間」ではなく**距離**で管理し、1回のジャンプで進む距離 `D = speed × 滞空時間` を基準に、直前と次の種類の組み合わせで必要な間隔を変える
  - 跳ぶ必要があるもの → **跳ぶと当たる高い星**: `D × 1.90`（着地しきる余裕）
  - 次が跳ぶ必要があるもの: `D × 1.45`
  - 高い星 → その他: `D × 1.25` / それ以外: `D × 1.10`（最低 260px）
  - 速度が上がると `D` も伸びるので、**間隔も自動的に広がる**
- 高い星が「走れば通れて、跳ぶと当たる」ことを幾何で検算。走行時の頭 y=209 に対し星の当たり範囲は y=108〜155（通れる）、ジャンプ頂点の頭 y=59（当たる）

#### 見た目・演出

- **スタート画面では背景を止めた**（以前は待機中もスクロールしていた）
- **走りのコマを `run_01 → run_04 → run_02` の3枚のみ**に変更（10fps）
- **`run_` と `jump_` を左右反転して書き出すようにした。** 原本は左を向いており、右へ走るゲームでは逆だったため。`dead_` は倒れる向き（頭が左）が右向き走行と整合しているので反転しない
- **ロード画面**を追加。`tatie.png` を 1.5秒/回転で回す。素材の読み込みが終わったらフェードアウト
- **ぶつかったときの演出**を追加。`dead1〜4` を 9fps で1回だけ再生し、**以降リトライまで `dead4` のまま**。演出が終わってからゲームオーバー画面を出す
- **BGM `madakana.mp3` をループ再生**。自動再生はブラウザに止められるので最初の操作で鳴らし始める。4.8MB あるため読み込み完了は待たない。ヘッダーに ON/OFF ボタンを置いた（指定にはないが、止められない BGM は困るため）

#### ハイスコアランキング

- **`cloudflare/worker.js` に `/dino-ranking` を追加**
  - `GET`: 上位一覧を返す。**IP は落として返す**
  - `POST`: `{ name, score }`。名前は**15文字以内**に切り、制御文字を除去。空なら 400。スコアは整数で 0〜9999999 の範囲チェック
  - 保存先は `_blocklist.json` と同じ非公開リポジトリの `_dino_ranking.json`（専用の KV は作っていない）。読み込み→追記→書き戻しで、**同時投稿による sha 衝突は最大4回まで再試行**する。上位100件を保持
  - 日付は**日本時間**で持つ。`toISOString()` のままだと 00:00〜09:00 JST が前日になるため
  - `DELETE /admin/dino-ranking`（要 `ADMIN_SECRET`）で1件削除／全消しができる。荒らし対応用
- **ゲーム側**: ゲームオーバー画面に名前入力（`maxlength=15`）と登録ボタン、Top10 の一覧（順位・名前・スコア・日付）。登録すると自分の行が強調される。名前は次回のために localStorage に覚える。ヘッダーの `Ranking` ボタンからはいつでも一覧を開ける

#### 動作確認

Playwright（Chromium）で実機確認。ジャンプの頂点・滞空、速度の毎秒+0.5%、スタート画面で背景が止まること、走りが3枚だけ読み込まれること、`Math.random` を固定してフグに突っ込み **dead1→dead4 と進んで index 3 で止まる**こと、ゲームオーバー到達、ランキング登録（2位で登録され一覧に反映）、`maxlength=15`、Ranking ボタンの開閉を確認。API 側も直接叩いて、空名前で 400、16文字が15文字に切られること、不正スコアで 400、GET に IP が含まれないことを確認した

---

## v2.906.0 — 2026-09-06

### feat: DINO ページ追加（Chrome の恐竜ゲーム風ミニゲーム）

https://kukucome-chara.pages.dev/dino に、Chrome の Dinosaur Game を再現したミニゲームを追加。素材は `public/dino/` のドット絵。

#### 導線

- **ヘッダー**: LIVE バッジと同じ場所に `DINO` リンク（`.dino-badge`）を追加。**配信していないときだけ表示**する。`/live-status` の判定に合わせて `#hd-live` に `is-live` を付け外しし、`#hd-live.is-live .dino-badge { display: none }` で出し分ける。取得に失敗したときは配信オフ扱いなので DINO が出る
- **フッター**: `hico1` を `<p>` から `<a href="/dino">` に変更し、常時 DINO へのリンクにした。`align-self: flex-start` を足して当たり判定を文字幅に収めている

#### ゲーム（`cloudflare/pages/dino.html`）

- 論理解像度 1200x400。`back.png`(2172x724) を高さ400に合わせると幅ちょうど1200になるので、背景1枚＝画面1つ分になる
- **接地位置**は背景の空と地面の境目を実測して決めた。原寸 `y=551/724`（76.1%）が草の線なので `GROUND = H * 0.7611 + 4`
- **背景のつなぎ目**: `back.png` は左端と右端の色が揃っておらず（平均差 11.8）、そのまま並べると縦線が見える。**1枚おきに左右反転**して並べることで必ず同じ端どうしが接するようにした。つなぎ目の列の色差を実測して 43.4 → **0** になったことを確認済み
- **視差**: 太陽が最も遅く（背景の 0.06 倍）、雲が中間（0.20〜0.36 倍）、背景が最速。雲は cloud1〜5 からランダムに選び、位置・大きさ・速度もばらつかせる
- **キャラ**: `run_01〜08` を 14fps、`jump_01〜06` を 12fps で送る。ジャンプは最後のコマで止める。コマごとに画像の高さが違うので**下端合わせ**で描き、足が浮かないようにしている
- **障害物**: フグ(陸・飛び越える) 42% / 星(低い空中・飛び越える) 26% / 星(高い空中・**ジャンプすると当たる**ので走って抜ける) 14% / カニ 18%
- **カニ**: 当たってもゲームオーバーにならない。当たると `kaniyarare.png` に差し替えて回転させながら弾き飛ばし、+25点
- 当たり判定は見た目より内側（キャラは幅56%・高さ86%、障害物は上下左右16%削る）。ドット絵の透明部分で理不尽に死ぬのを避けるため
- ゲームオーバーは Press Start 2P のドット文字で `GAME OVER` ＋ スコア ＋ `retry.png` のボタン。ハイスコアは localStorage に保存
- スコアは右上。ジャンプ時に `jump.mp3`
- 障害物の間隔は「秒 × (開始速度/現在速度)」で決めており、速度が上がっても**画面上の間隔は約302pxで一定**になる

#### 素材（`scripts/build-dino-img.js`）

- `public/dino/*.png`（7.19MB）→ `cloudflare/pages/img-dino/*.webp`（**0.44MB**）に変換。`jump.mp3` はそのままコピー
- 障害物と UI はトリミングしてから縮める（`hosi.png` は上下左右に 80〜350px の透明があった）
- **キャラのコマはトリミングしない。** コマごとに余白を削ると足の位置がズレて歩きがガタつくため、原本の余白を残して等倍で扱う

#### 画面幅への対応

- 3:1 のままだと 390px 幅の端末で盤面が 374x125 になり遊べなかったので、`aspect-ratio` を画面幅で変える（900px以下 2:1 / 640px以下 1.45:1）
- 背景タイル幅 `TILE=1200` と、実際に映す幅 `W` を分離。`W` は表示領域の縦横比から決め、背景は必要枚数だけ並べる
- 狭い画面は見える範囲も狭くなるので、横移動をすべて `W/TILE` 倍にして**障害物を見てから反応できる時間をどの画面でも揃えた**

#### 動作確認

Playwright（Chromium）で実際に操作して確認。ロード完了、スタート、ジャンプ、9秒放置でのゲームオーバー、リトライでの再開、4xx/5xx なし、コンソールエラーなし。`Math.random` を固定してカニだけ出るようにした状態で走らせ、**当たってもゲームオーバーにならず弾き飛ばされたカニが空中に描かれる**ことを確認。`jump.mp3` のリクエスト発生、DINO バッジの表示と `is-live` での非表示、フッターが `<a href="/dino">` になっていること、390px 幅で横スクロールが出ないことも確認済み
...
- `sec-cmdref` に「🏦 MP預金」セクションを新規追加（従来まったく記載が無かった5コマンド）
  - `MPを預ける:N` / `全MP預ける`（`全MPを預ける`可） / `MPを引き出す:N` / `全MP引き出す`（`全MPを引き出す`可） / `MPを渡す:相手:N`
  - 引き出し時に預金へ1以上残る仕様、`MPを渡す` の相手指定（コメント番号または名前の部分一致）、全角・半角コロン両対応を明記
- `sec-cmdref` タイマンカードに追記：奪われるのは所持MPのみで預金MPは対象外、タイマン参加中は預け入れ不可
- `sec-features` に「【】で囲まれた部分は無視されます」を追加（コマンド判定・吹き出し表示から除去）
- 照合の結果、以下は差分なしを確認：汎用キーワード（`sdKeywordPrompts` 94件）、会話モード投与系、モーション・エフェクト・吹き出し・スタイル、ゲーム系（スロット/ガチャ/宝箱/Wordle/早押し/競馬）

### chore: Pages用キャラ画像を同期（324・325を追加）

- `public/chara/` が常に正。Pages配信用の `chara/` は WebP 変換版（変換できない GIF はそのまま配置）
- 未反映だった 324・325（いずれも `.gif`）を `public/chara/` から `chara/` へコピーし、`STANDALONE_CHARS` に追加
- 検証結果: `charImages.json`(313) / `STANDALONE_CHARS`(313) / `chara/` 実ファイル が完全一致。余剰・欠損ともに0件

### docs: sync-chars スキル定義に拡張子マッピング規則と検証手順を追記

`.claude/commands/sync-chars.md` を改訂。毎回同じ説明が必要だった落とし穴を手順書に永続化した。

- **「大前提：2つのディレクトリと拡張子マッピング規則」セクションを新設**
  - `public/chara/` が常に正（原本）、`chara/` は Pages 配信用（png/jpg→WebP、GIF等はそのまま）
  - `expected()`（`.png/.jpg/.jpeg` → `.webp`、それ以外はそのまま）を全手順共通の規則として明文化
  - 「やってはいけないこと」を明示セクション化
- **手順2を修正**: 比較を `expected()` 経由にした。従来は `charImages.json` の値と `chara/` の実ファイル名を直接比較していたため、変換済み306件が全て「不足」と誤判定され `.png` が大量コピーされていた。`convert-chara-webp.js` は変換先 `.webp` が既存だとスキップして元ファイルを消さないため、孤立 `.png` が残る問題があった
- **手順3を修正**: `STANDALONE_CHARS` に `charImages.json` の値をそのまま書かず、`expected()` 通した `.webp` 名で書き込むスクリプトに変更
- **手順7〜9を追加**: `update-index-webp.js` の実行、3者（charImages.json / STANDALONE_CHARS / chara実ファイル）の整合性検証、GitHub Pages デプロイ手順（push前にユーザー承認を得ることを明記）
- 追記したスクリプトは実行して検証済み（コピー予定0件・完全一致・divタグ2030対応を確認）

---

## v2.892.0 — 2026-09-05

### fix: 全MP引き出すが `✖1` で失敗する問題を修正

- **原因**: kukuluLIVE の `mypoint_change` は `point=0` を受け付けず `{ success:0, error:1 }` を返す。全額引き出しは残高を0にするため必ず失敗していた
- **切り分け**（API実測）: 同値の書き戻し（5018→5018）は `success:1`、部分引き出し（→74）も成功。正の値は通り0だけが弾かれることを確認
- `POST /api/mypoint/withdraw`: `newPoint = Math.max(1, current - amt)` とし、預金に必ず1以上を残すよう修正
  - 残高ちょうどの金額指定（例: 残高74で `MPを引き出す：74`）でも同じ理由で失敗していたため、全額版だけでなく共通経路で修正
  - 引き出せなくなる場合（残高1で全額指定）は `預金がありません` を返す
- 付与MPはクライアントが応答の `before - after` から算出するため、1残る仕様でも表示と実残高がズレない

---

## v2.891.0 — 2026-09-05

### change: タイマン参加者の預け入れを禁止

- タイマンは敗者の所持MP全額が勝者に移動するため、開始後に `全MP預ける` で退避すれば絶対に取られない抜け道があった
- `_mypointDeposit`（`app-06-comment-handler.js`）の先頭で、`taimanState.challenger` / `taimanState.target` に該当するユーザーの預け入れを拒否
  - `MPを預ける：N` と `全MP預ける` の両方がこの関数を通るため、1箇所のガードで両コマンドを塞げる
  - 対象はタイマン参加者2名のみ。観戦者は通常どおり預け入れ可能
- 引き出しは制限しない（預金を戻すと賭け金が増えるだけで、抜け道にはならないため）
- バトルロイヤルはMPの移動が無いため対象外（`app-04-battle-royale.js` にMP増減処理なしを確認）

---

## v2.890.0 — 2026-09-05

### feat: 「全MP預ける」「全MP引き出す」コマンドを追加

- `全MP預ける` … 所持MP全額を預ける（`全MPを預ける` も可）。所持MPが0なら実行せず案内
- `全MP引き出す` … 預金MP全額を引き出す（`全MPを引き出す` も可）
- 全額引き出しは事前に金額が分からないため、サーバー側で残高を取得してから全額を算出する方式にした
  - `POST /api/mypoint/withdraw` が `all: true` を受け付け、`mypoint_list` で取得した残高をそのまま引き出し額にする
  - クライアントは加算するMPをサーバー応答の残高差分（`before - after`）から求める（金額指定時も同様に統一）
- 引き出し成功時の吹き出しを `💎 N MP 引き出し！(預金MP: N)` に変更（従来の `(+NMP)` は重複表示だったため）

---

## v2.889.0 — 2026-09-05

### change: 預け/引き出しの文言を「OP」から「MP」に統一

- コメント通知（`postAIReply`）
  - `${amount}OPを引き出しました。残OP: N` → `${amount}MPを引き出しました。預金MP: N`（預け時と表記を統一）
  - `${amount}MPを預けました。預金OP: N` → `預金MP: N`
- 吹き出しも同様に統一（`⏳ N MP 引き出し中…` / `💎 N MP 引き出し！` / `💰 N MP を預けた！(預金MP: N)`）
- サーバーのエラー文言は所持MPとの混同を避けるため「預金」を明示
  - `OP不足 (現在: N)` → `預金MP不足 (現在: N)`
  - `OPアカウントが見つかりません` → `預金アカウントが見つかりません`
...
- コメント末尾に付く `【[おむらい]】` のような名前タグがコマンド判定を邪魔していた問題を修正
- `stripPrefix`（`app-06-comment-handler.js`）で `【...】` を全て除去するよう変更
  - コマンド判定・吹き出し表示・コメントログ・物理オブジェクトすべてがこの関数を通るため一括で反映される
  - 例: `MPを引き出す：500 【おむらい】` → `MPを引き出す：500` として正しくコマンド認識される
- URL検出用の `rawMessage` は元のまま（リンク判定に影響させないため）

---

## v2.887.0 — 2026-09-05

### feat: 配信番号(hash)の自動取得

- 開始ボタン押下時、hash欄が空なら `mylive/port_info`（既存 `/api/live-info`）から現在の配信番号を自動取得して設定する
- `autoFillHash()`（`app-07-ui-stage-ai.js`）を追加。取得した hash は入力欄と localStorage にも反映し、システムログに表示
- hash が無いと mypoint 系APIが cnum を無視して全員同じ口座を返す（v2.886.0参照）ため、この自動取得が事故防止になる

---

## v2.886.0 — 2026-09-05

### fix: 預金OPが全員共通になる／他人の口座から引き出されるバグを修正（API実測で原因特定）

- **実測による原因特定**（配信 hash=575865268 で検証）:
  - `mypoint_list?hash=H&cnum=C` … cnumごとに正しく別々のpid・OP残高を返す（正常）
    | ipid | pid | OP |
    |---|---|---|
    | f69fc5 | eba654b4 | 8574 |
    | ed9070 | 238a23bc | 4731 |
    | b2f9b0 | 77ae6374 | 37 |
  - `mypoint_list?cnum=C`（hashなし）… **cnumが無視され、全cnumが同一口座(pid=eba654b4)を返す**
  - クライアントの `hash` はUI必須入力でないため未設定になり得た → 全員が同じ口座を見て・操作していた
  - さらに誤った pid が `user.pid` にキャッシュされ、以降ずっとその口座を操作し続けていた
- `_mypointBody`: 必ず `hash` + `cnum` の両方を要求。どちらか欠ける場合はコマンドを実行せずエラー表示
- `user.pid` のキャッシュを廃止（`_loadOpOnce` / deposit / withdraw すべて）。毎回 hash+cnum で本人を特定する
- `server.js` `_fetchMypointEntry`: hash無しcnumを例外で拒否（同じ事故の再発防止）
- なお `comment.icon_num` は実測で空文字、`comment.from` は色コード。視聴者のユニークIDは `comment.ipid`（16人分ユニークな値を確認）

---

## v2.885.0 — 2026-09-05

### fix: OP預金アカウント混在 & キャラなし状態でのコマンド登場しないバグを修正

- **預金混在の根本原因と修正**:
  - 以前は全視聴者が `users['master']` を共有 → `user.pid` が配信者の pid になり、全員の預け/引き出しが配信者口座を操作していた
  - icon_num ipid 修正で各視聴者が独立した user を持つので、`user.pid` は正しく各視聴者のpidになる
  - `_mypointBody`: pid 取得済みなら pid 優先、未取得なら cnum でフォールバック（正しい設計に戻す）
  - `_loadOpOnce`: mypoint_list の entry.pid を `user.pid` にセット（セッション内メモリのみ、charSave には保存しない）
  - OP残高もpidも charSave に保存しない（APIから毎回取得）
  - サーバー deposit/withdraw: mypoint_list で取得した entry.pid で mypoint_change を叩く（同じユーザーへの確実な指定）
- **キャラなし状態でコマンドを使うとキャラが登場しないバグ修正**:
  - `MPを預ける`, `MPを引き出す`, `MPを渡す` コマンド処理前に `ensureCharOnStage(user)` を追加

---

## v2.884.0 — 2026-09-05

### fix: キャラとOP預金アカウントを初回から紐づけて管理（人の預金と混ざるバグ修正）

- **根本原因**: kukuluLIVEの安定したポイントアカウントID `pid` がセッション間で保持されておらず、毎回異なる cnum でユーザーを特定しようとしていたため、別ユーザーのOP残高を参照・操作する可能性があった。
- `CHAR_SAVE_FIELDS` に `pid` を追加 → `charSave.json` に永続保存
  - 初回コメント時に `_loadOpOnce` / deposit のレスポンスから `pid` を取得・保存
  - 次回セッションからは保存済み `pid` でキャラとOP口座が最初から紐づく
- `_mypointBody`: `user.pid` が判明していれば pid 優先に変更（cnum は pid 未取得時の初回のみ使用）
  - 前バージョンでcnum優先にしていたのは「全員がmasterのuserを共有していた」問題への回避策だったが、icon_num ipid修正後は各視聴者が独立したuserを持つため、pid優先が正しい動作になる

---

## v2.883.0 — 2026-09-05

### fix: MP預け/引き出しを繰り返すとMPが倍増するバグを修正

- **根本原因**: `server.js` の deposit/withdraw エンドポイントが `mypoint_change` のレスポンスの `success` フィールドを確認していなかった。kukuluLIVE が `{ success: 0 }` を返しても（特に `point=0` での全額引き出し時）サーバーは `{ ok: true }` を返し、クライアントがMP加算を行ってしまっていた。
- `server.js` deposit/withdraw エンドポイント: `mypoint_change` のレスポンスを確認し `success !== 1` の場合はエラーを返すよう修正
- `point=0`（全額引き出し）を `Math.max(1, current - amt)` でkukuluLIVEに渡す値を最低1に切り上げ（API制限への対応）
- サーバーログに kukuluLIVE のエラー内容を出力するよう追加

---

## v2.882.0 — 2026-09-05

### fix: mypoint deposit/withdraw をコメント主に正しく紐付け

- **根本原因**: kukuluLIVEのコメントは `comment.from` が視聴者コメントに設定されないため、全視聴者が `ipid='master'` の同一キャラを共有していた。`user.op`/`user.mp` も共有され「全員に影響」していた。
- `handleComment` : ipid解決に `comment.pid` と `comment.icon_num` を追加
  ```
  ipid = comment.ipid || comment.pid || String(comment.icon_num) || comment.from || 'master'
  ```
  `icon_num` は視聴者ごとに固有の値（charSave.jsonのキーと一致）→ 各視聴者が独立したゲームキャラを持てる
- `_mypointBody` を cnum 優先に変更
  - cnum がある場合は `hash + cnum` でコメント主を特定（`user.pid` は master共有時に誤った pid を返すため使わない）

---

## v2.881.0 — 2026-09-05

### feat: MPを渡すコマンドで名前指定に対応

- `app-06-comment-handler.js` : `MPを渡す` の第1引数が数字の場合はコメント番号検索、それ以外の場合は `users` オブジェクトから名前の部分一致で検索
  - 例: `MPを渡す：アゲル：100` → 「アゲル」という名前を含むユーザーに100MP転送
  - エラーメッセージを「コメントNのユーザー」→「「キー」のユーザー」に統一
...
- `app-06-comment-handler.js` : `_commentNumMap`（コメント番号→ユーザー逆引きMap）を追加、`handleComment` で随時更新
- コマンド `MPを渡す：<コメント番号>：<量>` — 送り手のMPを減らして指定コメントのユーザーにMPを加算
  - 自分自身・MP不足・コメント番号未登録の場合はエラーバブル
  - 成功時は両者にバブル表示＋アンカー付きコメント通知
- `：`（全角）`:`（半角）両対応
...
#### `index.html`
- 21セクション（sec-cmdgen〜sec-cmdref）のタイトル直下にゲーム画面スクショを自動挿入するスクリプトを追加
- クリックで既存ライトボックスに拡大表示、`img.onerror` でファイルなしは非表示
- Playwright + ローカルサーバー（`node server.js`）で `handleComment()` を呼び出してゲーム挙動を撮影

#### `chara/screenshots/`
- 新規ディレクトリ・21枚のゲーム画面スクショを追加
- ペットガチャ・ボス召喚・スロット・吹き出し形・文字色など各機能の実際の挙動を収録

#### `game-screenshots.js`（プロジェクトルート）
- Playwright自動撮影スクリプト（未追跡ファイル）

---

## v2.871.0 — 2026-09-05

### fix: サンプルプロンプト JPG 18枚にプロンプトを追記

#### `index.html`
- `#sec-promptsamples` のJPG 18枚: EXIF SubIFD UserComment からポジティブプロンプトを抽出・追加
- 前回スクリプトで取得できていなかった（サブEXIFのパース漏れ）を修正

---

## v2.870.0 — 2026-09-05

### feat: index.html にサンプルプロンプトセクションを追加

#### `index.html`
- `#sec-promptsamples`「🎨 サンプルプロンプト」セクションを新規追加
- `chara/promptSample/` の全29枚（PNG 11枚・JPG 18枚）をグリッド表示
- PNG 11枚はPNG Info（tEXtチャンク）からポジティブプロンプトを抽出・表示（180字超は「展開」ボタン付き）
- サイドバーリンクを追加

#### `chara/promptSample/`
- 新規ディレクトリ・29ファイルをgit追跡に追加

---

## v2.869.0 — 2026-09-04

### feat: index.html にキーワード組み合わせ例セクションを追加

#### `index.html`
- `#sec-kwsamples` に「🔗 キーワード組み合わせ例」サブセクションを追加
- 初期登録: もいちゃん＋水墨 / スピキ＋ねんど（2枚）
- 既存ライトボックスを共有してクリック拡大表示

#### `chara/keyword-samples/`
- 手動更新画像8枚を反映（80, ぎふ, ジャックO×2, スタイルE, フラット, ランダム人_plain, 制服_plain）
...
#### `scripts/gen-keyword-samples.js`（新規）
- `sdKeywordPrompts`（全94件）＋固定キーワード（ドット・リアル・もいちゃん）の計97件、各2枚を自動生成
- 各キーワードにつき「ベース+キーワード」「キーワードのみ」の2枚を出力
- `masterpiece,<lora:NSFWFilterXL_animagine:-1.5>,` を両ケース共通サフィックスとして付加
- 解像度 1272×1920 / Steps 23 / CFG 3.5
- 未生成のみ生成（`--force` で全件再生成、`--kw キーワード` で個別生成）

#### `chara/keyword-samples/`（新規）
- 全97キーワード × 2枚 = 194枚のサンプル画像を追加

#### `index.html`
- 「🖼 キーワードサンプル画像」セクションを新規追加（サイドバーリンクも追加）
- 各キーワードカードに「ベース+キーワード」「キーワードのみ」を横並びで表示
- 画像未生成の場合は 📷 プレースホルダーを表示

---

## v2.866.0 — 2026-09-03

### fix: SD画像生成タイムアウトでボディ受信中にabortされる問題を修正

#### `public/js/app-11-agru-state-sd.js`
- `clearTimeout(_fetchTimeout)` を `res.json()` の**前**に移動（ヘッダー受信完了時点でタイマー解除）
- タイムアウトを 30 秒に設定（SDサーバーの応答待ちタイムアウト）
- 以前の変更でタイマー解除が `res.json()` の後になっており、大きな base64 画像のボディ受信中に 28s タイマーが発火→ AbortError で画像が表示されない問題が発生していた
...
- **`server.js`** `/api/images`: 更新日時(mtime)降順ソートに変更（追加が新しいものが先頭）
- **`public/admin.html`**: グリッドを4列→3列、`aspect-ratio:1`→`2/3`（縦長キャラ画像に合わせた比率）
...
- **MP消費一覧**：エコ生成 -5、超生成 -500、移動:XXX -200 を追加
- **🎨 AI画像生成 セクション**：エコ生成・超生成の説明追加、gen キーワード追記、キーワード自動追加（ドット/リアル/もいちゃん/汎用）の仕様説明・プロンプト除去の旨を追記
- **移動コマンド**：sec-cmdgen（カード）・sec-cmdref の両箇所に MP200消費の記述を追加
- **sec-cmdref**：エコ生成・超生成のコマンドカードを新規追加（gen も追記）

---

## v2.836.0 — 2026-08-31

### feat: 移動コマンドに MP200 消費を追加

- **`public/js/app-06-comment-handler.js`**: `移動:XXX` コマンドに MP200 チェックを追加
  - MP が 200 未満の場合は「❌ MP不足 (現在MP/200)」を吹き出しで表示し移動変更をキャンセル
  - 成功時は MP を 200 消費して `updateStatsDisplay` を呼び出す

---

## v2.835.0 — 2026-08-31

### feat: SDキーワードをプロンプト本文から除去・KWPリスト編集機能・<>エスケープ

- **`public/js/app-11-agru-state-sd.js`** `_sdGenerateOne()`:
  - `ドット`/`リアル`/`もいちゃん` および `sdKeywordPrompts` のキーワードにマッチした場合、そのキーワードをプロンプト本文から除去してからサーバーへ送信（翻訳・SD生成に含まれなくなる）
  - キーワード除去後に余分なスペース/連続カンマを整理
- **`public/admin.html`** キーワード別ポジティブ（KWP）UIを改善:
  - `_escHtml()` ヘルパーを追加し、キーワード・ポジティブプロンプトの表示を HTML エスケープ（`<lora:...>` 等が消える問題を修正）
  - 各行に ✏ 編集ボタンを追加。クリックするとその行がインライン編集フォームに切り替わり、✓ で保存 / ✕ でキャンセル

---

## v2.834.0 — 2026-08-30

### feat: 全員MP+30 ボタンにステージ通知とSE追加

- **`public/js/app-13-race-admin-misc.js`** `debugMpBtn` ハンドラ: 実行時にステージ中央に「💧 全員 MP +30！」バナーを3秒表示し、`maplestory-lvl-up.mp3` を再生
- **`public/style.css`** `.mp-all-banner` クラスを追加（青系バナー、`brBannerIn/Out` アニメーション使用）

...
### chore: トップページの画像を9枚削除（マニフェスト再生成）

- `cloudflare/pages/img/` から **9枚削除**した
  - `an09` `an17` `an19` / `cos06` `cos08` `cos14` `cos21` `cos24` / `v8914`
- `node scripts/build-site-img.js` を流して `img/manifest.json` を作り直した
  （**121枚 → 112枚**。an 20→17 / cos 25→20 / v89 20→19 / ph 15 / ss 41）
- **番号は飛んでいるが（`cos05` の次が `cos07`）、配列としては詰まっている**ので表示に穴は出ない
- **確認**: Pages と同じフォールバックをするサーバで実測。
  ギャラリーのサムネ **20枚・カウンタ `01 / 20`**、PHOTO INDEX のセル **19個**、ヒーロー **142枚**、
  メイン写真は正常、**壊れた img は 0 件**、**消したファイルへのリクエストも 0 件**、`page error 0`
- v3.16.0 のマニフェスト方式に切り替えた直後なので、**`index.html` は一切触っていない**

---

## v3.16.0 — 2026-09-19

### feat: トップページの画像をマニフェスト方式に（画像を消しても歯抜けにならない）

#### 何が問題だったか

- `index.html` は画像のファイル名を**連番から組み立てていた**（`an01`〜`an20`、`cos01`〜`cos25` など）。
  そのため**1枚消すと `index.html` の数字も直さないと歯抜けになっていた**
- Cloudflare Pages は存在しないパスに **404 ではなく `index.html` を HTTP 200 で返す**。
  つまり消した画像は「壊れた img」として**枠だけ残る**。
  実測では PHOTO INDEX のグリッドに **211×158 の白い穴**が開いた
- それぞれの画像がヒーロー・ストリップ・ギャラリー・インデックスの複数箇所で使われているため、
  **4枚消すと壊れた img は 22 個**になっていた

#### 対策（C: マニフェスト）

- **`scripts/build-site-img.js` を追加**した。`cloudflare/pages/img/` を走査して
  **`img/manifest.json`** を接頭辞ごと（`an` / `cos` / `v89` / `ph` / `ss`）に書き出す
  - グループの役割は今までどおり。`ss` は**前半が strip1、後半が strip2**
  - グループに属さない `logo.png` / `cover.webp` / `cursors.png` は一覧に入れない
  - 現状 **121枚**（an 20 / cos 25 / v89 20 / ph 15 / ss 41）を書き出し、従来の連番と完全に一致
- `index.html` はマニフェストを読んでから描画するようにした。
  **読めないときは従来の連番にフォールバック**するので、取得に失敗しても白画面にはならない
- ギャラリーの枚数は固定値（`const TOTAL`）をやめて**その都度数える**ようにした。
  メイン写真の初期 `src` もマニフェスト経由になったので、**`cos01.webp` も消せる**ようになった

#### 保険（B: 読めない画像を取り除く）

- ストリップ・インデックスグリッド・ギャラリーのサムネに `onerror` を付け、
  **読めなかったものは要素ごと取り除く**ようにした
- メイン写真が読めないときは**一覧から外して次の1枚へ**進む（`dropGallery()`）
- これにより、**マニフェストの再生成を忘れても穴は開かない**

#### 確認

Pages と同じフォールバック（未知パスに index.html を 200 で返す）をするサーバで実測。
`cos01` `cos05` `v8903` `an02` の4枚を消した状態で比較した。

| | COS | サムネ | カウンタ | INDEXのセル | メイン写真 |
|---|---|---|---|---|---|
| 通常 | 25 | 25 | 01 / 25 | 20 | OK |
| **消して再生成** | 23 | 23 | **01 / 23** | **19** | OK |
| **消して再生成を忘れた** | 25（古い） | **23** | **01 / 23** | **19** | OK |

どのケースでも**穴は開かず、詰まって表示**される。`page error 0`。
（計測上残る壊れた img 3件は `src` 未設定のプレースホルダで、今回の変更と無関係）

#### これからの運用

1. `cloudflare/pages/img/` から画像を消す（あるいは足す）
2. `node scripts/build-site-img.js`
3. デプロイ

**`index.html` は一切触らなくてよい。**

---

## v3.15.0 — 2026-09-19

### feat: GAME をタブ切り替えから「下からせり上がる別ページ」に

- これまで GAME は INFO / PHOTO INDEX と並ぶ**ページ内のタブ**だった。
  Patchnotes と同じく**別ページとして開く**形にした
- **スライドの向きを変えている。** Patchnotes は左から、**Games は下から**上がってくる
  （`#gm-view` を `translateY(100%)` → `translateY(0)`。イージングと時間は Patchnotes と揃えて .72s）
- ヘッダー（60px）は共有したまま、その下だけが入れ替わる（Patchnotes と同じ作り）
- **URL を持つ**ようにした
  - 開くと `/games` に `pushState`。**ブラウザの「戻る」で閉じる**
  - **`/games` への直リンクでも開いた状態で出る**
    （未知パスは Pages が `index.html` を返すのを利用している）
  - Esc でも閉じる。ヘッダーの他のリンクを押しても閉じる
- 見た目は Patchnotes に揃えた。**大きなセリフの「Games」見出し、
  本数の表示（`2 titles`）、罫線、その下にカード一覧**
- カードを押したときの振る舞いは変わらない（`#game-panel` の iframe で起動。
  このモーダルは z-index 599 なので、z-index 150 の一覧ページの上に重なる）
- タブバーの GAME ボタンは残してあるが、**タブではなく一覧ページを開くボタン**になった
  （`TAB_IDS` は `['info','index']` に。`#tab-game` は削除）
- **確認**: Pages と同じフォールバックをするローカルサーバで実測。
  初期は `translateY(800px)` で画面外、GAME を押すと `translateY(0)` で `/games`・カード２枚・
  背景スクロールロックあり、**「戻る」で閉じて `/` に戻る**、ヘッダーの Game でも開く、
  **Esc で閉じる**、**`/games` 直リンクで開いた状態で出る**、
  カード押下で `games/crash` の iframe が開く、`page error 0`

---

## v3.14.0 — 2026-09-19

### fix: ショップで買ったアップグレードが再読み込みで全部消えていた

- **購入したレベルが一度も復元されていなかった。** ポイントだけ貯まり、
  買ったもの（反発係数・バンパー出現率・アゲルパワー・転がり摩擦・DIVEチャージ・バリア・エアリアル追加）は
  **ページを開き直すたびに全部 0 に戻っていた**
- 原因は**一時的デッドゾーン（TDZ）**。読み込み処理が

  ```js
  for (const u of UPG) lv[u.k] = clamp(+((d.lv || {})[u.k]) || 0, 0, u.max);
  ```

  と書かれていたが、**`clamp` はこの行より下で `const` 定義**されている。
  `const` は定義前に触ると `ReferenceError` になるため、この行で毎回例外が出ていた。
  それを囲っていた `try/catch` が**黙って飲み込んでいた**ので、エラーにも気づけなかった
  - `pts` はループの手前で代入しているので**ポイントだけは復元されていた**。
    「Pは残るのにアップグレードは消える」という症状はこれが原因
- **定義順に依存しないよう、この行だけ `Math.floor` と三項演算子で書き下した。**
  スクリプト全体を調べたが、**同じ型の問題はここ1箇所だけ**だった
- **確認**: 保存 → 再読み込みの往復を実測。
  `bounce:3 spacing:7 power:2 roll:1 charge:4 barrier:1 aerial:2` と 500P が**完全に一致して復元**され、
  ゲーム内の値にも反映される（反発 0.8 / 間隔 286 / パワー 1.7 / 転がり 58.7 / DIVE 15.8秒 / エアリアル 5回 / バリア有）。
  保存データを壊した場合も丸められる（`bounce:9999→30` `spacing:-3→0` `power:'x'→0` `barrier:77→1` `pts:-5→0`）
- **注意（仕様として）**: ショップの内容は `localStorage['crash-upg']` に平文で入っているので、
  **本人が書き換えれば強化し放題**。アゲルパワーは初速に直結する（最大 ×4.5）ため、
  **ランキングのスコアにも効く**。サーバー側の上限判定（経過秒数 × 最大ペース）はこれを見込んだ値にしてある

---

## v3.13.0 — 2026-09-19

### tweak: ゲームスピードを1.5倍に（挙動・飛距離はそのまま、時間だけ短く）／v3.11.0 の入れ直し

- **発射からゲームオーバーまでが長すぎたので、飛行中のシミュレーションだけ 1.5倍速にした**（`GAME_SPEED = 1.5`）
  - 弾道は**時間をスケールしても形が変わらない**（同じ曲線を速くなぞるだけ）ので、
    **重力・反発・速度上限などの数値は一切いじっていない**
  - 掛けているのは `physics()` と `camera()` に渡す時間だけ。
    **エフェクト（火花・ポップ・風・ヨクネロボの回避）とヒットストップには掛けていない**
  - 物理の刻み幅（1/120秒）は**スケール後の時間で数えている**ので、精度は落ちていない
- **確認**: ヘッドレスで同じシードのまま実測。角度30°・パワー70の単発ケースで
  **飛距離 78.4m（変更前 78.6m）・物理時間 9.88秒（変更前 9.90秒）と一致したまま、
  実時間だけ 9.88秒 → 6.59秒**（＝1/1.5）になった
- **副作用（把握したうえで入れている）**: ヒットストップとスペシャル受付（0.7秒）は**実時間のまま**なので、
  その0.7秒の間に進む距離が1.5倍になる。そのため**多段ヒットの試合展開は変更前と完全には一致しない**
  （実測でも、当たる相手がずれて 7ヒット→7ヒット のケースと 1ヒット→3ヒット のケースがあった）。
  「止まっている長さ」の体感は変わらない

### 入れ直し: 発射パワー95%以上を100%扱いに（v3.11.0 の再適用）

- v3.11.0 で入れたつもりだった `FULL_POW = 95` と PERFECT 演出が、**適用スクリプトの中断で
  ファイルに書き込まれていなかった**。今回入れ直した
- **確認**: 発射直後の初速を実測。
  **パワー 95 / 99 / 100 はいずれも 100 に切り上がり初速 216 m/s・PERFECT演出あり**、
  **94.9 は 94.9 のまま初速 207 m/s・演出なし**。閾値どおりに動いている

---

## v3.12.0 — 2026-09-19

### fix: ランキングへの直接投稿（チート）を塞いだ／管理画面にランキング管理を追加

#### 何が起きていたか

- **ランキングの登録APIに検証が一切なかった。** 名前とスコアを JSON で POST するだけで、
  **ゲームをプレイせずに任意のスコアを登録できる状態**だった（上限チェックが `9999999` のみ）
- 実際に DINO のランキングに **Normal 1位「このゲーム簡単すぎ」99999 / Hard 1位「このゲーム簡単すぎW」99999** が入っていた。
  どちらも**キリのいい手打ちの値**で、名前も挑発。**ブラウザの devtools か curl から直接叩かれたもの**
- 名前に **URL を入れた宣伝投稿**も入っていた（Hard 2位）

#### 対策（プレイした分の時間を掛けないと登録できないようにする）

- **受付トークンを導入**した。`GET /rank-token?game=dino|crash` が
  **発行時刻入りの HMAC 署名トークン**を返す。登録時はこれを必ず添えさせる
  - 署名は `ゲーム名 | 発行時刻 | IP` に対する HMAC-SHA256。**IP と紐づけているので他所で取ったものは使えない**
  - 鍵は `RANK_SECRET`（未設定なら `GITHUB_TOKEN`）。外には出ない
  - 有効期限6時間。ゲーム側はページを開いたときに1回取り、期限が近づいたら取り直す
- **スコアの上限を「経過秒数 × ゲーム上の最大ペース」で決める**ようにした
  - DINO: 実測の最大ペースは `speedMax 1100px/s ÷ 12 = 91.7 点/秒` → **110点/秒 + 100** を上限に
    （99999点を出すには**最低でも15分**かかる計算になる）
  - AGERU CRASH!!: `MAX_SPD` から **64.8 表示m/秒** → **150m/秒 + 30** を上限に
  - 絶対上限も `9999999` → **1000000** に締めた（DINO の `SCORE_MAX` と揃えた）
- **名前から URL を落とす**ようにした（`https?://` と `www.` 以降を除去）
- **確認**: 本番のエンドポイントに対して実測。
  **①トークン無しの99999 → 拒否**「ページを再読み込みしてから…」、
  **②トークンを取った直後の99999 → 拒否**「0秒では最大 128 まで」、
  **③署名を改ざん → 拒否**「登録データが不正です」、
  **④発行時刻を過去に詐称 → 拒否**（署名が合わないため）、
  **⑤妥当なスコア → 登録成功**

#### 限界について（正直に）

- **クライアントで動くゲームである以上、完全には防げない。** ゲームのコードは誰でも読めるので、
  トークンを取って本当に待ってから投稿する、という手は残る
- ただし**「待った時間ぶんしか点が入らない」**ので、**チートしても実際にプレイするのと同じだけ時間が掛かる**。
  「一瞬で99999を入れて1位」という今回の攻撃は成立しなくなった
- より強くするなら、プレイ中の入力ログをサーバーで再生して検証する（リプレイ検証）方式になるが、
  物理演算をサーバー側にも持つ必要があり、規模が大きく変わる

#### 管理画面にランキング管理を追加

- `upload-admin.html` に **Ranking パネル**を追加した。DINO と AGERU CRASH!! をタブで切り替えられる
  - ハイスコアと最新のプレイを一覧表示し、**1件ずつ削除**できる（`DELETE /admin/<game>-ranking` の `index` 指定）
  - **一覧の全消し**もできる
  - 名前は `textContent` で入れているので、名前に HTML を仕込まれても実行されない
- これまで管理用の削除は curl を打つしかなかった。**チート投稿を見つけたら管理画面から消せる**

---

## v3.11.0 — 2026-09-19

### tweak: 発射パワー95%以上を100%扱いに／100%発射の専用エフェクトを追加

- **パワーゲージ 95〜100% を「100%」として扱う**ようにした（`FULL_POW = 95`）
  - ピタリ100%で止めるのは難しすぎて、専用SE（`100hit`）がほとんど鳴らなかった
  - 95%以上で離すと**パワーを100に切り上げてから初速を計算する**ので、実際に最大距離が出る
- **100%発射の演出を追加**
  - `100hit` の効果音（従来は「ちょうど100%」のときだけだった）
  - `PERFECT!!` の大きい文字、金色の爆発（70粒）＋白の追い打ち（30粒）、衝撃波、フラッシュと画面揺れ
  - **金色のリングが3重に広がる専用エフェクト**（`drawPerfect()`）を追加。
    通常のヒットエフェクトとは別レイヤーで、速度線より前に描いている
- **この版の時点では実際には入っていなかった。**
  適用スクリプトが途中の置換失敗で中断し、**ファイルに書き出されないままデプロイしていた**
  （`perfectFx` の宣言と描画だけが入っていて、それをセットする側が無い状態）。
  **v3.13.0 で入れ直して実測確認までやった。**以後、適用後は必ず対象の識別子を grep して確かめる

---

## v3.10.0 — 2026-09-19

### feat: AGERU CRASH!! にランキングと図鑑「CRASHES」を追加

#### ランキング（DINO と同じ仕組み）

- Worker に **`/crash-ranking`** を追加した。保存先は非公開リポジトリの **`_crash_ranking.json`**
  （DINO の `_dino_ranking.json` とは別ファイル。専用のKVは使わず既存の置き場に合わせている）
  - `GET` → `{ ranking, recent }`
  - `POST` → ハイスコア登録。**同じ名前は1件にまとめ、いちばん遠い記録だけ残す**
  - `POST { type:'recent' }` → **ゲームオーバーのたびに自動で「最新のプレイ」に積む**（名前が空なら「名無し」）
  - 管理用に **`DELETE /admin/crash-ranking`** も用意（`clear` / `index` / `name+score` の3通り）
  - DINO と違う点は2つだけ: **ハードモードが無い**ことと、**スコアが飛距離(m)なので小数点2桁を持つ**こと
  - 書き戻しの競合（同時投稿）は sha 衝突を検出して最大4回やり直す（DINO と同じ）
- ゲーム側は **タイトルとリザルトの両方に `RANKING` ボタン**を追加
  - リザルトには**名前の入力欄と「ランキングに登録」ボタン**。名前は `localStorage['crash-name']` に保存
  - ランキング画面からは**自己ベストをいつでも登録**できる（何度でも押せる）
  - 入力欄とボタンは `pointerdown` を止めている。**止めないと「クリックでリトライ」が走って画面が閉じる**
  - 名前は必ず `textContent` で入れている（`innerHTML` に混ぜない）

#### CRASHES（図鑑）

- タイトルに **`CRASHES` ボタン**を追加。**1ページに1キャラ**で、大きな立ち絵と解説を出す
- **全12ページ** — ヒコ／アゲル（自転車・エアリアル・ダイブ）／バンパー8種
- **バンパーの数値は `TYPES` から組み立てている**ので、バランス調整をすると図鑑の表示も自動で追従する
  - 出現率・打ち上げ角・速度倍率・最低速度・スペシャル条件・スペシャルの値・アゲルスペシャル
  - 自機側も `rollVal()` `powerVal()` `aerialMax()` `chargeVal()` を参照しているため、
    **ショップで強化した現在値がそのまま出る**
- 操作は ◀ ▶ ボタン・下のドット・**キーボードの左右キー**。Esc でも閉じる。端で一周する
- 画像が読めないキーは非表示にして、文章だけで成立するようにしている
- `uiOpen()` に `ov-rank` / `ov-dex` を追加し、開いている間はクリックがゲームに届かないようにした
- **確認**: Worker をデプロイして疎通を実測。`GET` が `{ranking:[],recent:[]}` を返し、
  `POST` でハイスコアと最新プレイの両方に積まれること、ゲーム画面から**`file://` と `http://` の両方で
  ランキングを取得して表示できる**こと、図鑑が**12ページすべて正しい画像・名前・数値で表示され一周すること**、
  `page error 0`。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）
- **やってしまったこと**: 疎通確認のため**本番のランキングにテスト投稿を2件入れた**
  （ハイスコア1件・最新のプレイ1件、どちらもスコア 12.34）。
  ハイスコア側は curl の文字コードの都合で**名前が文字化けしている**。
  管理用エンドポイントには `ADMIN_SECRET` が要るためこちらでは消せない。消し方はリリースノート外に記載

---

## v3.9.0 — 2026-09-19

### tweak: DIVEのチャージを溜まるのに5倍時間がかかるように／ショップに「DIVEチャージ」を追加

- **DIVEのチャージ回復を 1/5 にした**（`CHARGE_RATE` 62/3 → 62/15）
  - 満タンまで **約 4.8秒 → 約 24.2秒**。高度の制限が無い分、連発しすぎていた
- **ショップに「DIVEチャージ」を追加**（7項目目）
  - 1回ごとに回復 +0.55%/秒、**最大30回**。**買い切ると変更前と同じ速さ（約4.8秒）に戻る**
  - 価格はフィボナッチ式。カードには回復速度ではなく**「満タン ○○秒」**で出している（そちらの方が分かりやすいため）

  | 購入回数 | 0 | 10 | 20 | 30 |
  |---|---|---|---|---|
  | 回復 %/秒 | 4.13 | 9.64 | 15.16 | 20.67 |
  | 満タンまで | 24.2秒 | 10.4秒 | 6.6秒 | **4.8秒** |

- 画面上部のステータス表示に `DIVE 24.2s` を追加。`lv.charge` として `localStorage['crash-upg']` に保存される
- ショップの脚注を「**バリアとエアリアル追加以外**は買うたびに値上がり」に書き換えた（項目を列挙すると長すぎるため）
- **確認**: `file://` で実測。上表の数値、`lv`/`UPG` に `charge` が入ること、
  `saveUpg()` で `localStorage` に保存されること、ショップが**3×3の7枚で崩れずに並ぶこと**
  （スクリーンショットで確認）、`page error 0`。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v3.8.0 — 2026-09-19

### tweak: NEXTパネルを顔のアップに／自機の表示位置を中央寄りに

- **右上の NEXT パネル（次の5体）を顔のアップにした**
  - これまでは全身の立ち絵を 42px に縮めて並べていたが、8種とも同じ全身構図なので
    **縮小すると誰が来るのか判別できなかった**
  - `faceIcon()` を追加し、**絵の顔まわりを正方形で切り出して丸く抜いて**描くようにした
    - 切り出し位置は絵の高さに対する比率で持っている（中心 `FACE_CY = .17` / 半辺 `FACE_R = .20`）。
      1200×1824 の絵なら 730×730 を上から切り出す形で、**ネコ耳・帽子・ナース帽まで入り、顎で切れない**
    - 比率で持っているので、**絵を差し替えても構図（上寄りに顔）が同じならそのまま使える**
    - 画像が読めていないキーは従来どおりプレースホルダにフォールバックする
  - 縁に細いリングを足して丸く抜いたことが分かるようにした。ボスは赤、スペシャル条件が
    整っているキャラは従来どおり黄色く明滅する
- **画面外バンパーのマーカー（スマブラ風の画面端アイコン）も同じ顔アップにした**
- **自機の画面上の横位置を 30% → 45% に移した**（`CAM_X_FRAC`）
  - バンパーに当たる瞬間は自機とバンパーが同じ位置に来るため、**衝突が画面左端寄りで起きて見づらかった**
  - 先が見える範囲は 840px → 660px に減るが、NEXTパネルと画面外マーカーで先の状況は分かるので問題ない
  - 1つの定数にしたので、寄せ具合の調整はここだけ触ればよい
- **確認**: `deviceScaleFactor=2` でスクリーンショット確認。
  NEXTパネルの5枠すべてが**顔のアップで表示され、服装や帽子で見分けがつく**こと、
  画面外マーカーも顔になっていること、自機が画面の 540px（＝1200×0.45）の位置に来ること、`page error 0`。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### fix: ゲーム内の画質が悪い問題を修正（canvas を実ピクセル解像度で描くように）

- **元画像は高解像度なのにゲーム内だとボケる**問題を修正した。原因は画像側ではなく canvas の解像度
  - canvas の中身（バッキングストア）は `width="1200" height="600"` 固定だったのに、
    CSS では `width:100%` で表示サイズが伸びる
  - **`devicePixelRatio` が 1 より大きい環境**（Windows の表示スケール 125%/150%、Retina など）では、
    1200×600 の絵をブラウザが物理ピクセルまで引き伸ばすため、**元画像がいくら高解像度でもボケる**
- **バッキングストアを「CSS上の表示サイズ × devicePixelRatio」に合わせるようにした**（`fitCanvas()`）
  - 描画の先頭で `CTX.setTransform(DPR, 0, 0, DPR, 0, 0)` を入れているので、
    **ゲーム側のコードは今までどおり 1200×600 の論理座標のまま**で書ける
  - `devicePixelRatio` は 3 で頭打ちにして、4K・高倍率環境で描画量が爆発しないようにしている
  - canvas はサイズを代入するとコンテキストの状態（変換・補間設定）が初期化されるので、
    サイズ変更は「変わったときだけ」、設定の投入は毎フレーム `draw()` の先頭で行う
  - `resize` と `ResizeObserver` の両方で追従する（iframe のモーダル開閉でも表示サイズが変わるため）
- **あわせて `imageSmoothingQuality = 'high'` を指定**した。バンパーは 1200×1824 の絵を
  267px 程度まで縮小して描いており、**4.7倍の縮小**になる。既定の補間だと縮小時にジャギが出る
- 入力座標への影響なし（このゲームはマウス座標を一切使っていないため）
- **確認**: `deviceScaleFactor` 1 と 2 の両方で実測
  - dsf=1 → CSS 1200×600 / バッキングストア **1200×600** / DPR 1.0
  - dsf=2 → CSS 1200×600 / バッキングストア **2400×1200** / DPR 2.0
  - `imageSmoothingQuality` が `high` になっていること、dsf=2 のスクリーンショットで
    HUD・背景・自機が**二重拡大されず正しい位置に鮮明に描かれる**こと、`page error 0`
  - **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### fix: 転がり→バンパー→転がりが永久に続く問題を修正／ショップに「転がり摩擦」を追加

- **地面を転がっている間ずっとバンパーに当たり続ける問題**を、原因2つから直した
- **A. バンパーの最低打ち出し速度を現在速度に連動させた**（`RELAUNCH_CAP = 2.2`）
  ```
  sp = Math.max(speed() * mul, Math.min(min * POW_X, speed() * RELAUNCH_CAP));
  ```
  - これまでは `min * POW_X` が**現在速度と無関係に満額**だったため、止まりかけで当たっても
    毎回 99〜121 m/s で撃ち出され、**転がり→バンパー→転がりが永久に切れなかった**
    （ベビーの例: 99 m/s・45度で飛距離222m ＋ 転がり233m ＝ 455m で、間隔300mの次に必ず届く）
  - 新ルールは「**バンパーは現在速度の2.2倍までしか出せない**」。45 m/s 以上なら従来と完全に同じ挙動で、
    遅いときだけ弱くなる（5 m/s なら11 m/s、20 m/s なら44 m/s）。速度を維持するほど有利な方向にも働く
- **B. 転がりの減速を 21 → 60 m/s² に上げた**（`ROLL_FRIC`）。転がり距離は `v²/(2a)`

  | 着地時の横速度 | 変更前(21) | 変更後(60) |
  |---|---|---|
  | 60 m/s | 86m | 30m |
  | 100 m/s | 238m | 83m |
  | 150 m/s | 536m | 188m |
...
- **ショップに「転がり摩擦」を追加**（6項目目）
  - 1回ごとに摩擦 −1.3、**最大30回**。買い切ると **60 → 21**（＝変更前の転がりやすさ）になる
  - 価格はフィボナッチ式（反発係数・バンパー出現率・アゲルパワーと同じ）
  - 画面上部のステータス表示にも `ROLL` を追加。ショップの脚注も4項目に更新
  - `lv.roll` として `localStorage['crash-upg']` に保存される（既存のセーブは 0 から始まる）
- **確認**: `file://` で実測。`rollVal()` がレベル 0/10/20/30 で **60 / 47 / 34 / 21**、
  打ち出し下限が現在速度 5/20/45/60/100 m/s に対して **11 / 44 / 99 / 99 / 99**（45以上で頭打ち＝従来どおり）、
  `lv` と `UPG` に `roll` が入ること、`saveUpg()` で `localStorage` に保存されること、
  ショップが**3×2の6枚できれいに並ぶこと**（スクリーンショットで確認）、`page error 0`。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: ヨクネロボに当たったとき、右へすっと動いて避ける演出を追加

- ヨクネロボに当たると、**その場に残ったまま右へ 9m（ワールド）スライドして避ける**ようにした
  - `markHit()` でロボのときだけ `dodgeT` を立て、`dodgeOf()` が経過時間から横ズレを返す
  - イージングは **ease-out（最初が一番速くてそっと止まる）**。0.45秒で avoid しきる
  - 描画位置にだけ効かせている（`drawChars()` の `SX(c.x + dodgeOf(c))`）。当たり判定は移動しない
    （当たった時点で `hit` が立って判定対象から外れるため、動かす意味がない）
- 進行は `update()` の `dt`（スロー込みの時間）で進めているので、
  **ヒットストップ中は避ける動きもスローになり、当たった瞬間の演出と速度が揃う**
- 避ける距離の目安: 絵の横幅はワールドで約11.2m なので、**ほぼ自分の体1つぶん**右にずれる
- `c_robo_hit.webp` を置けば、避けている間の絵もそちらに差し替わる（置かなければ通常の絵のまま）
- **確認**: 構文チェックと、`dodgeOf()` が描画にだけ入っていること（当たり判定側に入っていないこと）を確認。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: バンパーを当たっても消さず被弾絵に差し替える／全バンパー同サイズ／表示1.5倍

- **バンパーの表示倍率を 2倍 → 1.5倍**にした（`CHAR_DRAW`）。当たり判定は従来どおり据え置き
- **バンパーごとのサイズ違いを廃止**し、**ボスを含む8種すべて同じ大きさ**にした
  - `CHAR_BASE_H = 260` を追加し、描画・当たり判定・NEXTパネルの3箇所すべてをこの1つの値から計算するようにした
  - 従来は `ASSETS` の `h`（カニ160〜ボス360の2.25倍差）で大小を付けていた。
    **`ASSETS` の `w/h` は画像が無いときのプレースホルダ専用**になり、大きさには使われない
  - 横幅は画像の縦横比から決まるので、差し替えられた8枚（すべて1200×1824）はきれいに揃う
- **当たったバンパーが吹っ飛んで消える演出を廃止**した
  - `knockAway()`（速度と回転を与えて1.4秒かけてフェードアウト）を廃止し、
    **当たった印を付けるだけの `markHit()`** に置き換え。char の `kt` / `kvx` / `kvy` / `krot` と、
    それを動かしていた物理ループも削除した
  - 代わりに**当たった瞬間から絵を `c_<種類>_hit.webp` に差し替える**（`IMG_HIT_OF`）。
    その場に立ったまま残り、画面外に出たところで消える
  - **画像がまだ無いキーは通常の絵のまま残る**ので、用意できたものから順に置けばよい
  - 被弾絵の**描画の高さは通常の絵と同じ**。横に広い絵（アッパーで吹き飛ばす等）にすれば横に広く描かれる。
    **当たり判定は絵に関係なく `CHAR_BASE_H` 固定**なので、大きい絵にしても当たりやすさは変わらない
- **エアリアル／ダイブの効果音を `slam` に統一**した（SLAM と同じ音）。
  `aerial` / `dive` のキーと音源は差し替え用にそのまま残してあるが、現在は鳴らしていない
- 副作用: `knockAway()` が1ヒットにつき `rnd()` を3回消費していたのをやめたため、
  **同じシードでもステージの並びが以前と変わる**（乱数列の消費位置が変わるため）
- **確認**: 構文チェックと、`knockAway` / `kt` / `kvx` / `kvy` / `krot` の残存が0件であること、
  `CHAR_BASE_H` が描画・判定・NEXTパネルの3箇所に入っていることを確認。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### tweak: バンパーの表示サイズを2倍に（当たり判定は据え置き）

- バンパー画像の差し替えに合わせて、**画面上の大きさだけ2倍**にした
- `CHAR_DRAW = 2` を追加し、**描画の箇所にだけ掛けている**
  - `drawChars()` の `hpx` — 実際に描く高さ
  - NEXTパネル（画面外マーカー）の `hgt` — マーカーが**見た目の中心**を指すように合わせた
- **当たり判定には掛けていない**（`physics()` の `hgt` / `MAX_CH_H` は `CHAR_SCALE` のまま）。
  自機の `PLAYER_SCALE` / `PLAYER_FLY_SCALE` と同じ「見た目だけ」の扱い
- そのため**絵の上半分は当たらない**。判定は今までと同じ位置・同じ大きさのまま
- 差し替えられた画像は8種すべて 1200×1824（アスペクト比が揃ったので、
  `ASSETS` の推奨サイズ `h` の比だけで大小関係が決まる＝ボスが一番大きい）
- **確認**: 構文チェックと、`CHAR_DRAW` が描画2箇所にだけ入っていること
  （当たり判定・`MAX_CH_H` には入っていないこと）を確認。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### tweak: アゲルスペシャルから「飛距離が100の倍数の地点」条件を撤廃

- アゲルスペシャルの発動条件から **「表示上の距離の下2桁が00（＝ワールドで1000の倍数）」を外した**
  - 判定幅がワールド±15m（表示±1.5m）しかなく、狙って合わせるのがほぼ不可能だったため
  - 発動条件は **「ブロック状態 × アゲル系（ベビー／ゴス／デンパ／ボス）に当てる」だけ**になった
- **これに伴いヨクネロボの役割が変わった。** これまでは「次のアゲル系の効果を消す」ブロックで、
  100の倍数地点でなければ `BLOCKED…` と表示されて効果が潰れていたが、
  **今後はブロック状態でアゲル系に当てれば必ずアゲルスペシャルになる**＝実質的に「次のアゲル系を強化する布石」
  - `BLOCKED…` の表示と、そこで鳴っていた `hit_slow` の分岐は**到達しなくなったので削除**した
  - ヨクネロボ自体の効果（角度そのまま・速度×0.94）は変更なし
- 「遊び方」画面のアゲルスペシャルの説明と、`TYPES.robo` の `desc` も新しい仕様に合わせて書き換えた
- 発動率の目安: ヨクネロボの出現比は 6/58（約10%）、アゲル系の合計は 30/58（約52%）なので、
  ロボに当てた後は**次のヒットの約半分がアゲルスペシャル**になる
- **確認**: 構文チェックと、`near100` / `BLOCKED` の残存が0件であることのみ確認。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: ボイスを追加（キャラヒット／エアリアル・ダイブ／ゲームオーバーでランダム再生）

- **効果音とは別レイヤーのボイスを追加**した。`sfx-crash/hit/` 以下から1つランダムに鳴らす
  - `hit/`（11点）— **地面以外＝キャラに当たったとき**。`onHit()` の先頭に入れているので、
    ベビー・ゴス・デンパ・カニ・フグ・ネコ・ヨクネロボ・ボス**すべてのヒットで鳴る**（地面バウンドでは鳴らない）
  - `hit/diveArial/`（4点）— **エアリアル／ダイブを撃ったとき**（`aerial()` / `dive()`）
  - `hit/over/`（5点）— **ゲームオーバー**（`gameOver()`。デンパで復活した場合は鳴らない）
- **ボイス同士は重ねない（1ch）。** 連続ヒットで重なると何を言っているか分からなくなるため、
  新しいボイスが鳴るときに前のボイスを止める。効果音・BGM とは重なる
- **直前と同じファイルは選ばない**ようにした（同じ声が2連続で出ない）
- 抽選は `Math.random()` を使っている。**地形の抽選に使っている `rnd()`（シード付きLCG）は消費しない**
  — 消費するとボイスの抽選結果でステージ構成が変わってしまうため
- リセット時とサウンドOFF時はボイスを停止する
- **音量を全20点そろえた**（`v` は 0.85・全体音量スライダーも効く）
  - 元は **r=0.044〜0.271 の約6倍差** → 全部 **r=0.219〜0.220**（誤差0.5%以内）に正規化
  - `volume` の上限が1で全ファイルが1.2〜5.0倍の持ち上げを必要としたため、**ファイル側を書き換えた**
  - ピークで頭打ちにすると歪むので、しきい値0.7から上だけ **tanh ソフトリミッタ**でなめらかに潰し、
    リミッタで下がった分をゲインに戻す反復で目標ラウドネスに寄せている（結果のピークは全て ≤0.998）
  - `「えい！」.mp3` `「ドン！」.mp3` はデコードして `.wav` を書き出した（**元の mp3 は残してある**）
  - **正規化前の原本は `assets/crash/voice-orig/` にバックアップ済み**
- 静的サイトはフォルダを列挙できないため、ファイル一覧は `crash.html` の `VOICE` に配列で持っている。
  **ボイスを足したらこの配列にも足すこと**（README に手順を記載）
- **確認**: `file://` と `http://` の両方で実測し**挙動が一致**。
  20点すべて読み込み成功（読めなかったファイル0件）、`hit` を300回抽選して**11ファイル全部に行き渡り連続重複0回**、
  再生位置が進むこと、**新しいボイスが前のボイスを止める**こと（前の要素が `paused=true` / `currentTime=0`）、
  全体音量20%で音量が 0.85×0.2=0.17 になること、日本語ファイル名（`「えい！」.wav`）も読めること、`page error 0`。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### tweak: リザルトの操作ヒントと上部バーの「作成中」バッジを削除

- リザルト画面（`#ov-over`）から「SPACE でもリトライできます」を**削除**した
  （v3.0.0 のタイトル側と同じく**表示だけの削除**で、SPACE でのリトライは今までどおり効く）
- 上部バーの **「作成中」バッジ（`.wip-badge`）を削除**し、使われなくなった CSS も一緒に消した
- これでタイトル・リザルトとも**説明文が一切なくなり**、ロゴ／スコア／ボタンだけになった

---

## v3.0.0 — 2026-09-19

### tweak: タイトル画面の説明文を3行削除

- タイトル画面（`#ov-title`）から以下を**削除**した
  - 「SPACE でも開始できます」（操作自体は残っているので、SPACE でも今までどおり開始できる）
  - 「アゲルが自転車でヒコをはね飛ばす。／地上のキャラに当てて、どこまでも飛ばせ。」
  - 「⚠ このゲームはまだ作成中です。仕様や見た目は変更されることがあります」
- 残るのは **ロゴ／START・SHOP ボタン／ベスト記録／所持ポイント**だけになった
- リザルト画面の「SPACE でもリトライできます」と、上部バーの「作成中」バッジは**そのまま**

---

## v2.999.0 — 2026-09-19

### tweak: 全体音量スライダーの既定値を 80% → 50% に

- 画面右上の音量スライダーの初期値を **50%** にした
- `masterVol` の初期値と、JS が動く前に表示される `<input value>` およびその横の数値ラベルの**3か所をそろえている**
- すでに一度でも音量を触った環境は `localStorage['crash-vol']` の値が優先されるので変わらない

---

## v2.998.0 — 2026-09-19

### fix: 音が全く鳴らなくなった問題を修正（Web Audio をやめて素材側を正規化）

- **v2.997.0 で音が完全に出なくなっていた。原因は自分の変更。**
  音量を1倍超まで持ち上げるために全ての音を `AudioContext` の `GainNode` 経由にしたが、
  **`createMediaElementSource()` は `file://` で開いたページだと素材を別オリジン扱いにして出力を無音にする**
  - 実測: 同じコードで **http 配信なら MASTER の出力ピーク 0.297 → `file://` では 0.000**
  - ローカルで HTML を直接開いて確認する使い方なので、この経路には依存できない
  - v2.997.0 の確認をローカルの HTTP サーバー経由でしか行わなかったため見逃した。
    **`file://` でも開く前提のページは `file://` でも確認する**
- **Web Audio を全廃し、`HTMLAudioElement.volume` だけで音量を決める方式に戻した**
  - `gain = v * SFX_TGT / r` で素材ごとの音量差を打ち消す仕組み（v2.997.0）はそのまま維持
  - `volume` の上限は 1 なので、**`gain` が 1 を超えていた3つは素材側を正規化**した
    - `click` — `click.mp3`（r=.074・gain 1.49）をデコードして **×2.98 した `click.wav`** を書き出し
    - `tame` — `tame.mp3`（r=.033・gain 4.00）をデコードして **×6.60 した `tame.wav`** を書き出し
    - `stop` — `stop.wav`（r=.083・gain 1.86）を **×2.65 して上書き**
  - mp3 を再エンコードする手段（ffmpeg / lame / sox）が無いため wav として書き出し、
    `SFX_LIST` に **`pref:'.wav'`**（このキーだけ wav を優先して読む）を追加した。**元の mp3 は消していない**
  - `wind` は104秒の mp3 で wav 化するとサイズが大きすぎるため正規化せず、
    `v` を `.4`→`.33` にして最高速でも gain 1.008（実質1.0で頭打ち）に収まるようにした
  - `100percent` も `v` を `.85`→`.82` にして gain 0.997 に収めた
- **これで全21キーの gain が 1 以下に収まり、`volume` だけで正しい比率が出せる**
  （`click` 0.50 / `tame` 0.60 / `stop` 0.70 / `hit` 0.27 / `crash` 0.38 / BGM 0.64）
- **やってしまった事故**: `stop.wav` の正規化で**正規化済み（±1）の値に 32767 を掛け戻すのを忘れ**、
  全サンプルが 0 に丸められて**無音のファイルで上書き**した。git 未追跡で復元できなかったが、
  CC0 の配布元（OpenGameArt「8-Bit Sound Effect Pack Vol.001」の `Exit 1.wav`）から取り直したところ
  **サイズ・ラウドネス・ピークが破壊前と完全一致**（176692バイト / r=0.0831 / peak=0.1959）したため復旧できた
- **確認**: `file://` と `http://` の両方で実測。**両者の挙動が完全に一致**し、
  21キーすべてが音源に解決（無音キー0件）、単発・ループ・BGM すべて**再生位置が進んでいる**こと、
  `volume` が期待値どおり（`hit` 0.216 = gain 0.27 × master 0.8 など）、
  スライダーで `masterVol`・`localStorage['crash-vol']`・再生中の BGM 音量が 0.4 に追従すること、`page error 0`。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: 全効果音の音量を実測値でそろえた／全体音量スライダーを追加

- **素材ごとの音量差を打ち消す仕組みにした。** 実測では**最小 0.033（`tame.mp3`）〜最大 0.693（`hit.mp3`）で 21倍の開き**があり、
  「ヒットは割れるほど大きいのに溜め音は聞こえない」状態だった
  - `sfx-crash/` の**全ファイルを Web Audio で実際にデコードして計測**した
    （モノラル化 → 300ms 窓の移動 RMS の最大値＝体感の大きさに近い指標）
  - `SFX_LIST` の各行に**実測ラウドネス `r` を追加**し、鳴らす音量を **`gain = v * SFX_TGT / r`**（`SFX_TGT = 0.22`）に変更
  - `v`（従来の `v`）は「**演出としてどれくらい目立たせたいか**」だけを表す値になり、素材の大小に左右されなくなった
- **ファイルは1つも書き換えていない。** mp3 を再エンコードする手段（ffmpeg / lame / sox）がこの環境に無いため、
  wav だけ書き換えると mp3 の4本（`click` `tame` `hit` `wind`）だけそろわない。**再生時のゲインでそろえる方式に統一した**
- **Web Audio 経由に変更**（`acInit()` / `setGain()`）
  - 小さい素材は**1倍を超えて持ち上げる必要がある**（`tame` 4.00倍・`stop` 1.86倍・`click` 1.49倍）が、
    `HTMLAudioElement.volume` は最大1で頭打ちになるため、**全ての音を `GainNode` 経由**にした
  - Web Audio が使えない／`AudioContext` がまだ起きていない場合は、**従来どおり `volume` に 0〜1 でクランプ**して鳴らす（無音にはならない）
  - 最初の `pointerdown` / `keydown` / `touchstart` で `AudioContext.resume()` する（自動再生制限の解除）
  - 単発音は複製ノードごとに `MediaElementSource` を作り、`ended` で `disconnect()` してグラフから外す
- **全体音量スライダーを追加**（画面右上の ♪SOUND の隣）
  - 0〜100%、既定80%。`MASTER` GainNode 1つに効くので**効果音・ループ音・BGMすべてに同時に反映**される
  - `localStorage` の **`crash-vol`** に保存。スライダーを離すとその音量で `click` を鳴らして試聴できる
- **ループ音の呼び出しを「ミックス値」に統一**（`seLoop(k, mix)`）。`wind` は `.55`→`.4`、`roll` は `.5`→`.35` に付け替えた
  （持続音は同じ RMS でも大きく感じるため、単発より控えめにしている）
- **BGMも同じ基準でそろえた**（`BGM_GAIN = .42 * SFX_TGT / .145` ＝ 0.637）
- **ショップの所持ポイントは以前から `localStorage` に保存済み**（`crash-upg` に `{pts, lv}`）。
  ポイント獲得時（`gain > 0`）と購入時の両方で `saveUpg()` を呼んでいる
- **確認**: ヘッドレスブラウザで実測。`AudioContext` が **running**、**21キーすべてが音源に解決**（mp3 6件・wav 15件・無音0件）、
  ゲインが**単発・ループ・BGMすべて `GainNode` に載る**こと（`tame` 4.00 / `wind` 1.22 / BGM 0.637）、
  スライダー操作で **`MASTER.gain` と `localStorage['crash-vol']` が 0.35 に追従**すること、`page error 0`。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: 用意された効果音4種をイベントに接続（click / tame / 100percent / 100hit）

- `sfx-crash/` に置かれていたのに未接続だった4ファイルを、指定されたタイミングに割り当てた
  - **`click.mp3`** — クリックしたとき。キー名を `ui_click` → **`click`** に改名し、
    **角度決定・START・SHOP・戻る・購入の9箇所**をこのキーに寄せた（CC0側の `ui_click.wav` も `click.wav` にリネーム）
  - **`tame.mp3`** — **押しっぱなしにしている間**。`loop:true` で登録し、
    角度決定（`press()` の `'angle'`）で鳴り始め、**離した瞬間（`release()` の先頭）で停止**する
  - **`100percent.mp3`** — **ダイブのチャージが100%になった瞬間**。物理更新でチャージが
    100未満→100に**乗り越えたフレームだけ**鳴らす（毎フレーム鳴らないようにしている）
  - **`100hit.mp3`** — **100%パワーで発射したとき**（`Math.round(power) >= 100`）。
    **スペシャル発動時**（`trySpecial()` / ボスチャンス成功）にも同じ音を使う
- スペシャルは従来 `special` を鳴らしていたが、**2つ重ねると濁るため `100hit` に置き換えた**（`special.wav` は差し替え用に残置）
- **確認**: 置換箇所の静的チェックのみ（`ui_click` の残り0件・`100hit` 4箇所・`tame` 2箇所・`100percent` 2箇所、構文OK）。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: 効果音を一式そろえた（CC0素材）／BGMを発射〜ゲームオーバーで再生

- **効果音18種のうち16種を CC0 素材で用意**した
  - **8-Bit Sound Effect Pack Vol.001**（@Shades / CC0・OpenGameArt）から用途に合わせて割り当て
  - **`wind` / `roll` のループ音は合成**した。一回モノの音をループさせると継ぎ目で途切れるため、
    ノイズの端をクロスフェードした2秒のシームレスループを生成している
  - 変換ツール（ffmpeg / lame / sox）がこの環境に無いため、**wav のまま**配置した
- **読み込みを「mp3優先・無ければwav」に変更**（`makeSfx()`）
  - すでに置かれていた `hit.mp3` `wind.mp3` はそのまま優先して使われる
  - 後から同じキー名の mp3 を置けば自動でそちらに切り替わる（wav を消す必要はない）
- **BGMを追加**（`アゲル CRASH!!.mp3`）
  - **発射と同時にループ再生を開始し、ゲームオーバーで停止**する（音量0.42）
  - リスタート時は頭出しして再生。♪SOUND を切ると即停止する
  - ファイル名にスペースと日本語が含まれるため `encodeURIComponent` でURL化している
- **やってしまった事故**: `sfx-crash/` に**利用者が用意した音源があることに気づかず書き込み**、
  **`roll.wav`（295KB）を合成音（75KB）で上書きして失わせた**。git 未追跡のため復元できない。
  以後、既存ファイルのあるディレクトリへ書き込む前に中身を確認する
- **未接続のまま残っている音**: `click.mp3`（キー名は `ui_click`）、`tame.mp3`、`100hit.mp3`、`100percent.mp3`。
  キー名が一致しないか、対応するイベントがまだ無い
- **確認**: ブラウザで実測。**18キーすべてが音源に解決**（mp3 2件・wav 16件・無音0件）、
  BGMは**発射で再生開始・ゲームオーバーで停止**、`page error 0`

---

## v2.994.0 — 2026-09-19

### tweak: タイトル・リザルト画面の背景ぼかしを解除

- `.ov`（オーバーレイ共通）に掛かっていた `backdrop-filter: blur(2px)` を、
  **タイトル（`#ov-title`）とリザルト（`#ov-over`）だけ無効化**した
- **暗い覆い（`rgba(10,12,18,.62)`）は残している**ので、文字の読みやすさは維持される
- 「遊び方」と「ショップ」のぼかしはそのまま。ショップは背景がほぼ不透明（`.94`）なので元々ぼかしの影響はほぼ無い
- **確認**: CSSの適用箇所のみ確認。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.993.0 — 2026-09-19

### feat: 購入画面を独立させ、START / SHOP のボタン構成に

- **タイトル画面をボタン式に**した。`START` と `SHOP` の2つ（`SPACE` でも開始できる）
  - 従来はタイトルに購入リストを直接並べていたが、**専用のショップ画面へ分離**した
- **リザルト画面も `RESTART` / `SHOP` の2ボタン**にした。ショップから戻ると**リザルト画面に復帰**する
  （どちらの画面から来たかを `shopFrom` で覚えている）
- **ショップ画面（`#ov-shop`）を新設**。参考UIのレイアウトに合わせた構成
  - 上部バー: **戻る（←）／ SHOP ／ 所持ポイント**
  - 左に**立ち絵**（`shop_ageru.webp`）、右に**カードのグリッド**（3列）
  - 各カードに **名前／レベル（n / 上限）／効果／現在値／購入ボタン（価格）** を表示
  - 買える項目は金色で強調、上限到達は水色で `MAX`、ポイント不足は暗く表示
  - 狭い画面（760px以下）では立ち絵を隠して2列にする
  - 配色は参考UI（白基調）ではなく**ゲーム本体の暗色＋ピンク/金**に合わせた
- **立ち絵素材を追加**: `img-crash/shop_ageru.webp`（524×900 / 95KB、`00585-473137864.png` から
  余白除去→高さ900→webp化）。**ファイルが無い場合は自動で非表示**にする
- **オーバーレイ上のクリックでゲームが始まらないように**した
  （`.ov` 内のクリックは発射の入力判定から除外。ショップ表示中は `press()` / `release()` も無効）
- **確認**: ブラウザで10項目を実測
  - タイトルにSTART/SHOPが出る、**余白クリックでは開始しない**
  - SHOP→カード5枚と立ち絵が表示、購入でレベル・ポイント・現在値が更新される
  - 戻る→タイトル復帰、START→`angle` へ遷移
  - 終了→RESTART/SHOP表示、**リザルトからSHOPへ行って戻るとリザルトに復帰**
  - RESTART→タイトルへ、`console error 0 / page error 0`

---

## v2.992.0 — 2026-09-19

### tweak: スペシャルの受付表示を画面中央に大きく

- スペシャルのクリック受付が**画面上部（y=92）に24px**で出ていて気づきにくかったため、**画面中央に大型化**した
  - 文字: **24px → 56px**（2.3倍）、位置: 上部 → **画面中央（y=252）**
  - 背景に負けないよう**黒の縁取り（9px）**を追加し、**6%の脈動**（毎秒約3.5回）で視線を引く
  - 残り時間のバー: 幅 **180px → 420px**、高さ **8px → 14px**
- 受付時間そのもの（`SP_WIN = 0.7秒`）と、その間ずっとヒットストップがかかる挙動は変更なし
- **確認**: 構文チェックとサイズ・座標の確認のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.991.0 — 2026-09-19

### feat: タイトルにステータスアップ画面を追加（ポイント制の永続強化）

- **ポイント（P）**: 1プレイ終了時に **表示距離100mごとに1P** 獲得。リザルトに獲得ぶんを表示する
- **アップグレード5種**（`UPG` / `lv`）。購入状況とポイントは **localStorage（`crash-upg`）に保存**され次回も引き継ぐ

  | 項目 | 効果 | 上限 | 価格 |
  |---|---|---|---|
  | 反発係数 | 1回ごとに +0.1（基準0.5） | 30回 | フィボナッチ |
  | バンパー出現率 | 1回ごとに 間隔 -2（基準300・下限20） | 100回 | フィボナッチ |
  | アゲルパワー | 1回ごとに 発射の威力 +0.1（基準1.5） | 30回 | フィボナッチ |
  | バリア | 即死を1回だけ防ぐ（毎プレイ1回） | 1回 | 100P |
  | エアリアル追加 | 1回ごとに +1回（基準3回） | 3回 | 100P |

- **フィボナッチ価格**は 1, 2, 3, 5, 8, 13, 21, 34 … と1回ごとに上がる
  （10回目89P・20回目10,946P・**30回目1,346,269P**。上限まで買い切るのは事実上不可能な設計）
- **バリアの挙動**: ボスアゲルにスペシャル条件を満たさず接触したとき、**即終了の代わりに1回だけ肩代わり**して
  45度に打ち上げ直す（青い画面バーンと `BARRIER!!` 表示）。買い切りで、**毎プレイ1回**使える
- **アゲルパワーは発射（最初の激突）のみ**に乗る。エアリアル・ダイブ・キャラの打ち上げには影響しない
- **HUDに現在の強化状況を表示**（速度表示の下に `BOUNCE / GAP / POWER / AERIAL / BARRIER`）
- **画面右下の DIVE ボタンを削除**（HTML・CSS・イベント・状態切り替えをすべて除去）
- ショップの行をクリックしてもゲームが始まらないよう、`#shop` 内のクリックは
  発射の入力判定から除外している（`pointerdown` / `pointerup` の両方）
- **注意（要判断）**: 反発係数は**5回買うと 1.0 に達し、着地でエネルギーが減らなくなる**。
  仕様どおり上限30回（最大3.5）で実装しているが、1.0以上では跳ね続けて**プレイが終わらなくなる**可能性がある
- **作業中に作り込んだ不具合と修正**: ショップのコードを挿入する際、目印にした `/* 遊び方 */` という
  コメントが **CSS と JS の両方に存在**していたため、置換が**先に出現するCSS側**に当たり、
  JSのコードが `<style>` ブロックの中に入り込んで `drawShop is not defined` になっていた。
  こちらの構文チェックが `<script>` の中身だけを見ていたため検出できず、ブラウザでの配線確認で発覚した。
  CSS側から取り除いてJS側へ入れ直し済み
- **表示の作り込み（配線確認で見つけて修正）**
  - レベル欄（`4 / 30`）が列幅に収まらず**折り返して文字が重なっていた**ため、列幅を広げ `white-space: nowrap` を指定
  - 価格は桁が伸びる（30回目で134万、100回目は21桁）ので**短縮表示**にした
    （`17.7K` / `196.4K` / `1.3M` / `33.0B` / `5.7E20`）。1万未満はそのまま数字で出す
- **確認**: 構文チェック、フィボナッチ価格の検算に加え、ブラウザで配線を実測
  - ショップ5行が表示され、価格が **1P / 1P / 1P / 100P / 100P** で出る
  - **0Pでクリックしても購入されず、ゲームも始まらない**（`state` が `title` のまま）
  - 購入すると Lv+1・ポイント減・**次の価格が 1→2→3→5→8** と上がる
  - 強化が実際に反映される（間隔280・エアリアル5発・バリア有効）
  - 表示300mで終了すると **3P獲得**しリザルトに表示される
  - `console error 0 / page error 0`、DIVEボタンは0個

---

## v2.990.0 — 2026-09-19

### tweak: 地面の反発係数を 0.8 → 0.5 に

- 通常の着地の縦方向の跳ね返り `BOUNCE` を **0.8 → 0.5** にした
  - 跳ね上がる高さは前回比で **64% → 25%** になる（高さは速度の2乗に比例するため）
  - 落下100m/sで着地した場合、跳ねる回数は **約15回 → 約5回**、1回目の跳ね上がりは **72.6m → 28.3m**
- **影響範囲**
  - 変更したのは**通常の着地のみ**。**ダイブの叩きつけ（SLAM）は別式**（`p.vy = vd * .5`）なので影響しない
  - **横方向は変更していない**（`p.vx = p.vx * .88 + vd * .3`）。落下エネルギーの30%を前へ変換する挙動はそのまま
  - `bounceBoost`（ゴスアゲルのアゲルスペシャル）の上乗せ（横1.5倍・縦1.15倍×5回）もそのまま
- 跳ねが早く収まるので、着地後に転がって停止するまでが短くなり、1プレイの飛距離も縮む見込み
- **確認**: 構文チェックと跳ね返り回数・高さの検算のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
- 背景の空に、**控えめな星のレイヤー**を追加した（`drawSkyStars()`）
  - **80個の小さな点**（1〜2px）。位置は固定パターンから作り、毎フレーム生成はしない
  - **透明度 0.18〜0.40 でゆっくりまたたく**（`sin(t * 1.5 + 位相)`）。主張しない明るさに抑えている
  - **カメラの6%だけ動かして遠景として流す**（縦は5%）。飛ぶと星もゆっくり流れる
  - 描画順は **空 → 星 → 遠景・近景** なので、**地上付近は景色に隠れて空の部分だけに見える**
- **流れ星**を追加（`skyShoot`）
  - **7〜17秒に1回**、0.7秒だけ。線の始点を透明・終点を白（最大 α0.45）にしたグラデーションで、
    尾を引いて消える細い線
- `bg_sky.webp` を差し替えても星は上に描かれるので、画像を変えても効果は残る
- **確認**: 構文チェックと描画順の確認のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.988.0 — 2026-09-19

### tweak: 重力を1.5倍に（滞空時間の短縮）

- 滞空時間が長すぎたため、**重力のみ**を1.5倍にした（`GRAV_X = 1.5`、`G = 9.8 × 3 × GRAV_X`）
  - **29.4 → 44.1 m/s²**
  - 発射速度・エアリアル・ダイブの力は**変更していない**
- 効果（45度・全力発射、空気抵抗を除いた計算値）

  | | 旧 | 新 |
  |---|---|---|
  | 発射のみの滞空 | 10.4秒 | **6.9秒** |
  | エアリアル3回込みの滞空 | 29.7秒 | **19.8秒** |
  | 最高到達点 | 397m | **264m** |
  | 射程（表示） | 159m | **106m** |

- 調査で分かったこと: **滞空の約2/3はエアリアル由来**（`p.vy = max(p.vy,0) + 63 × POW_X` で
  落下速度を打ち消してから95m/s を足すため、1回で6.4秒ぶん浮き直していた）。
  今回は指示により重力のみを変更したので、さらに詰める場合はエアリアルの縦方向が次の候補
- 落下が速くなるぶん地上のバンパーへ到達する頻度は上がるので、ヒット数は微増する見込み
- **確認**: 構文チェックと滞空・高度・射程の検算のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.987.0 — 2026-09-19

### tweak: バンパーの間隔を300ごとに

- `CHAR_SPACING` を **100 → 300**（内部座標。`DIST_SCALE = 0.1` なので**画面表示では30mごと**）
- **ボスの出現開始を 400 → 900 に引き上げた**
  - `BOSS_FROM = 400` は「間隔100のとき4体目以降」という意図だったが、間隔を3倍にすると
    **2体目（600地点）でボスが出得る**ようになり、序盤の回避不能な即死が増えてしまう
  - 間隔の3倍（900＝4体目以降）に合わせ直した
- **先読み生成の範囲を間隔に連動させた**
  - 従来は固定値（初期600・先読み `p.x + 700`）だったため、間隔300では**2体しか作られず
    NEXTパネルの5体が埋まらない**。`CHAR_SPACING * 6` に変更して常に先6体ぶんを用意する
- 配置: 300, 600, 900, 1200 …（表示では 30, 60, 90, 120m）。
  画面に入るのは横114（表示11.4m）なので、**同時に見えるのは最大1体**。先は NEXT パネルで把握する
- **確認**: 構文チェックと配置・遭遇数の計算のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.986.0 — 2026-09-19

### tweak: ダイブをもっと横方向へ飛ばすように

- ダイブの横方向は2か所で決まっているので、両方を強めた
  - **撃った瞬間の横加速**: `p.vx += 30 * POW_X` → **`55 * POW_X`**（45 m/s → **82.5 m/s**、+83%）
  - **叩きつけ着地での変換**: 落下速度を前方向へ変える係数を **0.55 → 0.8**
- あわせて**着地で上へ返す量を 0.75 → 0.5 に下げた**。同じ落下エネルギーを
  上ではなく前に向けたいので、横を強めるだけでなく縦を減らす必要がある
- 効果（`POW_X = 1.5` の場合）

  | ダイブ開始高度 | 落下速度 | 前方向 | 上方向 |
  |---|---|---|---|
  | 10m | 110 m/s | 60 → **88 m/s** | 82 → **55 m/s** |
  | 30m | 116 m/s | 64 → **93 m/s** | 87 → **58 m/s** |
  | 60m | 123 m/s | 68 → **99 m/s** | 93 → **62 m/s** |

- **確認**: 構文チェックと変換後の速度の検算のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.985.0 — 2026-09-19

### tweak: ダイブのチャージ回復を1/3に

- チャージの回復速度を **62 %/秒 → 20.7 %/秒**（`CHARGE_RATE = 62 / 3`）にした。溜まるのが早すぎたため
  - 満タンまで **約1.6秒 → 約4.8秒**
  - 滞空10.4秒（45度・全力発射）の場合、ダイブを撃てる回数は **約6回 → 約2回**
- 数値を直書きせず `CHARGE_RATE` として定数化したので、今後は1行で調整できる
- 「遊び方」のチャージの説明にも**満タンまでの目安（約4.8秒）**を明記した
- **確認**: 構文チェックと回復時間の検算のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.984.0 — 2026-09-19

### feat: 最初の激突だけ画面全体を覆う特大エフェクトに ／ 停止を0.8秒へ

- **発射時の激突（アゲル→ヒコ）の演出だけを別格にした**
  - **スペシャルと同じ画面バーン**（全画面の放射グラデーション＋46本の集中線）を重ねた
  - ヒットエフェクトの星を **`pw = 4.5`**（通常ヒットは1.2）に拡大。**最大半径720px**で、
    画面の対角半分（671px）を超えるため**画面全体を覆う**
  - 粒子を80＋36に増量、衝撃波を2重（3倍・2倍）、フラッシュ0.85・シェイク0.75
- **巨大サイズでも線が細く見えないよう、線幅をサイズ連動にした**（`drawHitFx()`）
  - 星の輪郭 3px → **8.1px**、放射線 4px → **10.8px**（`pw = 4.5` の場合）
  - **通常ヒット（`pw = 1.2`）は従来どおり3px/4pxのまま**変わらない
- **発射時の停止時間を 0.456秒 → 0.8秒 に**（`IMPACT_T = 0.8`）
  - `HITSTOP_X` を掛けない固定値にした（演出の尺として直接指定したいため）
  - 発射の流れ: 0.00秒 アゲル登場 → **0.55秒 激突＋0.8秒の完全停止** → 1.35秒 ふっとぶ
- **確認**: 構文チェックと、エフェクト半径が画面を覆うかの計算検算のみ。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### tweak: スペシャル受付の0.7秒をまるごとヒットストップにした

- **キャラへのヒット時のヒットストップを、スペシャルのクリック受付と同じ 0.7秒 に統一**
  - 従来は 通常ヒット0.456秒／ボス接触0.66秒 と、**受付0.7秒より先にヒットストップが終わって**いた
  - `hitstop()` に `abs` 引数を追加し、`HITSTOP_X` を掛けずに秒数そのままで止められるようにして `hitstop(SP_WIN, …, true)` で呼ぶ。
    **`SP_WIN` を変えれば受付とヒットストップが自動で一致する**
- **止まり方の曲線を「維持してから戻す」形に変更**（`timeScale()`）
  - 従来は `r²` で**最初から徐々に通常速度へ戻して**おり、受付の後半はほぼ通常速度に戻っていた
  - **0〜85%は深く止まったまま維持し、最後の15%だけ通常速度へ戻す**ようにした
    （0.7秒なら 0.595秒まで止まったまま、残り0.105秒で復帰）
  - この曲線は他のヒットストップ（発射時の激突・スペシャル・SLAM・エアリアル/ダイブ）にも効くので、
    全体的に「止まってから動き出す」手応えになる
- 現在の長さ一覧

  | 場面 | 長さ | その間の速度 |
  |---|---|---|
  | キャラへのヒット（通常） | **0.700秒** | 12% |
  | ボスアゲル接触 | **0.700秒** | 7% |
  | 発射時の激突 | 0.456秒 | 完全停止 |
  | スペシャル発動 | 0.540秒（大技0.720秒） | 10% |
  | ダイブの着地（SLAM） | 0.312秒 | 20% |
  | エアリアル／ダイブ | 0.264秒 | 15% |

- **確認**: 構文チェックと秒数の検算のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.982.0 — 2026-09-19

### feat: 発射後に自機を1.5倍へ縮小 ／ エアリアル・ダイブにヒットエフェクトとヒットストップ

- **自機の倍率を場面で変えた**
  - 発射前（待機・突っ込み・激突）は **2倍のまま**、**発射後（飛行中・ダウン）は1.5倍**に縮む（`PLAYER_FLY_SCALE = 1.5`）
  - 画面上では **301px → 226px**
  - **当たり判定は衝突が起きる「飛行中の大きさ」基準**に変更（当たり半径 9.18m → **6.89m**）
- **アゲルのサイズ計算を整理**（不具合の予防）
  - これまで `hpx / PLAYER_SCALE` と**自機の描画サイズから逆算**していたが、自機の倍率が場面で2種類になったため
    そのままでは飛行中に誤った大きさになる。倍率を掛ける前の基準値を返す **`playerBase()`** を用意し、
    アゲル4か所すべてをそこから計算するようにした（自機の倍率を今後変えても影響しない）
- **エアリアル・ダイブの停止位置を自機側へ寄せた**
  - 横 `-1.0` → **`-0.75`**、縦 `±0.45` → **`±0.35`**（自機の高さ基準）
  - 実寸では 停止位置 横-170px・縦±79px（出現時は 横-260px・縦±136px から寄ってくる）
- **エアリアル・ダイブにヒットエフェクトとヒットストップを追加**
  - ヒットエフェクト（星＋放射線）をそれぞれの色で表示（エアリアル=ピンク／ダイブ=水色）
  - ヒットストップ **0.264秒**（速度15%まで低下）
  - ダイブには衝撃波とシェイクが無くエアリアルと手応えが揃っていなかったので、あわせて追加した
- **確認**: 構文チェック、旧方式（`hpx / PLAYER_SCALE`）の残りが無いこと、サイズ・位置・秒数の検算のみ。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### fix: アゲルが自機の拡大に引きずられて巨大化していた ／ エアリアル・ダイブの停止位置を調整

- **fix: v2.980.0 の `PLAYER_SCALE = 2` の副作用で、アゲルまで2倍になっていた**
  - アゲル（自転車・飛び蹴り・ダイブ）の大きさは自機の描画高 `hpx` から算出していたため、
    自機を2倍にしたときに**道連れで2倍**になっていた
  - 結果、**飛び蹴り／ダイブが693px・自転車が753px** と、**画面高600pxを超えてはみ出していた**。
    「エアリアルとダイブが自機に重なる」と見えていた主因はこれ
  - 算出式を `hpx / PLAYER_SCALE * …` に変え、**自機の倍率から切り離した**（飛び蹴り/ダイブ346px・自転車377px に復帰）
- **エアリアル・ダイブの停止位置を変更**
  - 従来は時間が経つとオフセットが0に収束し、**最終的に自機とぴったり重なる**動きだった
  - **左に寄った位置で止まる**ようにした（自機の高さの1.0倍ぶん左＝約301px）
  - さらに **エアリアルは「左下」、ダイブは「左上」** で止まるよう縦方向も終端を持たせた（±0.45倍＝約135px）
  - 寄ってくる動き自体は残している（横 +0.4倍・縦 +0.25倍ぶん外側から近づいて止まる）
  - 物理的にも、エアリアルは下から蹴り上げ、ダイブは上から叩き落とす動きに合う
- **確認**: 構文チェックとサイズ・位置の計算検算のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.980.0 — 2026-09-19

### fix: 自機が地面から浮いていた ／ feat: 自機の表示を2倍・飛ぶ力を弱めた

- **fix: 発射前の自機が地面から浮いていた**
  - 初期位置が `p.y = 1.4`（＝足元が1.4m宙に浮いた状態）だったため。`p.y = 0` にして**地面にぴったり接地**させた
  - あわせて角度・パワーの矢印の起点を、固定値 `SY(1.4)`（足元付近）から**体の中心 `p.y + PLAYER_H * 0.5`** に変更した
- **自機の表示サイズを2倍に**（`PLAYER_SCALE = 2`）
  - `hico_stand` だけを大きくすると**発射の瞬間に急に縮む**ので、待機・飛行・ダウンの**自機全体**に掛けている
  - 「当たり判定も見た目どおりに」の方針に合わせ、**当たり半径も同じ倍率**で追従（`PLAYER_H` から算出）
  - 自機の高さはカメラ倍率に依らず一定なので、`PLAYER_H` として1か所で持つようにした
- **飛ぶ力を弱めた**（`POW_X` 2 → **1.5**）。上空にいる時間が長すぎたため
  - 滞空時間は速度に比例するので約25%短縮、到達高度と飛距離は速度の2乗に比例するので約44%減
- **確認**: 構文チェックと、サイズ・滞空時間・射程の計算検算のみ。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: AGERU CRASH!! 発射前と飛行中で自機の画像を分けた

- 新しい素材キー **`hico_stand`**（発射前のヒコ）を `ASSETS` に追加した
- **使い分け**

  | 場面 | 画像 |
  |---|---|
  | タイトル／角度決め／パワー決め | **`hico_stand`** |
  | アゲルが突っ込んでくる間（`crash`） | **`hico_stand`** |
  | 激突の停止中（`impact`・震えている間） | **`hico_stand`** |
  | ふっ飛んでいる間（`fly`） | `hico_fly` |
  | 停止後のリザルト（`end`） | `hico_down` |

- **`hico_stand.webp` が無い間は `hico_fly` で代用する**（`standKey()`）。
  素材を用意するまでプレースホルダの箱が出てしまうのを避けるため、**今のままでも見た目は変わらない**
- 発射前は**傾けない**ようにした（従来は飛行用の画像を 0.2rad 傾けて表示していた）。
  立ち絵を斜めに描くと不自然なため
- 必要画像の一覧（`img-crash/README.md`）にも `hico_stand.webp` の行を追加
- **確認**: 構文チェックと参照箇所の確認のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.978.0 — 2026-09-19

### tweak: AGERU CRASH!! ヒットストップの長さを一律1.2倍に

- **`HITSTOP_X = 1.2` を追加**し、ヒットストップ系の長さをすべてこの1か所で調整できるようにした
  - 完全停止側は `IMPACT_T = 0.38 * HITSTOP_X`
  - スロー側は `hitstop()` の中で `dur * HITSTOP_X` として掛けるので、**呼び出し側3か所は変更不要**
- 結果の長さ

  | 場面 | 種類 | 旧 | 新 |
  |---|---|---|---|
  | 発射時の激突 | 完全停止 | 0.38秒 | **0.456秒** |
  | キャラへの通常ヒット | スロー(12%) | 0.38秒 | **0.456秒** |
  | ボスアゲル接触 | スロー(7%) | 0.55秒 | **0.66秒** |
  | スペシャル発動 | スロー(10%) | 0.45秒 | **0.54秒** |
  | スペシャル（大技） | スロー(10%) | 0.6秒 | **0.72秒** |
  | ダイブの叩きつけ（SLAM） | スロー(20%) | 0.26秒 | **0.312秒** |

- **確認**: 構文チェックと結果の秒数の検算のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.977.0 — 2026-09-19

### feat: AGERU CRASH!! 衝突時にヒットエフェクトを追加

- これまで衝突の演出は**粒子・衝撃波リング・フラッシュだけ**で、「当たった」ことが一目で分かりにくかった。
  マンガの衝突表現にあたる描画を追加した（`hitEffect()` / `drawHitFx()`）
- **内容**: 12本のギザギザの星（キャラごとの色）＋白い縁取り＋中心の白い芯＋外へ伸びる放射線8本。
  **0.42秒**かけて拡大しながらフェードし、わずかに回転する。芯は最初だけ強く光ってすぐ縮む
- **適用した衝突は3か所**
  - 発射時の激突（アゲル→ヒコ）— ヒットストップと同時に出る
  - **キャラへのヒット** — キャラの色で出る。ボスアゲルだけ**1.6倍**の大きさ
  - ダイブの叩きつけ着地（SLAM）— 水色で地面の位置に出る
- 大きさは**カメラ倍率に追従**するので、ズームアウトしても不自然に巨大化しない
- 描画順は**衝撃波リングの後・文字ポップの前**（`LAUNCH UP!!` などの文字が隠れないように）
- **確認**: 構文チェックと呼び出し箇所・描画順の確認のみ。
  **見た目の確認とプレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.976.0 — 2026-09-19

### feat: AGERU CRASH!! 激突の瞬間にヒットストップ（一瞬停止してヒコが震える）

- **発射の流れを3段階に変更**
  1. `crash`（0.55秒）アゲルが画面外から加速して突っ込んでくる
  2. **`impact`（0.38秒）激突。世界が完全に止まり、ヒコだけが震える** ← 今回追加
  3. `fly` ヒコがふっとぶ
- 新しい状態 `impact` と定数 `IMPACT_T = 0.38` を追加
  - この間は `physics()` を呼ばないので**世界は完全に静止**する（カメラも自機も動かない）
  - 停止時間は**実時間（`rdt`）で数える**ので、ヒット時のスロー演出（`timeScale()`）と干渉しない
  - 震えは `impactT` が減るにつれ**振幅が16px→4pxに収まって**いく（カメラ倍率に比例）。回転も微妙に揺らす
  - 停止中はクリックしても何も起きない（`press()` に `impact` の分岐を作っていないため）
- **激突の演出を `launch()` から `startImpact()` へ移動**
  - フラッシュ・粒子46・衝撃波・`CRASH!!`・効果音が、**発射時ではなく停止の開始と同時**に出るようになった
  - `launch()` 側にはふっ飛ぶ瞬間の追い打ち（粒子34・シェイク）だけ残している
- **確認**: 構文チェックと状態遷移の確認のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.975.0 — 2026-09-19

### feat: AGERU CRASH!! アゲルの表示サイズを2倍に／素材を900pxで作り直し

- **アゲル（自転車・飛び蹴り・ダイブ）の表示倍率を2倍**にした。`AGERU_SCALE = 2` を追加し、
  **1か所で調整できる**ようにしている
  - 突っ込み時の自転車: `hpx * 1.25` → `hpx * 1.25 * AGERU_SCALE`
  - 飛び蹴り／ダイブ: `hpx * 1.15` → `hpx * 1.15 * AGERU_SCALE`
  - 画面上の大きさ（拡大率10.5px/m時）: 自転車 **377px**・飛び蹴り/ダイブ **347px**（画面高600pxの63%/58%）。
    ズームアウト時（5.25px/m）はその半分
  - 当初3倍で作ったが、自転車が画面高の94%とほぼ画面いっぱいになったため2倍に変更した（この版は未公開のまま調整）
- **fix: 同じ `ageru_bike` の表示倍率が2か所でバラバラだった**
  - 突っ込み時は `hpx * 1.25`、激突後の置き去りは `hpx * 1.9` で、**同じアゲルが場面によって別の大きさ**になっていた。
    置き去り側を突っ込み時と同じ式に統一した
- **素材を元PNGから900pxで作り直した**（表示377pxに対して2.39倍の余裕）
  - 工程: `trim()` → 高さ**900px** → `flop()`（左右反転）→ webp(q88)
  - v2.974.0 では高さ300pxで生成していたが、拡大表示ではぼやけるため**この版で置き換えた**

    | ファイル | v2.974.0 | v2.975.0 |
    |---|---|---|
    | `ageru_bike.webp` | 240×300 / 36KB | **720×900 / 159KB** |
    | `ageru_dive.webp` | 278×300 / 35KB | **835×900 / 146KB** |
    | `ageru_kick.webp` | 228×300 / 25KB | **683×900 / 116KB** |

  - `img-crash/` の合計は 864KB → **1,138KB**
- **確認**: 構文チェックと表示サイズの計算検算のみ。**プレイテストは未実施**（ユーザー側で実施するとの指示のため）

---

## v2.974.0 — 2026-09-19

### chore: AGERU CRASH!! アゲルの絵を新素材に差し替え

- `cloudflare/pages/games/img-crash/sozai/` に置かれた新しいPNG3枚を webp 化し、ゲームが使う素材を差し替えた
- 変換は既存素材と同じ工程（**元PNGから `trim()` で透過の余白を除去 → 高さ300pxにリサイズ → webp(q88)**）

  | ファイル | 旧 | 新 | 元PNG |
  |---|---|---|---|
  | `ageru_bike.webp` | 331×300 / 68KB | **240×300 / 37KB** | 1504×1872 / 2,571KB |
  | `ageru_dive.webp` | 204×300 / 36KB | **278×300 / 36KB** | 1432×1568 / 1,757KB |
  | `ageru_kick.webp` | 397×300 / 29KB | **228×300 / 26KB** | 1504×1872 / 2,080KB |

  - 元PNGは 1500〜1900px あるが、ゲーム内でアゲルが描かれるのは150〜190px程度なので、
    そのまま使うと**必要の6倍の解像度を毎フレーム縮小する**ことになる。既存素材と同じ300pxに揃えた
  - 3枚とも**アルファチャンネルと実際の透過ピクセルが保持されている**ことを確認
- **左右反転**: 元の絵が左向きだったため、ゲーム用の3枚と `sozai/` のwebp3枚を `flop()` で反転した。
  ゲーム内ではアゲルが**画面左から右へ突っ込んでくる**ので、右向きに揃える必要がある。
  **元PNG3枚は原本として未変更**のまま残している（再変換の元にするため）
- **Windowsのファイルロック対策**: 最初 `toFile()` + `rename` で書き換えようとしたが、
  直前に `sharp(path).metadata()` で開いた先のファイルを掴んだままになり `EPERM` で失敗した。
  **読み書きともバッファ経由**にし、`sharp.cache(false)` を指定して中間ファイルを使わない方式に変更した
- `sozai/` の元PNG（計6.3MB）は**指示により退避していない**。`cloudflare/pages/` 配下は
  ディレクトリごと公開されるため、次回デプロイでは元PNGと変換後webpも一緒に公開される

---

## v2.979.0 — 2026-09-19

### fix: AGERU CRASH!! 減速系（フグ）・ブロック（ロボ）に当たったとき真下に落下しやすい問題を修正

- **原因**: フグ（減速）・ロボ（ブロック）は `relaunch(ang=null, mul, min)` で**角度そのまま・速度だけ変更**する。
  すでに真下寄り（横方向の速度成分がほぼ無い）に落下している状態でこれらに当たると、角度が保たれたまま
  速度だけ落ちるため、以降そのまま真下に落ち続けてしまっていた
- **`cloudflare/pages/games/crash.html`** `relaunch()`
  - 新定数 `MIN_HORIZ_RATIO = 0.5`：ang=null 経路で、速度に対する横方向の割合がこれを下回っていたら
    横方向の割合を最低ラインまで持ち上げる（縦方向はその分だけ縮めて速度の大きさは変えない）
  - 元の横方向がプラス／マイナスならその向きを維持。**完全に真下（横方向ゼロ）のときは前方（右）へ**流す
  - すでに横方向が十分にある斜めの軌道は今までどおり角度を変えない
- **動作確認**: `relaunch(TYPES.hugu.ang, ...)` / `relaunch(TYPES.robo.ang, ...)` に真下寄り・完全に真下・
  左寄り・十分に横向き・完全に横向きの軌道を通し、横方向の最低割合の確保・向きの維持・速度の減衰率
  （58%）・補正不要ケースで角度が変わらないことを確認（全13項目通過）

---

## v2.978.0 — 2026-09-18

### docs: AGERU CRASH!! 作成中であることをヘッダー・タイトル画面に表示

- **`cloudflare/pages/games/crash.html`**
  - ヘッダーバー：「hico1」ロゴの横に丸みのあるバッジ `.wip-badge` で「作成中」を常時表示（プレイ中もずっと見える）
  - タイトル画面：説明文の下に「⚠ このゲームはまだ作成中です。仕様や見た目は変更されることがあります」を同系色（`#ffb454`）で表示
- **動作確認**: Playwrightでバッジ・注意書きの表示テキストと視認性、JSエラーなしを確認（全5項目通過）

---

## v2.977.0 — 2026-09-18

### feat: AGERU CRASH!! カメラの上下バイアス（上昇中は自機を下寄り・下降中は自機を上寄りに）

- **`cloudflare/pages/games/crash.html`** `camera(dt)`
  - 従来は縦速度に関係なく常に自機を画面の垂直中央（frac=0.5）に捉えていたのを、
    **上昇中（`p.vy>0`）は自機を画面下寄り**（行き先の空を広く見せる）、
    **下降中（`p.vy<0`）は自機を画面上寄り**（着地点の地面を広く見せる）に変更
  - 狙う画面位置（frac）は、既存の画面外判定と同じ帯 **0.16（上寄り）〜0.66（下寄り）** の間で、
    縦速度に応じて連続的に滑らか変化。新定数 `CAM_BIAS_VY = 120`（m/s）で最大偏りに達する速度を調整可能。
    静止・低速時はその中間（frac≈0.41）
  - 追従イージング（`dt*4.5`）・既存の安全クランプ（16%〜66%を超えない）はそのまま
- **動作確認**: 実際の `camera(dt)` に上昇/下降/静止/極端な速度を通し、自機の画面位置が期待どおりに
  上寄り・下寄り・中間になること、速度が大きいほど偏りが強くなること、安全クランプを超えないことを確認
  （全6項目・スクリーンショットで見た目も確認）

---

## v2.976.0 — 2026-09-18

### fix: AGERU CRASH!! 近景・遠景の背景が地面から浮いて見える問題を修正

- **原因**: v2.974.0 で差し替えた `bg_near.webp` / `bg_far.webp` に、上下とも大きな透明の余白が入っていた
  （近景: 768px中 実画像は202pxのみ、上324px・下242pxが透明／遠景: 793px中295pxのみ、上368px・下130pxが透明）。
  `tileX()` は画像の下端＝地面として描画するため、実際の絵（街並み・山）が地面ラインより上の宙に浮いた状態だった
- **`cloudflare/pages/games/img-crash/bg_near.webp` / `bg_far.webp`**: 透明ピクセルの行を検出して
  上下の余白を切り抜き（幅はタイリングを崩さないよう変更なし）。近景 2048×768→2048×202、
  遠景 1983×793→1983×295
- **動作確認**: 切り抜き後の画像を実際にロードし、上下の透明余白がほぼ無い（2px以下）ことを確認。
  スクリーンショットで、地上付近では絵が画面下端まで途切れなく続くことを確認

---

## v2.975.0 — 2026-09-18

### fix: AGERU CRASH!! カメラが離れても近景・遠景が画面下端に張り付いたまま消えない問題を修正

- **原因**: `drawBg()` の `horizon = clamp(SY(0), 0, H)` が上限 `H`（画面下端）で頭打ちになっていたため、
  v2.973.0 のズームアウトでカメラが大きく離れ、`drawGround()` 側の地面帯は正しく画面外に消えているのに、
  `bg_near`（近景）・`bg_far`（遠景）だけが常に画面下端に張り付いたまま残っていた
  （バンパーが見えなくなるほど離れても、近景の草・ヤシの木がずっと表示され続ける）
- **`cloudflare/pages/games/crash.html`**: `horizon` の上限クランプを外し、`Math.max(0, SY(0))` に変更。
  地面と同じ生の値を使うことで、`tileX()` の描画矩形が自然に画面外へ出て消えるようにした
- **動作確認**: 実際のカメラ追従式 `camera(dt)` に現実的な高度（`p.y=1000`）を通し、
  `cam.s` がズームアウト下限（5.25px/m）に張り付くこと、地面・近景・遠景が**すべて同時に**画面外へ出ることを
  幾何計算とスクリーンショットで確認。中間高度・地上付近への復帰でも矛盾がないことを確認（全12項目通過）

---

## v2.974.0 — 2026-09-18

### chore: AGERU CRASH!! 近景・遠景の背景画像を新しい絵に差し替え

- `cloudflare/pages/games/img-crash/` に新しく置かれた `bg_far.png`（1983×793）・`bg_near.png`（2048×768）を
  既存フォーマットに合わせて WebP（quality 85）に変換し、`bg_far.webp` / `bg_near.webp` を置き換え。元のPNGは削除
- サイズは推奨値（1600×420 / 1600×300）と異なるが、描画は高さ基準で縦横比を保持するため問題なし（README記載どおり）
- 変換後も同じ幅×高さで書き出せていることを検証してから置き換え

---

## v2.973.0 — 2026-09-18

### feat: AGERU CRASH!! バンパーと自機が同時に映るようカメラを動的ズームアウトに

- **固定拡大をやめ、地面（バンパーが立っている高さ）と自機が同時に入るように画角を広げる**
  - 必要拡大率を `(GROUND_SY - H*0.18) / max(12, p.y + 6)` で毎フレーム求め、`CAM_S`（10.5px/m）と
    その 1/2 の間にクランプする。**広げるのは初期画角の2倍まで**（`CAM_ZOOM_MAX = 2`）
  - 高度 **約41mまで**は初期の拡大率（10.5px/m・横114m×縦57m）のまま
  - そこから徐々に広がり、**高度約88mで2倍の画角に到達**（5.25px/m・横229m×縦114m）。以降は固定
  - **それ以上の高度ではバンパーは画面外**になるが、v2.971.0 で入れた画面端のアイコンで位置が分かる
- ズームアウト中はキャラの表示も小さくなる（ベビーアゲルで178px→89px）。
  これは画角を広げる以上避けられないため、上限を2倍に抑えることで見失わない範囲に留めている
- **確認**: 構文チェックと、拡大率の計算値（開始高度・上限到達高度・各高度での表示サイズ）の検算のみ。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: AGERU CRASH!! エアリアル・ダイブをいつでも撃てるように

- **高度制限（地上9〜30m）を撤廃**。`aerial()` / `dive()` の高度チェックを削除し、
  判定に使っていた `inBand()` と定数 `BAND_LO` / `BAND_HI` も削除した
- **チャージを常時回復に変更**（従来は低空帯にいる間だけ回復）
  - ダイブの高度制限だけ外しても、**チャージが低空でしか溜まらないままでは高空で結局撃てない**ので、
    セットで変更しないと「いつでも撃てる」が成立しない
- **低空帯のガイド表示（点線の帯）を廃止**。示すべき制限が無くなったため
- HUDのエアリアル残数・チャージバーにあった「帯の外だと暗く表示」を廃止し、常に有効色で表示する
- 「遊び方」の該当記述も更新（「地上9〜30mでのみ発動」→「高度に関係なくいつでも撃てる」、
  チャージは「飛んでいる間つねに回復」）
- **変えていない点**: エアリアルは1プレイ**3回**まで、ダイブは**チャージ100%必要**（回数は無制限）。
  ここも無制限にすると延々と飛び続けられてしまうため、指示の「いつでも」は
  **高度による発動制限の撤廃**と解釈した
- **確認**: 構文チェックと、削除した `inBand` / `BAND_LO` / `BAND_HI` の参照残りが無いこと。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: AGERU CRASH!! 飛ぶ力2倍／距離表示を1/10に／画面外バンパーのアイコン表示／画面下端を地面に

- **飛ぶ力を2倍に**（`POW_X = 2`）。キャラが大きくなって飛ばなくなったため
  - 発射速度（**最大288m/s＝1,037km/h**）・各キャラの最低打ち出し速度・エアリアル（上+126m/s）・ダイブ（下-144m/s）・復活に適用
  - **速度上限も2倍**（`MAX_SPD` 330→**660m/s＝2,376km/h**）。上限を上げないと倍にした意味が無くなるため
  - **弾む力**は着地の跳ね返りを `0.5 → 0.8`（`BOUNCE`）、着地時の前方変換も `0.2 → 0.3`。
    **1.0（文字どおり2倍）はエネルギーが減らず永久に弾む**ので避けた
- **距離表示を1/10に**（`DIST_SCALE = 0.1`、ワールド100m＝表示10m）
  - 表示を `fmt()` 1か所に集約したので、左上の飛距離・BEST・リザルトがまとめて切り替わる
  - 地面の距離目盛りと NEXT パネルの距離も同じ単位に統一
  - アゲルスペシャルの「**表示距離の下2桁が00**」条件も表示単位に合わせ、ワールド1000mの倍数（±15m）に変更
- **画面外のバンパーをアイコン表示**（`drawOffscreen()`）
  - 高く飛ぶと地上のキャラが画面下に消えて何も見えないため、**スマブラの画面外表示のように画面端にアイコン**を出す
  - 最大3体。**ボスは赤枠**、**スペシャル条件成立は黄色枠**、実際の位置を指す三角と残り距離つき
- **画面の下端＝地面に変更**（`GROUND_SY` 520 → `H`）。バンパーが画面下部にぴったり立つ
  - **自機の描画基準を「`p.y` ＝足元」に変更**。中心で描いていたため、地面が下端になると**接地時に自機が半分画面外**に出ていた
  - **下部HUD（AERIAL / CHARGE）を画面上部の左へ移動**し、下部の黒帯を廃止。下端に立つバンパーと重なるため
  - 地面線と距離目盛りを**地面線から上向き**に描くよう変更（従来は下向きで、下端が地面だと画面外になる）
  - 突っ込むアゲル・置き去りのアゲルの接地位置も `.46 → .5` に揃えた
  - **fix: カメラが上がると距離目盛りと低空帯のガイドが消えていた**。
    `drawGround()` 冒頭の `if (gy > H) return;` が、地面＝画面下端にしたことで
    **カメラが少しでも上がった瞬間に成立**するようになり、同じ関数内にある目盛りと
    エアリアル可能帯（9〜30m）のガイドまで巻き添えで消えていた。早期returnを廃止し、
    地面テクスチャだけを出し分けるようにした
- **確認**: 構文チェック、廃止・変更した定数の参照整合、速度と距離の計算値の検算のみ。
  **プレイテストは未実施**（ユーザー側で実施するとの指示のため）
...
### feat: AGERU CRASH!! 自機をバンパーと同じ大きさに／当たり判定を見た目どおりに

- **自機（ヒコ）の描画をバンパーと同じスケールに**した
  - 旧: `150 * cam.s / 46`（cam.s=10.5 で 34px）→ 新: `ASSETS.hico_fly.h * cam.s / 46 * CHAR_SCALE`（**151px**）
  - バンパー（ベビーアゲル 178px）とほぼ同じ大きさになる
- **当たり判定を「見た目そのまま」に変更**（v2.967.0 では見た目だけ3倍で判定は据え置きだった）
  - 描画が `h * cam.s / 46 * CHAR_SCALE` なので、**ワールド上の大きさ `h * CHAR_SCALE / 46`（m）はカメラ倍率に依らず一定**になる。これをそのまま判定に使う
  - キャラごとの絵の高さで判定する: ヒコ 14.3m ／ ベビーアゲル 17.0m ／ ボスアゲル 23.5m ／ カニ 10.4m
  - 幅は**読み込んだ画像の実アスペクト比**から出す（描画と同じ計算）。円ではなく**箱**で判定する
  - これに**自機の半径 `PLAYER_R = 4.59m`**（自機の見た目の高さの32%）を足した範囲で当たる
  - 旧定数 `HIT_R = 3.0` / `CH_MID = 2.5`（地上2.5m中心・半径3mの円）は**廃止**。
    判定が届く高度はベビーアゲルで **5.5m → 約21.6m**（絵の高さ17.0m＋自機4.59m）に広がる
  - 早期打ち切りの上限も `MAX_CH_H`（最大キャラ＝ボスの23.5m）から自動計算するようにした
- 自機が大きくなったのに合わせて、**突っ込んでくるアゲルの倍率を 1.9 → 1.25** に下げた（そのままだと285pxとバンパーより大幅に大きくなるため）。
  エアリアル／ダイブの画像サイズと表示位置も自機サイズ基準（`hpx * 1.15` / `hpx * .22` / `hpx * .3`）に変更
- **確認**: 構文チェックと、廃止した定数の参照残りが無いことを確認。
  サイズの計算値（ワールド換算m・画面px・`PLAYER_R` / `MAX_CH_H`）を検算した。
  **プレイテストは実施していない**（ユーザー側で実施するとの指示のため）

---

## v2.969.0 — 2026-09-18

### feat: AGERU CRASH!! ヒット時に効果を大きな文字で表示

- キャラに当たったとき、**そのキャラの効果を大きい文字（36px 太字）で表示**するようにした
  - 以前はキャラ名（「ベビーアゲル」など・17px）を小さく出すだけで、**何が起きたのか分からなかった**
  - `TYPES` に `fx` を追加して効果名を持たせた:
    `LAUNCH UP!!`（ベビーアゲル/45度打ち上げ）・`HIGH JUMP!!`（ゴスアゲル/高角度）・`BOOST!!`（デンパアゲル/低角度加速）・
    `HOP!!`（キショネコ/小さく打ち上げ）・`ANGLE UP!!`（カニ/角度変更）・`SLOW DOWN…`（フグ/減速）・
    `BLOCK!!`（ヨクネロボ）・`DANGER!!`（ボスアゲル）
  - 色はキャラごとの色をそのまま使う。ブロックも小さい「BLOCK」から大きい `BLOCK!!` に変更
- **出た瞬間だけ1.45倍に弾ませる**演出を追加（`drawFx()` の `grow`）。縁取りも太くして背景に負けないようにした
- **fix: 大きい文字どうしが重なって読めなくなる問題**（この版で36pxに拡大した副作用）
  - 同時に出たポップの段差が `recent * 24px` 固定だったため、`HIGH JUMP!!` と `SPECIAL!!` が重なって
    「HSPECIAL!!!!」のように潰れていた（スクリーンショットで発覚）
  - **各ポップの大きさぶんを積む**方式に変更（大きい文字46px・小さい文字26px）。
    大小が混在しても正しく段が付く
- **動作確認**: 実プレイで `HIGH JUMP!!` `SPECIAL!!` `CRASH!!` `SLAM!!` が出ることを確認、全8ラベルの定義と `page error 0` を実測。
  重なり修正後は、大文字が同時に出た4サンプルすべてで**段差72px以上**（36px文字に対して十分）を確認

---

## v2.968.0 — 2026-09-18

### feat: AGERU CRASH!! 発射演出をやり直し（照準中はアゲルを出さず、左から突っ込んでくる）／fix: 発射直後に飛び蹴り画像が出ていた

- **照準・パワー決定の間は `ageru_bike` を表示しない**。以前は待機中からずっと自転車が画面に置かれていた
- **発射するとアゲルが画面左から加速しながら突っ込んでくる**（`drawAgeruRush()`）
  - 演出の長さは `CRASH_T = 0.55秒`。位置の補間は `pr * (0.45 + 0.55 * pr)`。最初 `pr²` にしたところ**前半ほとんど動かず、突っ込みが一瞬しか見えなかった**ため（計測: crashT=0.37 の時点でまだ画面左端に見切れ、0.08 でようやく到達率73%）、**最初から速く走り、最後にもう一段加速する**曲線に変更した。修正後はアゲルの画面X座標が -78 → 76 → 223 → 275 と画面を横切る（ヒコは X=360）
  - 勢いを出すため、後方に伸びるスピード線を8本重ねている（進むほど長く・濃く）
- **激突後は激突地点に置き去りになる**（`crashX` に激突時の `p.x` を記録し、そこに描画）。カメラはヒコを追うので自然に後ろへ流れて消える。なお**約287km/hで飛ぶため発射0.6秒後には画面外**（実測X=-235）に去る。表示時間を延ばしても画面外なので意味がなく、これは挙動として正しい
- **fix: 発射直後の0.42秒間、`ageru_kick`（飛び蹴り）が誤って描かれていた**
  - `ageruFx.kind === 'bike'` の分岐が無く「dive以外はkick」という判定だったため、自転車のはずが飛び蹴りの絵になっていた
  - `ageruFx` に `dur`（持続時間）を持たせ、`bike` / `kick` / `dive` を種類ごとに正しく描き分けるようにした
- **動作確認**: 照準中（アゲル非表示）→突っ込み中（`crashT` 0.37 と 0.08 の2点）→発射後（置き去り）の各段階をスクリーンショットで確認。
  `ageruFx.kind='bike'`・`crashX` の設定、通しプレイの完走、`console error 0 / page error 0` を実測

---

## v2.967.0 — 2026-09-18

### feat: AGERU CRASH!! ぶつかるキャラの表示サイズを3倍に

- `CHAR_SCALE = 3` を追加し、地上キャラの**描画サイズのみ**を3倍にした（`drawChars()`）
  - ベビーアゲルで 59px → **178px**（ワールド換算 5.6m → **17m**）。足元が地面に接する描き方（`SY(c.y) - hpx/2`）は維持
- **当たり判定は変更していない**（`HIT_R = 3.0` / `CH_MID = 2.5` のまま）。判定が届く高度は **5.5m** のまま
  - 実装の途中で判定も3倍（`HIT_R = 9` / `CH_MID = 7.5`）にして計測したが、**ヒット数は 2.5回 → 2.3回 とほぼ変わらなかった**。
    高く飛んでいる時間が長く、判定を広げても低空にいなければ当たらないため。指示により**見た目のみの拡大**に戻した
  - このため、以前から保留中の「ヒット回数が少ない」件は**判定の拡大では解決しない**ことが判明した。
    有効なのは配置間隔を詰める（`CHAR_SPACING`）か、1か所に複数体置く方向
- **動作確認**: 定数（`CHAR_SCALE=3` / `HIT_R=3` / `CH_MID=2.5`）、描画178px＝17m、判定到達高度5.5m を実測。
  通しプレイは 5,747m / 4,568m（ヒット1〜2回）で3倍化前と同等、`page error 0`

---

## v2.966.0 — 2026-09-18

### feat: AGERU CRASH!! キャラを100m間隔に／右上に先読みパネル／カメラを固定拡大に

- **ぶつかるキャラを100mちょうどの地点に1体ずつ配置**（`CHAR_SPACING = 100`）
  - 以前は8〜22mのランダム間隔だった。`nextX` を100ずつ進めるだけの素直な実装に変更
  - スペシャルの「並び」条件（ベビー↔ゴス、デンパ連続）の相方は**次の100m地点**に置かれる。`pendingType` で予約する方式
  - 生成率の実測: **1,200体中302組（25.2%）**がスペシャル条件の並びになる（条件が成立しない不具合が無いことを確認）
- **カメラを固定拡大に**（`CAM_S = 10.5 px/m`）
  - 以前は高度に応じて 1.6〜11 でズームアウトしており、**高く飛ぶとキャラが豆粒になって見えなかった**
  - 固定にしたぶん画面に入るのは横114m・縦57m程度になり、100m間隔のキャラはほぼ1体ずつしか見えない。**見えない先は NEXT パネルで補う**
- **fix: 自機が画面外に出る問題**
  - 固定拡大＋速度3倍にした結果、上昇・落下にカメラの縦追従（lerp）が間に合わず、**自機が枠外に消えていた**（高度111mのスクリーンショットで発覚）
  - 追従後に `cam.y` をクランプし、**自機の画面Yが上16%〜下66%の帯に必ず収まる**ようにした
  - 実測: 3回の通しプレイ **7,052サンプルで画面外0回**（画面Y 96〜520に常時収束、最高高度1,105mでも維持）
- **右上に NEXT パネルを追加**（`drawUpcoming()`）
  - これから来るキャラを**5体ぶん、アイコン＋残り距離**で表示
  - **ボスは赤枠**で警告、**スペシャル条件が成立しているキャラは黄色い輪**で予告。100m間隔＋固定カメラでも先の並びを読んで狙える
- **ボスの出現開始を 150m → 400m に**。100m間隔になって序盤のキャラ数が減り、最初の数体でボスに当たる事故の比重が上がったため
- **バランスへの影響（注意）**: この変更で**ヒット回数が5.4回 → 1〜2.5回に減少**した。飛距離は約4,000〜6,000mで維持されているが、**伸びの主因がキャラへのヒットからバウンドに移っている**
- **動作確認**: 配置は先頭から x=100,200,300… で100の倍数ちょうど（ずれ0体）、400m未満のボス0体、カメラ `cam.s` は常に10.5、`page error 0`

---

## v2.965.0 — 2026-09-18

### feat: AGERU CRASH!! を本家寄りに大改修（速度3倍・時速表示・地上限定・ヒット演出強化・スペシャル条件変更・効果音対応）

- **飛行速度を3倍に**（`SPD_X = 3`）
  - 発射速度・各キャラの最低打ち出し速度・エアリアル／ダイブの加速量・速度上限（`MAX_SPD` 110→**330 m/s = 1188km/h**）をすべて3倍
  - **重力も3倍**（`G = 9.8*3`）にして弾道の形を保った。速度だけ3倍にすると放物線が9倍に伸びて、かえって間延びするため
  - 空気抵抗の係数（`sp*sp/810000`）と、着地判定・転がり摩擦・停止しきい値も3倍スケールに合わせて調整
  - エアリアル／ダイブの可能帯も3倍（地上3〜10m → **9〜30m**）
- **時速（km/h）を表示**（画面上部中央）。速度に応じて水色→黄→赤に変わるバー付き。リザルトに**最高速度**も追加（`topKmh`）
- **速いほど風のエフェクト**
  - 進行方向の後ろに流れる風の筋（`winds`）を、速度が上がるほど濃く・長く発生（45m/s から出はじめ、245m/s で最大）
  - さらに **120m/s 超で画面端の速度線と周辺減光**（`drawSpeedLines()`）を重ねてトンネル感を出す
- **ぶつかるキャラを地上限定に**
  - 空中配置をやめ、全キャラが地面に立つ（`ensureChars()` は `y:0` 固定、描画も足元が地面に接するよう `-hpx/2`）
  - 当たり判定はキャラの中心を地上 `CH_MID = 2.5m`、半径 `HIT_R = 3.0m`（見た目の立ち絵とほぼ一致）
- **ヒット演出を強化＋スローモーション**
  - 当たった瞬間に**ヒットストップ**（`hitstop()` / `timeScale()`）。通常0.38秒・ボス0.55秒だけ時間が1/8前後に落ちて、演出が見えるようになる
  - 粒子を52+22個に増量、**衝撃波リング**（`shockwave()`）、画面フラッシュ、シェイク強化
  - **当たったキャラが回転しながら吹っ飛ぶ**（`knockAway()`）
  - スペシャル受付（0.7秒）は**実時間で数える**ので、スロー中は体感の猶予が伸びて押しやすい
- **スペシャル発動時に画面が「バーン」**（`bang()` / `drawBang()`）
  - 着弾点から広がる放射グラデーション＋**46本の集中線**を**背景として**描画。加えて二重の衝撃波・大量の粒子・強フラッシュ
- **スペシャルの発動条件を本家に合わせて変更**（今までは「当たれば毎回0.7秒クリックで発動」だった）
  - `ベビーアゲル`: **次に並んでいるキャラがゴスアゲル**のとき（本家の「霧→美希の並びで霧に触れる」）
  - `ゴスアゲル`: **次に並んでいるキャラがベビーアゲル**のとき（同「美希→霧」）
  - `デンパアゲル`: **直前に当てたのもデンパアゲル**のとき（同「冬子→冬子、間に他を挟まない」）
  - `ボスアゲル`: **他のキャラに当てた後、バウンドせずに**当てたとき（同「他女性キャラ接触後ノーバウンドで見里」）。**条件を満たさないボス接触は即終了**
  - 条件を満たすキャラは**黄色い輪で光って予告**されるので、狙って取れる
  - 上記のためにキャラ配置は「ベビー→ゴス」「ゴス→ベビー」「デンパ→デンパ」の並びを一定確率で意図的に生成する
- **操作を本家と同じ「押しっぱなし」方式に変更**
  - 旧: クリックで角度 → もう一度クリックでパワー
  - 新: **押した瞬間に角度確定 → 押しっぱなしでパワーが動く → 離した瞬間に発射**（`release()`）。解説記事の「パワーゲージが最高値に近い時にボタンを離す」に合わせた
  - マウス・タッチ・キーボード（Space等）すべて対応。押したままカーソルが外に出た場合も発射される
- **効果音に対応**（`./sfx-crash/<キー>.mp3`）
  - 18種のイベントに対応した `SFX_LIST` を追加。単発は `se()`、風・転がりは音量が連動する `seLoop()`
  - **ファイルが無くても黙って鳴らないだけ**で、エラーにならない（実測でコンソールエラー0）
  - ヘッダーに **♪ SOUND** のON/OFFボタン（localStorage に保存）
  - 必要な音の一覧は **`cloudflare/pages/games/sfx-crash/README.md`**
- **バーンの白飛びを調整**。放射グラデーションの減衰を早め（中心0.18で白45%→0.5で10%）、全体アルファを0.7に、画面フラッシュにも上限0.4を設定。最初の実装は画面全体が真っ白になり、**「スローにして演出を見せる」目的と逆に演出も背景も見えなくなっていた**
- **序盤はボスアゲルを出さないようにした**（`BOSS_FROM = 150`）。キャラを地上限定にした結果、発射直後にボスに当たる事故が起きるが、そのときは他キャラに未接触でスペシャル条件（他キャラ接触後ノーバウンド）を満たしようがなく、**回避不能な即死**になっていた（実際に33mで終了する例を確認）。150m未満のボスはキショネコに置き換える
- **下部HUDに下地を追加**。地面に絵が入って AERIAL/CHARGE が読めなくなっていたため
- **動作確認**: 自動プレイ5回で **平均4,493m（従来1,283m）・ヒット5.4回・バウンド2.8回・最高1,188km/h・スペシャル1.8回/回**、スロー発生率3〜6%、**page error 0**。押しっぱなし操作（押下で角度確定→離して発射）と、空中キャラ0体（地上限定）も実測で確認。ボス抑制は配置40パターン（生成2,756体・ボス105体）で**150m未満のボス0体・最も手前152m**を確認し、調整後の通しプレイも5,076m／3,701mでエラー0

---

## v2.964.0 — 2026-09-18

### feat: AGERU CRASH!! に背景・ロゴを追加 ／ fix: タイトルロゴが出ない・モーダルのボタンが本文に重なる

- **背景4点を配置**（`cloudflare/pages/games/img-crash/`）: `bg_sky` 1200x600 ／ `bg_far` 1600x420 ／ `bg_near` 1600x300 ／ `ground` 600x160。いずれも仕様どおりの名前・サイズ
- **背景に焼き込まれていた寸法ラベルを除去**
  - 各画像の左上に `bg_sky.webp 1200×600` `1600×420` `) 1600×300` `600×160` が印字されており、**そのままゲーム画面（空や地面）に表示されていた**
  - 同じ画像の**同じy帯の別位置からきれいな帯をコピーして上書き**する方式で除去（単純な塗り潰しと違い、グラデーションや稜線が連続するので継ぎ目が出ない）
  - 元ファイルは `assets/crash/*_src.webp` へ退避。**`cloudflare/pages/` 配下は丸ごと公開される**ので、作業用の原本をそこに置かない
- **ロゴを反映**（`logo.webp` 905x300 / 104KB）
  - `img-crash/` に生PNG（2172x724 / 1.8MB）が置かれていたが、ゲームが読むのは `logo.webp` なので**反映されていなかった**。`trim()` → 高さ300で webp 化し、原本は `assets/crash/logo_src.png` へ退避
- **fix: タイトルロゴが表示されないバグ**（`games/crash.html`）
  - `<img id="title-logo">` は **HTML のパース時点で読み込みが始まる**ため、その後に `onload` を代入する実装では、ローカルなど読み込みが速い環境で**イベントが既に発火済みで二度と来ず**、`hidden` が外れなかった
  - `lg.complete` を見て、読み終わっていれば `naturalWidth` の有無で表示／`remove()` を自分で判定するよう修正
- **fix: ゲームモーダルの ✖ / ↗ がゲーム画面の文字に重なっていた**（`index.html`）
  - ボタンをパネルへ絶対配置していたため、iframe 内のヘッダー（ヒント文・「遊び方」）と重なっていた
  - パネル内に **`#game-bar`（高さ46px・左にゲーム名、右にボタン）を新設**し、iframe をその下に配置。ボタンは `.game-btn` としてバー内に収め、絶対配置をやめた。`#game-loading` の inset も 46px 下げた
- **動作確認**: ローカルで実測し、**✖ボタン下端 66px ≤ iframe 上端 73px**（＝重なりゼロ）、`ASSETS` **17/17 読み込み成功**、タイトルロゴ表示 true・テキスト非表示 true、モーダル内でプレイ開始までコンソールエラー0

---

## v2.963.0 — 2026-09-18

### feat: AGERU CRASH!! にキャラ画像13点を配置（プレースホルダを実素材に差し替え）

- **`cloudflare/pages/games/img-crash/`** に `Pictures/AGERUPHOTO/sozai`（593点）から選んで書き出し
  - 選定方法: sharp で全点を走査し、**実際に透過ピクセルを持つ切り抜き素材135点**に絞り込み（`hasAlpha` だけでは不透明PNGが混ざるので `stats().isOpaque` で判定）、コンタクトシートを作って役割ごとに選択
  - 書き出しは **透過の余白を `trim()` してから高さ基準でリサイズ → webp(q88)**。余白を残すとキャラが当たり判定より小さく見えるため
- **割り当て**（あとで差し替えるときの対応表）

  | キー | 元ファイル | 出力 |
  |---|---|---|
  | `ageru_bike` | `strike.png` | 331x300 |
  | `ageru_kick` | `ageruingrid.png` | 397x300 |
  | `ageru_dive` | `pogo.png` | 204x300 |
  | `hico_fly` | `7457hghgf.png` | 135x220 |
  | `hico_down` | `fefer246t34.png` | 218x160 |
  | `c_baby` | `babyageru.png` | 270x260 |
  | `c_goth` | `gothicageru.png` | 198x280 |
  | `c_denpa` | `denpaageru_transparent.png` | 178x280 |
  | `c_boss` | `00102-125482963.png` | 268x360 |
  | `c_robo` | `gatitoukaw_diferuw.png` | 155x260 |
  | `c_kani` | `3.png` | 229x160 |
  | `c_hugu` | `cloudflare/pages/img-dino/hugu.webp`（sozai にフグが無いため既存素材を流用） | 204x200 |
  | `c_neko` | `kisyoneko.neko.png` | 197x200 |

- **背景（`bg_sky` / `bg_far` / `bg_near` / `ground`）とロゴ（`logo`）は未配置のまま**。指示により対象外で、コード側のフォールバック（高度で色が変わる自動グラデーション＋星空）がそのまま使われる
- **動作確認**: ローカルで実際にゲームを起動し、`ASSETS` 17キーのうち**13点が `IMG` に載り（＝プレースホルダに落ちていない）、残り4点が背景のみ**であることと、発射→飛行までエラー0で動くことを確認

---

## v2.962.0 — 2026-09-18

### feat: 公開サイトに GAME タブ／ゲーム一覧モーダルを追加、第1弾「AGERU CRASH!!」を実装

- **`cloudflare/pages/games/crash.html`（新規）**: NANACA†CRASH!! 風の飛距離アタック **AGERU CRASH!!**
  - アゲルが自転車でヒコをはね飛ばし、空中のキャラに当てて飛距離を伸ばす canvas ゲーム（論理解像度 1200×600・1/120秒の固定ステップ更新）
  - **発射**: クリックで角度決定 → もう一度クリックでパワー決定（`state`: title / angle / power / crash / fly / chance / end）
  - **エアリアル**（クリック・SPACE／上方向・**1プレイ3回**）と**ダイブ**（右クリック・↓・Shift／下方向・**チャージ100%必須・無制限**）。どちらも**地上3〜10m（`BAND_LO`〜`BAND_HI`）でのみ発動**し、その帯にいる間だけチャージが回復する
  - ダイブの叩きつけは着地時に前方向の速度へ変換される（`SLAM`／`physics()` 内の着地処理）
  - **キャラ8種**（`TYPES`）: ベビーアゲル(45°打ち上げ)／ゴスアゲル(高角度)／デンパアゲル(低角度・加速)／キショネコ(小打ち上げ)／カニ(角度変更)／フグ(減速)／ヨクネロボ(ブロック)／ボスアゲル(即終了)
  - **スペシャル**: 接触から**0.7秒以内にクリック**で強化打ち上げ（`spW` / `trySpecial()`）。ボスアゲルは接触した瞬間に時間が止まり、0.7秒以内のクリックで**大逆転の超打ち上げ**（`bossChance()`）
  - **アゲルスペシャル**（`ageruSpecial()`）: ヨクネロボによる**ブロック状態** ×**飛距離が100の倍数の地点** ×アゲル系キャラ接触で発動。100人ぶん打ち上げ／5回バウンド加速／停止時に1度だけ復活／以後3回の加速2倍
  - **キャラの出現高度はヒコの弾道を先読みして決める**（`ensureChars()`）。高さを完全ランダムにしていたときは高く飛ぶと誰にも当たらず（自動プレイで飛距離1153m・ヒット0回）、「キャラに当てて伸ばす」というゲームが成立していなかった
  - **速度上限 `MAX_SPD = 110` m/s と、速度の2乗に比例する空気抵抗**を追加。打ち上げが現在速度の掛け算なので、上限が無いと発散する（入れる前は自動プレイで平均13万m・最高高度15万mまで飛んでいた）
  - 同じ文言のポップアップは1秒に1回までに制限（`popAt`）。「地上3〜10mでのみ」「CHARGE不足」の連打で画面が文字で埋まっていたため
  - ズームアウト時もヒコ・キャラが見失われないよう描画サイズに下限（自機30px／キャラ24px）。HUD の BEST が飛距離表示と重なっていたのも修正
  - ベストスコアは `localStorage['crash-best']`。高度が上がるとカメラが自動でズームアウトする（`camera()`）
  - **画像が1枚も無くてもプレースホルダを描いて遊べる**。`ASSETS` の定義がそのまま必要画像の一覧（`./img-crash/<キー>.webp`）
- **`cloudflare/pages/games/img-crash/README.md`（新規）**: 必要画像の一覧（キー名・推奨サイズ・内容）
- **`cloudflare/pages/index.html`**
  - ギャラリーのタブに **GAME** を追加（INFO / PHOTO INDEX / GAME）。タブ切り替えを `showTab()` に一般化し、対象を `TAB_IDS` で管理するようにした（タブ追加時に分岐を増やさなくてよい）
  - ヘッダー nav に **Game**（`#nav-game`）を追加。GAME タブを開いてその位置までスクロールする
  - `#tab-game` にゲーム一覧グリッド（`.game-grid` / `.game-card`）。**`GAMES` 配列に1行足すだけでゲームが増やせる**構成
  - カードを押すと `#game-panel`（iframe モーダル）で起動。右上の ↗ で単独ページを別タブ表示、閉じたときは iframe の `src` を捨ててゲームのループ・音を止める
  - サムネ画像が無いカードは**グラデーション＋頭文字**にフォールバック（`.game-thumb.noimg`）
  - 既存の **DINO** も一覧に掲載（`/dino`）
- **動作確認**: playwright で、ゲーム単体ページ（角度→パワー→飛行→エアリアル／ダイブ→停止→リザルト）と、トップの GAME タブ→カード→モーダル起動→閉じる（閉じたら `src` が落ちる）まで、コンソールエラーなしで確認。
  バランスは自動プレイ5回×2条件で計測し、**何も押さない場合 平均382m／エアリアル・ダイブ・スペシャルを全部使う場合 平均1,283m**（最高到達点628m・ヒット3回）に収まることを確認
...
- **背景**: 「キャラ作成」（`createCharImage()`）は `user.charImageData`（一時画像、その場かぎり）を設定し、ステージのアバターは `applyAvatarStyle()` でこれを最優先表示する（`user.charImageData || charaSrc(imgFile)`）
- **fix: 「ランダムキャラ」で見た目が切り替わらない**
  - 「キャラN」／エイリアス経由のキャラ変更は `charImageData` を削除してから再描画していたが、**「ランダムキャラ」だけ削除し忘れており**、内部の `charImage` は変わっても画面には古いキャラ作成画像が残り続けていた
  - **`public/js/app-06-comment-handler.js`**: 「ランダムキャラ」コマンドでも `charImage` 設定時に `delete user.charImageData` するよう追加（「キャラN」と同じ扱いに統一）
- **feat: 「ステータス確認」モーダルにも一時画像を反映**
  - `showStatusModal()`（`app-12-features-minigames.js`）のアバター画像は `user.charImage`／`charDef` からのみ組み立てており、`charImageData` を見ていなかったため、キャラ作成中でもステータス確認では元のキャラ画像が表示されていた
  - **`public/js/app-12-features-minigames.js`**: `avatarSrc = user.charImageData || '/chara/' + encodeURIComponent(imgFile)` を追加し、`.sm-avatar` の `src` をこちらに変更（ステージのアバターと同じ優先順位）
- **動作確認**: ステージの実コードで、キャラ作成相当の状態（`charImageData` 設定）を作ってから「ランダムキャラ」「キャラN」「エイリアス経由のキャラN」「ステータス確認」を実行し、いずれも一時画像が正しく反映・解除されることを確認。繰り返し実行・未使用ユーザーでの回帰も確認（全19項目通過）

---

## v2.959.0 — 2026-09-17

### feat: 「大きさ：大」を使うたびに大きくなるように（上限500%）＋「大きさ：特大」（大5回分）を追加

- **`public/js/app-01-core-characters.js`**: 定数 `SIZE_BIG_GROW_STEP`（1回あたり+20%）・`SIZE_BIG_MAX_PCT`（上限500%）・`SIZE_BIG_MP_COST`（大＝200MP）・`SIZE_TOKUDAI_MP_COST`（特大＝1000MP）・`SIZE_TOKUDAI_STEPS`（特大＝5回分）を追加。永続化用 `CHAR_SAVE_FIELDS` に `sizeBigPct` を追加（ページ再読み込みしても成長が消えない）
- **`public/js/app-06-comment-handler.js`**: 「大きさ：大」コマンドの処理を変更し、「大きさ：特大」を新規追加
  - 従来は毎回サイズが120pxに固定されていた（＝2回目以降は何も変わらない）のを、使うたびに120px→144px→168px…と20%ずつ大きくなるように修正
  - **大きさ：特大**：1000MPで「大きさ：大」5回分を一気に進める（同じ成長値 `sizeBigPct` を共有する加速コマンド。固定サイズではない）
  - 200MP（大）／1000MP（特大）消費は据え置き。上限（500% / 600px）に達した後は消費せず、「大きさ：大／特大は上限(500%)です」と表示。上限をまたぐ場合は上限で頭打ちしつつMPは消費する
  - 「大きさ：中」「大きさ：小」に切り替えても成長度合いは保持され、「大きさ：大」「大きさ：特大」のどちらでも続きから成長する
  - MP不足時は従来どおり拒否（成長・MP消費なし）
- **動作確認**: ステージの実コードでコメントを流し、初回100%→2回目120%→3回目140%、中/小を挟んでも保持、上限到達、MP不足時の拒否、新規ユーザーの初期値、特大の5回分一括成長・上限またぎ・MP不足を確認（全18項目通過）

---
## v2.958.0 — 2026-09-17

### docs: コマンドマニュアル（index.html）の汎用キーワードを登録内容に同期

`data/settings.json` の `sdKeywordPrompts` が 331件になったので、ルートの `index.html` を追従させた（画像はまだ無いのでキーワード名のみ）。

- **追加 58件**: `神` `SAZ` `花柄` `地雷` `囚人` `森神` `スト6` `見せられないよ` `マカロン` `ふわふわ` `ぬい` `ミホヨ` `絵服` `ほむら` `前が見えねェ` `くるま` `猫影` `ぼこちち` `おもちゃ` `ぷにき` `ゴミ箱` `スパンキング` `服影` `アーニャ` `モブサ` `OK.` `あくあ` `アキラ` `猫ダンス` `黒猫` `キング` `永遠` `タイ` `ひこいち` `トルコ` `ロシア` `ブラジル` `赤ずきん` `ドリブル` `カラオケ` `バニー` `天` `クライム` `BLE` `DOA` `3D` `ドール` `公園` `ランチ` `ブランコ` `作業員` `カウボーイ` `犬` `キャンプ` `巫女` `ボクサー` `モーニング` `騎士`
- **削除 1件**: `たえあえｇｈ`（テスト用に登録していたもの。設定側では既に削除済み）
- **キーワードサンプル（`sec-kwsamples`）**: サンプル画像がまだ無いため `KW_TEXT` に追記し、「📝 サンプル画像準備中」のカードで表示（76件 → 133件）。画像付きの 201件は変更なし
- **機能解説（`sec-features`）の汎用キーワードchip**: 登録順どおり 331件に更新
- 照合の結果、ほかに古い記載・重複は無し

---
## v2.957.0 — 2026-09-15

### feat: 画像生成キーワードを登録するとき、登録済みなら通知する

- **`public/admin.html`**「🎨 SD生成設定」の「キーワード別ポジティブ（汎用）」
  - **追加**: 既に登録されているキーワードなら**追加せず**、「⚠️ 「〇〇」は既に登録されています（N番目 / ポジティブ: …）。追加していません」と表示。登録済みの行を見える位置までスクロールして枠を光らせる。入力欄は消さない
  - **入力中**: キーワード欄に入力した時点で、登録済みなら同じ通知を出す（`_sdKwpCheckInput()`）
  - **編集（✏）**: ほかの行と同じキーワードに書き換えたときは保存せず、編集中の行のすぐ下に通知（一覧が長いため）。自分の行のままポジティブだけ変えるのは従来どおり保存できる
  - **組み込みキーワード**（ドット / リアル / もいちゃん）を登録しようとしたときも、別の欄で設定されていてそちらが優先される旨を通知
  - 比べるのは前後の空白を除いた完全一致（`ｽﾋﾟｷ` と `スピキ` のような表記違いは別のキーワードとして登録できる）
  - 追加できたときは「✓ 「〇〇」を追加しました」
- 同じキーワードが2つあってもステージ側は先に登録した方しか使わないため、重複登録は意味がなかった
- **動作確認**: Playwright で管理画面を開き（設定の保存・送信は遮断）、追加・入力中・編集・組み込み・前後の空白・表記違いを確認

---
## v2.956.0 — 2026-09-13

### feat: 生成画像がモザイクになったとき、引っかかったモザイクワードをパネルに表示

- **`public/js/app-11-agru-state-sd.js`**
  - `_sdMosaicHits()` を追加: モザイクワードのうち、元のプロンプト・翻訳後のどちらかに含まれるものを**すべて**返す（判定は従来どおり小文字にしての部分一致）
  - `_sdNeedsMosaic()` は `_sdMosaicHits()` の先頭を返す形に（会話モードの画像吹き出しなど、既存の呼び出し元の動きは変わらない）
  - `showSDImage()`: モザイクになったとき、キーワード行の下に **「🔲 モザイク」＋引っかかったワード** の行を表示。その分パネルの高さを1行ぶん増やす。ワードはエスケープして表示
- **`public/style.css`**: `.sd-image-mosaic`（キーワードのチップと同じ形の赤系）を追加
- 部分一致なので、`ass` が `class` に含まれて引っかかった、のような場合も何に当たったか分かる
- **動作確認**: ステージのコードで `showSDImage()` を呼び、引っかかったワードの表示・複数ヒット・大文字小文字・エスケープ・ヒットなしで行が出ないこと・モザイクがかかること・パネルの高さを確認

---
## v2.955.0 — 2026-09-13

### fix: 名前タグ【大谷】で画像生成のプロンプト書き換えが効かなくなっていた問題を修正

- **原因**: v2.888.0 で `stripPrefix()` が `【…】` を全部消すようにしたため、コメント末尾の名前タグ `【大谷】` が画像生成のプロンプトに残らず、`_sdGenerateOne()` の「大谷＋ぺこら系 → さくらみこに書き換え」の判定に当たらなくなっていた（最後に効いたのは 8/30。保存画像のファイル名で確認）
- **`public/js/app-06-comment-handler.js`**: `extractNameTags()` を追加。`【】` を消す前のコメント（`rawMessage`）から名前タグの中身を取り出し、生成コマンド（生成 / エコ生成 / 超生成）で `generateSDImage()` / `generateSDImageGomi()` / `generateSDImageCho()` に渡す
- **`public/js/app-11-agru-state-sd.js`**: 名前タグ（`nameTags`）をキュー経由で `_sdGenerateOne()` まで渡し、「大谷 / otani」の判定を **プロンプト または 名前タグ** で行うように。ぺこら系の判定は従来どおりプロンプトのみ
- 【】をコマンド判定・吹き出し表示・プロンプトから除く動きは変わらない（タグの中身をプロンプトに戻すことはしない）
- **動作確認**: ステージのコードに名前タグ付きのコメントを流し、画像生成リクエスト（`/api/sd-generate`）の中身を確認（実際の生成・投稿・保存は遮断）

---
## v2.954.0 — 2026-09-13

### feat: キーワードサンプルの変換ボタンで JPG も WebP にできるように

- **`lib/kw-samples.js`**: PNG に加えて **`.jpg` / `.jpeg`** も変換対象に（`convertPngs()` → `convertImages()`、`POST /api/kw-samples/convert-png` → `convert-images`、状態の `pngs` → `sources`）
  - 変換のしかたは PNG と同じ（WebP quality 85、同じ名前の WebP は置き換え、変換できたら元の画像は削除、壊れた画像は失敗として残す）
  - **同じ名前で PNG と JPG が両方あるとき**は PNG（→ JPG → JPEG の順）を使い、ほかは消さずに残して `skipped` で返す
  - **EXIF の向き**を反映して回転してから保存（スマホで撮った JPG が横倒しにならないように）。サイズの照合も回転後の縦横で行う
  - JPG / JPEG を置いてある（変換待ちの）キーワードも自動生成の対象から外す
- **`public/admin.html`**: ボタンを「🔄 PNG / JPG を WebP に変換」に。状態欄は「未変換の画像（PNG / JPG）」
  - 同じ名前の別形式を使って残したファイルは「〇〇.jpg（〇〇.png を使用）。不要なら消してください（残したままもう一度押すと、そのファイルで置き換わります）」と表示
- **動作確認**: 一時フォルダで JPG の新規変換・EXIF の向き（縦横が入れ替わる）・`.jpeg`・PNG と JPG の重複（PNG を使い JPG は残る→もう一度押すと JPG で置き換わる）・壊れた JPG・`generated.json` の更新を確認。Playwright で管理画面のボタン表示と JPG の変換を確認。本物の SD・配信サーバー・設定には触れていない
- **反映にはサーバーの再起動が必要**

---
## v2.953.0 — 2026-09-13

### feat: 手動で置いたキーワードサンプルの PNG を管理画面から WebP に変換

`chara/keyword-samples/` に手で置いた PNG を、管理画面のボタンで WebP にできるようにした（これまで手作業で変換していたもの）。

- **`public/admin.html`**: 「🖼 キーワードサンプル自動生成」に **🔄 PNG を WebP に変換** ボタンを追加
  - ボタンに未変換の PNG の件数を表示。状態欄に件数と一覧
  - 結果を「N枚を WebP に変換（うち置き換え M枚）／失敗 K件: ファイル名（理由）」で表示
- **`lib/kw-samples.js`**: `convertPngs()` と `POST /api/kw-samples/convert-png` を追加
  - `sharp` で WebP（quality 85）に変換。**同じ名前の WebP があれば置き換え**、変換できたら**元の PNG を削除**。変換後のサイズが元と違えば失敗扱いにして PNG は残す
  - 一時ファイルに書いてから置き換える。PNG はパスではなく読み込んだデータで `sharp` に渡す（パスで渡すとファイルを掴んだままになり、Windows で PNG を消せないため）
  - 変換した画像が登録キーワード（`<名前>.webp` / `<名前>_2.webp` …）のものなら、`generated.json` の枚数を実際の連番ファイル数に揃える → マニュアルの「サンプル画像準備中」のキーワードにも手で置いた画像がすぐ出る
  - PNG を置いてある（変換待ちの）キーワードは自動生成の対象から外す
  - `GET /api/kw-samples/status` が `pngs`（未変換の PNG 一覧）を返す
- **動作確認**: 一時フォルダで、新規変換・既存 WebP の置き換え・連番（`_2`）・未登録名・壊れた PNG（失敗して残る）・`generated.json` の更新・自動生成の対象から外れることを確認。Playwright で管理画面のボタン表示と変換結果を確認。本物の SD・配信サーバー・設定には触れていない
- **反映にはサーバーの再起動が必要**

---
## v2.952.0 — 2026-09-13

### feat: キーワードサンプル自動生成に「付与ワード」を追加

自動生成のときだけ付けたい語を、管理画面で自由に入れられるようにした。

- **`public/admin.html`**: 「🖼 キーワードサンプル自動生成」に **付与ワード** の入力欄（複数行、2000文字まで）を追加
  - 入力欄から離れたときに保存。入力してすぐ「▶ 生成開始」を押しても、開始時に一緒に送るので反映される
  - 保存の応答で入力欄を整えた値に書き戻すのは、送ったときから欄が変わっていないときだけ（枚数欄から付与ワード欄へ移った直後に入力した内容が、枚数の保存の応答で古い値に戻ってしまうのを防ぐ）
  - 状態欄に現在の付与ワードを表示。説明文にプロンプトの並び順を追記
- **`lib/kw-samples.js`**
  - プロンプトを **「付与ワード, `sdPositiveSuffix`, キーワードのポジティブ」** の順に。配信でコメント本文が入る位置に付与ワードが入るので、「<付与ワード> <キーワード>」とコメントしたときと同じ並びになる（翻訳はしない）
  - 空なら付けない。改行・連続空白は1つの空白にまとめ、前後の空白とカンマは落とす（`cleanExtra()`）
  - `data/server-config.json` の `kwSampleExtraPositive` に保存。`POST /api/kw-samples/config` と `POST /api/kw-samples/start` が `extra` を受け付け、`GET /api/kw-samples/status` が `extra` を返す
  - 生成中に付与ワードを変えても、動いている生成には影響しない（開始時の値で最後まで作る）
- **動作確認**: 偽の SD サーバーで、SD に送るプロンプトの並び・空欄のとき・整形・保存を確認。Playwright で入力欄の読み込み・保存・入力直後に開始した場合を確認。本物の SD・配信サーバー・設定には触れていない
- **反映にはサーバーの再起動が必要**（前回分と合わせて1回でよい）

---
## v2.951.0 — 2026-09-13

### feat: 管理画面からキーワードサンプル画像を自動生成できるように

サンプル画像がまだ無い登録キーワードの画像を、管理画面のボタン1つで SD 生成・WebP 保存できるようにした。1キーワードあたりの枚数も指定できる。

- **新規 `lib/kw-samples.js`**（`server.js` から `register()` で登録）
  - **対象**: 組み込みの `ドット` / `リアル` / `もいちゃん` ＋ `sdKeywordPrompts` のうち、ポジティブが空でなく `chara/keyword-samples/<キーワード>.webp` がまだ無いもの。ファイル名は `index.html` と同じ規則（`<>:"/\|?*` → `_`、60文字）で、同じファイル名になる語は先に出てくる方だけ
  - **生成設定**: 配信の画像生成と同じ `sdWidth` / `sdHeight` / `sdSteps` / `sdCfgScale` / `sdSampler` / `sdNegative`。プロンプトは `sdPositiveSuffix, <キーワードのポジティブ>`。`sharp` で WebP（quality 85）に変換
  - **保存名**: 1枚目 `<名前>.webp`、2枚目以降 `<名前>_2.webp`, `_3` …。途中で失敗しても番号は詰める。キーワードごとの保存枚数を `chara/keyword-samples/generated.json` に記録
  - **配信との両立**: SD の共有キュー `_sdQueue` に**1枚ずつ**積み、終わってから次を積む。まとめて積むと、後から来た配信の画像生成コマンドが全部の後ろに並んでしまうため
  - **停止**: 生成中の1枚が終わったところで止める。1枚180秒でタイムアウトし `/sdapi/v1/interrupt` を送る
  - **API**: `GET /api/kw-samples/status` / `POST /api/kw-samples/config`（`{count}`、1〜10枚。`data/server-config.json` の `kwSampleCount` に保存）/ `POST /api/kw-samples/start` / `POST /api/kw-samples/stop`
- **`public/admin.html`**: 「🖼 キーワードサンプル自動生成」セクションを追加（「🤖 AutoGen 自動生成」の下）
  - 1キーワードあたりの枚数、▶ 生成開始 / ■ 停止 / ↺
  - 状態・保存/失敗の数・生成中のキーワード・画像が無いキーワードの件数と一覧・開始したときの総枚数・生成設定・直近の結果。生成中は2秒ごとに更新
  - サーバーがこの機能より前の版のまま（API が 404）なら「サーバーを再起動すると使えます」と表示
- **`index.html`（マニュアル）**: 「サンプル画像準備中」のキーワード（`KW_TEXT`）で `generated.json` に枚数があれば、その枚数の画像を並べたカードで表示（3枚までは既存の2枚カードと同じ横並び、4枚以上は3枚ずつ折り返し）。`generated.json` が無い・載っていない語は従来どおり
- **動作確認**: 本物の SD・配信サーバー・設定には触れずに実施
  - 偽の SD サーバー＋一時フォルダで API を確認（対象の抽出、SD に送る値、番号の詰め方、`generated.json`、停止、生成中の二重開始、枚数の上限下限、生成の合間に配信の生成が入る順番）
  - Playwright で管理画面（表示・枚数保存・開始〜完了・キーワード名のエスケープ）とマニュアル（3枚・1枚のカード、`generated.json` が無いとき）を確認。JSエラーなし
- **反映にはサーバーの再起動が必要**

---
## v2.950.0 — 2026-09-13

### docs: キーワードサンプル画像を76件追加（index.html）

`chara/keyword-samples/` に置かれた PNG 76枚を、ルート `index.html` の「🖼 キーワードサンプル」一覧に反映。既存の画像付きカードは変更なし。

- **画像の変換**: 既存サンプルと同じ `sharp` の `webp({quality:85})` で WebP 化し、元の PNG は削除（合計 70.9MB → 5.9MB）。解像度はそのまま（896×1344 が75枚、`服資料` のみ 1920×1080 の横長）
- **ファイル名の対応**: `sketch.png` は登録キーワードに `sketch` が無く、LoRA が sketch 系の `スケッチ` のサンプルと判断して `スケッチ.webp` として保存
- **`KW_TEXT` → `KW` / `KW_SINGLE`**: 画像が付いた76件を「サンプル画像準備中」から外し、1枚表示（`KW_SINGLE`）に移動
  - 追加: `900` `92` `mememe` `VR` `ちびじど` `もふもふ` `アイス` `エイリアン` `エスカレーター` `カルト` `ガラスマスク` `ガングロ` `クラゲ` `ゴミ袋` `ゴースト` `サイバー` `サイン` `ジッパー` `スケッチ` `スタイル12` `スタイル14`〜`スタイル17` `スタイル20`〜`スタイル23` `スタイル25` `スタイル28`〜`スタイル30` `スタイル32` `スタイル34` `スタイル35` `スタイル37` `スタイル39` `スタイル40` `スタイル43` `スタイル44` `スタイルQ` `スタイルR` `スタイルW`〜`スタイルZ` `ディノタウロス` `ドラジ` `ネカフェ` `バブル` `パンチ` `ファイ` `ポテト` `ポール` `マグショット` `マネキン` `メガネ` `モンスター` `レトロトゥーン` `悪魔化` `抱き枕` `拘束` `撮る` `月刀` `服資料` `発光` `相撲` `絡まり` `腹パン` `腹口` `花` `虹` `貫通` `足裏` `金` `靴下`
  - 画像付き 125件 → 201件、準備中 152件 → 76件（カード総数 277件は変わらず）
- **動作確認**: 一時サーバーで index.html を開き、全画像を読み込ませて表示・404・JSエラーを確認

---
## v2.949.0 — 2026-09-13

### docs: コマンドマニュアル（index.html）に追加キーワード10件を掲載

`data/settings.json` の `sdKeywordPrompts` が 264件 → 274件に増えたので、ルートの `index.html` を追従させた。

- 追加したキーワード: `魔法` `スカルガールズ` `スタイル47` `スタイル48` `スタイル49` `絵画` `ブラックライト` `スタイル50` `処刑` `たえあえｇｈ`
- **キーワードサンプル（`sec-kwsamples`）**: サンプル画像がまだ無いので、既存の `KW_TEXT` に追記して名前だけのカードで出す（`KW_TEXT` 142件 → 152件）
- **キーワード自動追加（`sec-features` の汎用チップ）**: 10件を追加し、`settings.json` の274件と一致させた（`title` はキーワード名）
- 照合の結果、古い記載・重複は無し。前回 `KW_TEXT` に入れた142件にも、その後画像が付いたものは無かった

---
## v2.948.0 — 2026-09-13

### fix: サーバー未接続のまま管理画面で値を変えたら警告を出す（消えた画像生成キーワード9件も復旧）

- **経緯**: サーバーが止まっている間に管理画面で画像生成キーワードを9件追加 → 管理画面の localStorage とステージへの送信（BroadcastChannel / WebSocket）には乗ったがサーバーに届かず、サーバー起動後にステージの `preloadServerSettings()` がサーバーの古い値で localStorage を上書きして消えていた
- **データ復旧（コード変更なし）**: Chrome の Local Storage（LevelDB）に古い版が残っていたため、LevelDB を直接読むツール（SSTable＋Snappy展開＋WAL）で `sdKeywordPrompts` の全版を抽出。273件の版から、サーバーに無かった **魔法 / スカルガールズ / スタイル47 / スタイル48 / スタイル49 / 絵画 / ブラックライト / スタイル50 / 処刑** の9件を復元した
  - 既存264件は中身・順序とも一致していたので末尾に追加する形。`/api/settings` へ PATCH したうえで、稼働中のステージにも `sdKwpUpdate` を送り、ステージの自動保存で古い値に戻らないことを確認
- **`public/admin.html`**: 値を変えたのにサーバーへ届かないとき、**画面上部に赤い警告**を出す（自動の再送はしない）
  - `adminSend()` で WebSocket が未接続なら `warnServerDown()`。`getState` / `ping` / `identify` など読み取り系は対象外
  - 直接保存の `_patchSettings()`（`PATCH /api/settings`）が失敗・非200でも警告
  - 警告中にサーバーへ再接続したら、**「未接続の間に変えた値は保存されていないので設定し直してください」** に切り替えて知らせる（✕で閉じるまで残る）
- **動作確認**（Playwright で管理画面を操作。設定の書き込みは実サーバーに届かないよう遮断）
  - 接続中の変更では警告が出ない／WebSocket 未接続での変更で警告が出る／`getState` だけでは出ない
  - WebSocket 接続中でも PATCH が失敗すれば警告が出る／再接続すると黄色の「保存されていない」表示に切り替わる。JSエラーなし
...
`data/settings.json` の `sdKeywordPrompts` が 122件 → 264件に増えたので、ルートの `index.html` を追従させた。

- **キーワードサンプル（`sec-kwsamples`）**: 今回の **142件** を追加。サンプル画像がまだ無いので、新しい配列 `KW_TEXT` に入れて**名前だけのカード**（点線枠・「📝 サンプル画像準備中」）で出す
  - 既存の `KW` に入れると、1件ごとに存在しない画像を2枚読みにいって 📷 の空欄が並ぶため分けた
  - 画像を用意したら、そのキーワードを `KW_TEXT` から `KW`（2枚）か `KW_SINGLE`（1枚）へ移す
  - 凡例に「📝 画像準備中（名前のみ）」を追加
- **キーワード自動追加（`sec-features` の汎用チップ）**: **171件** を追加し、`settings.json` の264件と一致させた
  - 今回の142件に加え、**以前から載っていなかった29件**（`アリス` `ジョジョ` `スライム` `soi` `waoo` など。サンプル画像はあるのにチップだけ抜けていた）も入れた
  - マウスを乗せたときの説明（`title`）は、LoRA名から意味を推測すると外れるので既存の `スタイルE` と同じくキーワード名にした
- `デュオリンゴ` `催眠術` はプロンプトが変わっただけで、キーワード名は同じなので掲載内容に変更なし

---
## v2.944.0 — 2026-09-13

### fix: 画像生成キーワードを「長い方優先」で判定するように（部分一致の巻き込みを解消）

- **症状**: キーワード判定が `prompt.includes(keyword)` の部分一致だったため、**片方が片方を含む組み合わせで両方が発動**し、さらにプロンプトに残骸が残っていた
  - 例）`水` と `水墨` が登録されている状態で「水墨 ねこ」→ **両方のポジティブが付き**、短い `水` が先に除去されてプロンプトが「**墨** ねこ」になる（翻訳にも「墨」が乗る）
  - 「スタイル1」と「スタイル11」のような番号違いでも同じことが起きる（現状137件では `水 ⊂ 水墨` の1組が該当）
- **`app-11-agru-state-sd.js`**: `_sdGenerateOne()` のキーワード判定を作り直した
  - 組み込み（`ドット` / `リアル` / `もいちゃん`）と `sdKeywordPrompts` を1つの候補リストにまとめ（同名は先勝ち）、**文字数の長い順に照合して、当たったらその場で本文から除去**する。長い方を先に消すので短い方はもう一致しない
  - ポジティブを付ける順番は従来どおり（組み込み → 登録順）。表示チップとDiscord通知に渡す `_kwUsed` も同じ並び
  - 「もいちゃん」表記が無くても `moi` でスタイルだけ足す挙動（本文は消さない）はそのまま維持
- **動作確認**（Playwright、`/api/sd-generate` をスタブ化）
  - 「スタイル11 ねこ」（スタイル11を仮登録）→ キーワードは **スタイル11 のみ**、SDに送るプロンプトは「ねこ」（残骸なし）
  - 「スタイル1 ねこ」→ スタイル1 のみ
  - 「水墨 ねこ」→ **水墨のみ**（従来は水も発動して「墨 ねこ」になっていた）／「水 ねこ」→ 水のみ／「水墨 水 ねこ」→ 両方（正しく併用）
  - 「ドット リアル ねこ」→ 従来どおり両方。JSエラーなし

---

## v2.943.0 — 2026-09-13

### change: 生成画像のDiscord通知にキーワードも載せる

画像ポップアップには v2.930.0 で「効いたキーワード」を出すようにしたが、Discordへの連携には載っていなかった（キーワードはプロンプトから除去してSDに送るため、本文からも消えていた）。

- **`app-11-agru-state-sd.js`**: `/api/sd-generate` のリクエストボディに **`keywords`（`_kwUsed` の重複除去）** を追加。**ポップアップのチップと同じ内容**を送る
- **`server.js`**: `/api/sd-generate` で `keywords` を受け取り、`sendToDiscord()` に渡すよう変更
- **`server.js`**: `sendToDiscord()` に第5引数 `keywords` を追加し、本文に **`🏷 \`ドット\` \`リアル\` \`ageru\``** の行を足す（重複除去・空文字除外・最大12件）。キーワードが無いときは行ごと出さない
- **動作確認**
  - サーバー側: `sendToDiscord()` を切り出してローカルの受け口へ送信し、本文が「🎨 名前／プロンプト→英訳／🏷 キーワード」の3行になり画像も添付されることを確認（実際のDiscordには送っていない）
  - クライアント側: Playwright で `/api/sd-generate` をスタブ化し、送信ボディの `keywords` と**ポップアップのチップが完全一致**することを確認。JSエラーなし
...
### feat: ランキングパネルの表示人数・ダメージ欄の表示切替・リセットを管理ウィンドウから操作可能に

- **ダメージランキングのリセット**: 既存の `resetCumulativeDmg` ボタンを **「🗑 ダメージランキング リセット」** に名称変更（旧「通算DPSリセット」）。処理も見直し、`cumulativeDmgMap` に加えて **`bossDamageMap`（現ボス戦ぶん）も消して完全に空にする**ようにし、パネルの再描画とシステムログ出力を追加（`app-13-race-admin-misc.js`）
- **ダメージ欄の表示切替**: 管理ウィンドウに **「⚔️ ダメージ欄 表示中 / 💎 MPのみ表示中」トグル**を追加。`rankingDmgHidden`（既定OFF）で、**ダメージ欄を丸ごと隠してMPランキングだけにできる**
  - 隠したときは位置リセット（↺）と閉じる（✕）ボタンを **MPの見出しへ自動で移す**ので操作不能にならない
  - ボタンの文言と `active` 表示は、管理ウィンドウ接続時の `getState` でも同期する
- **表示人数の設定**: **「ランキング 人数」スライダー**（1〜20人・既定3人）を「📐 パネル外観」に追加。`rankingTopN` でダメージ／MP両方の表示件数を決める
  - 4位以降は 🥇🥈🥉 の代わりに「4位」「5位」…と数字で表示する（`rankMark()`）
- `rankingDmgHidden` / `rankingTopN` とも `app-03` の変数・`app-01` の同期キー・`app-13` の管理メッセージと `getState`・`admin.html` の設定マップ／初期同期に追加済み（OBS・他クライアントにも共有される）
- **動作確認**（Playwright、6人ぶんのダメージとMPを用意して検証）
  - 人数3→6でパネルが6行→12行（ダメージ6＋MP6）になり、4位以降が「4位」「5位」表記になること
  - ダメージ欄を隠すとMPの6行だけになり、↺と✕がMP見出しに残ること
  - リセットで `cumulativeDmgMap` / `bossDamageMap` が0件・「データなし」表示になり、MP側は影響を受けないこと
  - 管理ウィンドウ側もボタン・スライダーが表示されJSエラーなし（`/api/ollama-models` の500はOllama未起動によるもので本件とは無関係）

---

## v2.941.0 — 2026-09-09

### feat: BOタイトル画像を title2 に差し替え、大きさを管理ウィンドウから変更可能に

- **画像を差し替え**: `public/bo/title2.png`（1983×793・1.74MB）から **`title2.webp`（400×160・36KB）** を生成し、パネルはこちらを読む。元のPNGと旧 `title.png` / `title.webp` はそのまま残してある
- **大きさの設定**（`boTitleSize`・既定56px）: 管理ウィンドウ「📐 パネル外観」に **「BO タイトル大きさ」スライダー**（20〜160px）を追加。CSS変数 `--bo-title-h` で高さに反映し、「BO 文字サイズ」設定とも掛け合わせて拡縮する
- **ヘッダーの高さは画像サイズに影響されないように変更**: タイトル画像を `position: absolute`（左端・上下中央）にして、**どれだけ大きくしてもヘッダーは薄いまま**上下にはみ出すようにした
  - 「稼働中」バッジと重ならないよう、ヘッダーの左パディングを `calc(var(--bo-title-h) * var(--bo-fs) * 2.55 + 14px)`（ロゴの縦横比ぶん）で自動確保
- `app-03` の変数・`app-01` の同期キー・`applyPanelSettings()`・`app-13` の管理メッセージと `getState`・`admin.html` の設定マップ／初期同期にも追加済み
- **動作確認**（Playwright）: 40 / 56 / 100 / 140px と切り替えて、画像だけが 46→161px と拡大し、**ヘッダー高さは33pxのまま**、バッジとの重なりも無いことを確認。`/bo/title2.webp` は404なし・JSエラーなし

---

## v2.940.0 — 2026-09-09

### change: BOのヘッダーを薄くし、タイトル画像を大きくはみ出させる

- **`public/style.css`**: `.bo-header` の上下パディングを 5px → **2px** に詰め、角丸（`11px 11px 0 0`）をヘッダー側に持たせた
- **`.bo-title-img`**: 高さ `32px` → **`56px`**（文字サイズ設定に連動）。上下に `calc(-17px * var(--bo-fs))` のネガティブマージンを与えて、**ヘッダーの高さを押し広げずに上下へはみ出す**ようにした。影（`drop-shadow`）も追加
- はみ出した部分が切れないよう、`.bo-panel` の `overflow: hidden` を外した（角丸はヘッダー側の `border-radius` で維持）
- **動作確認**（Playwright）: ヘッダー高さ **31px**（従来より薄い）に対してタイトル画像は **61px**、上に16px・下に14pxはみ出し、パネル上端からも13px飛び出して表示されることを確認。チャートやマーカーの表示崩れなし

---

## v2.939.0 — 2026-09-09

### change: BOのエントリー音を「ジャン！」に変更

- **`app-01-core-characters.js`**: `SOUND_BO_ENTRY` を追加（`/sound/endcard/nc201523_【効果音】ジャン！（短）.mp3` を `encodeURIComponent` でURLエンコード。他の効果音定数と同じ書き方）
- **`app-12-features-minigames.js`**: `handleBoEntry()` の効果音を `SOUND_SLOT_STOP`（音量0.5）から `SOUND_BO_ENTRY`（音量0.7）に差し替え。**誰かが賭けるたびに鳴る**
- **動作確認**（Playwright）: エントリー時に該当のmp3がリクエストされること、404が出ないことを確認

---

## v2.938.0 — 2026-09-09

### change: BOチャートの先端を猫の絵文字に、タイトル画像を軽量版に差し替え

- **先端の●を 🐱 に変更**: SVGの `<circle class="bo-dot">` を廃止し、チャートに重ねるHTML要素 `.bo-cat` にした
  - SVGは `preserveAspectRatio="none"` で横に引き伸ばされるため、絵文字をSVG内に置くと歪む。HTML要素で重ねて `translate(-50%,-50%)` で中心合わせしている
  - 大きさは `calc(20px * var(--bo-fs))` で「BO 文字サイズ」設定に連動。位置は毎ティック更新（線の右端＝現在値に追従）
- **タイトル画像を軽量化**: `public/bo/title.webp`（400×160・34KB）を生成し、パネルはこちらを読むよう変更。元の `title.png`（1983×793・1.6MB）は残してある（**約49分の1**）
- **動作確認**（Playwright）: 猫がチャート内に収まって表示され、時間とともに右へ進む（7.36% → 8.94%）ことを確認。旧 `.bo-dot` が残っていないこと、タイトルが `/bo/title.webp` から404なしで読めることも確認。JSエラーなし

---

## v2.937.0 — 2026-09-09

### feat: BOパネルのタイトルを画像化、線の色と背景不透明度を管理ウィンドウから変更可能に

- **タイトルを画像に**: ヘッダーのテキスト「✨ MAGICAL OPTION」を **`public/bo/title.png`** に差し替え（`.bo-title-img`）。高さは `calc(32px * var(--bo-fs))` で、既存の「BO 文字サイズ」設定に合わせて拡縮する
- **線の色**（`boLineColor`・既定 `#1565c0`）: 管理ウィンドウ「📐 パネル外観」に **「BO 線の色」カラーピッカー**（＋リセットボタン）を追加。CSS変数 `--bo-line` で `.bo-line` の `stroke` と `.bo-dot` の `fill` に反映する
  - SVG に直書きしていた `stroke="#38bdf8"` を外し、CSS変数側で色を決めるようにした
  - `type: 'color'` の管理メッセージ（既存の背景色と同じ経路）で受け取る
- **背景の不透明度**（`boPanelBgOpacity`・既定100%）: 「BO 背景不透明度」スライダー（0〜100%）を追加。CSS変数 `--bo-bg-a` を**パネル地・ヘッダーの金帯・チャートの斜めストライプ**すべてに掛けているので、枠線と文字は残したまま**中身だけ均一に透ける**
- どちらも `app-03` の変数・`app-01` の同期キー・`applyPanelSettings()`・`app-13` の管理メッセージと `getState`・`admin.html` の設定マップ／初期同期に追加済み（他のクライアントとOBSにも共有される）
- **動作確認**（Playwright）
  - タイトル画像が `/bo/title.png` から読み込まれ、高さ32pxで表示されること（404なし）
  - 線色を `#e91e63` に変更 → 線と現在値の点が両方その色になり、`localStorage` にも保存される
  - 背景不透明度40% → パネル地が `rgba(255,253,243,0.4)` になり、ヘッダーとチャートも同じだけ透ける
  - JSエラーなし
...
### change: BOのUIを kukuluLIVE「マジカルコレクト」に合わせ、ヘッダーを MAGICAL OPTION に

参照元は `https://live.erinn.biz/login.slot.php`（マジカルコレクト）。実際のページから配色を抽出して合わせた（ゲーム枠の金 `#ffdb53` / 淡い金 `#ffe48f` / タイルのクリーム `#fff5bf` / ナビの黄 `#ffe73f`）。

- **`public/js/app-12-features-minigames.js`**: パネルのヘッダー表記を「📈 マジカルオプション」→ **「✨ MAGICAL OPTION」** に変更
- **`public/style.css`**: `.bo-panel` 一式をライトテーマに全面差し替え
  - パネル: クリーム白 `#fffdf3` ＋ 金の枠線 `#ffdb53`（3px・角丸14px）、影は金寄りに
  - ヘッダー: 金のグラデーション帯（`#ffe9a3` → `#ffd53f`）＋下線 `#f0b90b`、タイトルは焦茶 `#7a5600` に淡い金のテキストシャドウ
  - フォント: 参照ページが読み込んでいる系統に合わせて **`M PLUS Rounded 1c`**（未読込環境向けに `Meiryo UI` → `system-ui` へフォールバック）。ステージ側で既に読み込み済みのフォント
  - チャート: 参照ページのゲーム枠と同じ**斜めストライプ**（`repeating-linear-gradient(45deg, #fff5bf, #ffefa6)`）＋金の枠線。レート線は視認性優先で青 `#1565c0`（参照ページの「2X」の青に合わせた）
  - 「稼働中」バッジ・配当表示・急落／急騰チップも淡い金＋焦茶系に統一。HIGH/LOW と損益は明るい地でも読める `#1b9e4b` / `#e53935` に変更
  - マーカーのラベルは白地＋色付き枠線＋薄い影に。**賭けた値段から伸びる線は白だとクリーム地で見えないため、濃い茶 `rgba(90,70,10,0.75)`（判定済みは薄め）に変更**
- 勝敗アニメーション（`boWinJump` / `boLoseFall`）とサイズ・重なり順・文字サイズの設定（`--bo-w` / `--bo-chart-h` / `--bo-fs` / `--bo-z` / `--bo-scale`）はそのまま維持
- **動作確認**（Playwright）: 4人ぶんのマーカーと直近の結果を出した状態で描画し、フォントが `M PLUS Rounded 1c`、背景 `#fffdf3`、枠線 `#ffdb53` になること、長い名前のラベルが2行で秒数まで出ること、JSエラーが無いことを確認

---

## v2.935.0 — 2026-09-09

### fix: BOのマーカーで名前が長いと残り秒数が消える問題を修正

- **症状**: 賭けた時点のラベルで名前が長いと、`⏱15秒` が省略されて見えなくなっていた（`max-width` ＋ `text-overflow: ellipsis` で末尾から切れていた）
- **`app-12-features-minigames.js`**: ラベルを「名前＋金額」（`.bo-marker-name`）と「残り秒数／結果」（`.bo-marker-left`）の2要素に分離
- **`public/style.css`**: ラベルを `display:flex; flex-wrap:wrap` にし、**名前側だけ省略・秒数側は縮めない**（`flex:0 0 auto`）構成に変更。1行に収まらないときは**秒数が2行目に折り返す**
  - 0幅のマーカー基点に対する絶対配置なので、`width: max-content` ＋ `max-width` で「内容なりに広げてから上限で止める」形にした（これが無いと最小幅に潰れて常に2行になる）
  - 最大幅を 122px → 150px（文字サイズ100%時）に拡大
- **`app-12-features-minigames.js`**: ラベルの重なり回避を作り直し
  - 2行になったラベルの高さを**実測値**（描画済み要素の `offsetHeight`）で扱う。初回だけ全角/半角の文字幅から見積もる
  - 重なり判定を矩形の重なりに変更し、**本来の向き→逆向き**の順に空きを探す。どこも空いていなければ**重なり面積が最小の位置**に置く
- **動作確認**（Playwright）: 「あ」「ながいなまえのひと」「すごくすごくながい名前のリスナーさん」「Bob」「半角カナの長い名前」の5人が同時にエントリー → **全員の秒数が表示され、ラベル同士の重なり0組**、短い名前は1行・長い名前は2行になることを確認

### feat: BOのレートを管理ウィンドウから急落／急騰させられるように

- **`public/admin.html`**: BOセクションに **「📉 急落」「📈 急騰」ボタン**を追加。**押している間だけ**レートが動く（`onmousedown` で送信、`onmouseup` / `onmouseleave` / ウィンドウの `mouseup`・`blur` で停止）
- **`app-12-features-minigames.js`**: `setBoForce(dir)` を新設し、`_boNextPrice()` に `BO_FORCE_STEP = 0.5`／ティック（±0.75〜1.25倍の揺らぎ付き）の力を加算。`BO_FORCE_MAX_MS = 8000` の保険で、mouseup を取りこぼしても8秒で自動的に止まる
- パネルのヘッダーに **「📉 急落中」「📈 急騰中」** のチップを点滅表示（`.bo-force`）。再描画しても表示が保たれる
- **`app-13-race-admin-misc.js`**: 管理メッセージ `boForce` を追加
- **動作確認**（Playwright）: 急落を2秒 → **-9.50**、離すと通常の揺れ（1秒で-0.66）に復帰。急騰2秒 → **+9.40**。チップの表示・クラス切り替え、8秒保険での自動解除も確認。管理画面側もボタン4つ（開始／終了／急落／急騰）が出てJSエラーなし

---

## v2.934.0 — 2026-09-08

### fix: BOから戻ったキャラの向きが反転したままになる問題を修正

- **症状**: 判定後に元の位置へ戻ると、キャラが賭ける前と左右逆を向いていることがあった
- **原因**: 待機させるとき（パネル側を向かせる）と帰り道（進行方向を向かせる）で `facingRight` を書き換えていたのに、**元の向きを控えていなかった**ため、戻った先の向きが「帰ってきた方向」のままになっていた
- **`app-12-features-minigames.js`**: `u._boParked` に `facingRight` も控えるようにし、`_boReleaseUser()` で**戻り終えたタイミングで賭ける前の向きへ復元**する（`applyFacingFlip()` を再適用）。歩き出す場合は歩行側が向きを決めるので、そのまま自然に上書きされる
- **動作確認**（Playwright）: 右向き（`scaleX(-1)`）と左向き（変形なし）の2人で検証。待機中はどちらもパネル側を向き、帰宅後は **`facingRight` も画像の transform も賭ける前と完全一致**することを確認。位置も元どおり。JSエラーなし
...
- **`app-12-features-minigames.js`**: エントリーしたキャラを**BOパネルの右側に縦一列で待機**させる `_boParkBettors()` / `_boParkTo()` を新設
  - 移動タイマー・歩行を止め、元の座標と歩行状態を `u._boParked` に控えてから移動（0.6秒のトランジション付き）。パネル側を向く
  - 間隔はアバターの実寸基準（`.character` の枠からはみ出すため、枠とアバターの大きい方を使う）。縦に入り切らない人数になったら右へ折り返す
  - タイマン・バトルロイヤル・ボスアゲルバトル中は各機能が配置を握っているので待機させない（`_boCanPark()`）
- **`app-12-features-minigames.js`**: 判定時に `_boPlayResultAnim()` で**勝ち＝跳ねる／負け＝倒れる**アニメーションを再生し、終わったら `_boReleaseUser()` で元の動きに戻す
  - 同じ人が複数エントリーしている場合は、**全部の判定が終わるまで待機を解除しない**
  - 判定で抜けたぶんは1.6秒後に詰め直す。BO終了時（`stopBo()`）は待機中の全員を解除する
- **`public/style.css`**: `boWinJump`（4回バウンド）と `boLoseFall`（前のめりに倒れて伏せたまま）を追加。画像の左右反転や位置指定と衝突しないよう `.avatar` に当て、コメント時ジャンプより後に定義して優先させた
- **`app-01-core-characters.js`**: 集合処理3か所（`gatherCharacters()` / `gatherContentMode()` / `gatherCharactersBottom()`）で**待機中のキャラを対象外**にした
  - 新しいキャラが出るたびに走る自動集合（キャラ生成の500ms後）に待機列が引きずられて、パネル横から下段へ吸い込まれていたため
- **動作確認**（Playwright、実配信への投稿と共有設定の保存はスタブ化）
  - 3人がエントリー → 全員パネル右端+10pxに縦一列で並び、`_boParked` が立つことを確認
  - 判定時に勝者へ `bo-win`、敗者へ `bo-lose` が付き、演出後に `_boParked` が外れて元の動きに戻ることを確認
  - 集合除外を入れる前は1.5秒後に下段へ吸い込まれていたが、修正後は待機位置を維持することを確認。JSエラーなし

---

## v2.930.0 — 2026-09-08

### change: 生成画像のポップアップに「効いたキーワード」を表示

キーワード（`ドット` / `リアル` / `もいちゃん` / `sdKeywordPrompts` の登録分）は**プロンプト本文から除去してからSDに送る**仕様のため、ポップアップの表示（英訳プロンプト）からも消えていて、どのキーワードで生成されたか分からなかった。

- **`app-11-agru-state-sd.js`**: `_sdGenerateOne()` に `_kwUsed` を追加し、マッチしたキーワードを控えるようにした（`ドット` / `リアル` / `もいちゃん` / `moi`（正規表現マッチのぶん）／`sdKeywordPrompts` の登録キーワード）。除去用の `_kwStrip` とは別管理で、`moi` は除去されないが表示には出す
- **`app-11-agru-state-sd.js`**: `showSDImage()` に第7引数 `keywords` を追加。ユーザー名・英訳プロンプトの下に**キーワードのチップを並べる**（重複は `Set` で除去）。キーワードがあるときだけ1行ぶん（18px）ポップアップを高くする
- **`public/style.css`**: `.sd-image-keywords` / `.sd-image-keywords span` を追加（紫のチップ・1行に収まらない分は隠す）
- 対象は `生成` / `出ろ` / `出して` / `gen` / `エコ生成` / `超生成`（すべて `_sdGenerateOne()` 経由）
- **動作確認**（Playwright、`/api/sd-generate` をスタブ化してSD本体なしで検証）
  - `ねこ ドット リアル ageru` で生成 → SDに送られたプロンプトは `ねこ`（キーワードは除去済み）、ポップアップには **ドット / リアル / ageru** の3チップが表示され、高さも1行ぶん増える
  - キーワードを含まない `ねこ` では chip 行自体が出ないことを確認。JSエラーなし

---

## v2.929.0 — 2026-09-08

### change: DINO のランキングをノーマル／ハードで分離

同じ土俵で比べられない2つのモードが1つのランキングに混ざっていたので分けた。

#### 保存と集計（`cloudflare/worker.js`）

- まとめるときのキーを「名前」から **「名前＋モード」** に変更。同じ人でも**モードごとに1件ずつ**、それぞれの最高スコアが残る
- 保持件数（`RANKING_MAX = 100`）も**モードごとに数える**
- `GET` と `POST` は `ranking`（ノーマル）と `rankingHard`（ハード）を分けて返す。保存の形自体は1つの配列のまま（`hard` フラグで振り分ける）なので、既存データはそのまま使える
- 登録時に返す順位も、そのスコアが属するモードのランキング内での順位になる

#### 表示（`cloudflare/pages/dino.html`）

- **いまのモードのランキングだけ出す。** ノーマルなら `Normal — Top 10`、ハードなら `🅗 Hard — Top 10`。もう片方は隠す
- 「最新のプレイ」はどちらのモードでも共通で出す（🅗 の有無で見分けられる）
- 表は常に2つなので、ゲームオーバー画面の詰まり具合は従来どおり
- **自己ベストもモードごとに分けた。** localStorage のキーを `dino-best` と `dino-best-hard` に分け、モードを切り替えると表示中の `HI` も切り替わる

#### 確認

ノーマルとハードで表示される表が入れ替わること（どちらも2つ）、モードを切り替えると自己ベストが 777 と 42 で入れ替わり戻すと元に戻ること、ハードで死んだときのゲームオーバー画面がハードのランキングを出しつつ縦スクロールもはみ出しも無いことを、1440x900 / 1280x720 / 390x844 で確認した。

---

## v2.928.0 — 2026-09-08

### change: DINO — 歌詞の入れ替え / スコア上限 / いつでも登録 / 同名マージ

#### 背景の歌詞を10秒ごとに入れ替える

- 一度出たあとは `LYRICS_SWAP_MS = 10000` ごとに並びを作り直す。毎回シャッフルするので、出てくる語と配置が変わる
- 切り替えは `.swap` クラスで 0.6秒フェードアウト → 差し替え → フェードイン。初回表示の 2.2秒フェードとは別の速さにしている
- 作り直しのたびに領域の再計算も走らせる（`lyRectKey` をリセット）
- ゲームをやり直したらタイマーを止める

#### スコアの上限を100万に

- `SCORE_MAX = 1000000`。距離から計算する分にも、カニを弾いたボーナスにも効かせる
- 表示は5桁ゼロ埋めのままなので、上限に達すると `1000000` と7桁で出る

#### ハイスコアをいつでも登録できるように

- 登録処理を `submitScore()` に切り出し、**ゲームオーバー画面**と**ランキング画面**の両方から呼べるようにした
- ランキング画面に「自己ベスト ○○ を登録」欄を追加。プレイ中でも死んだ後でも、いつでも自己ベストを送れる
- **登録しても入力欄を隠さなくした。** 何度でも押せる（同名は最高スコアにまとまるので、重ねて押しても害がない）

#### 同じ名前は最高スコアにまとめる

- `worker.js` の手動登録で、同じ名前のエントリを1件にまとめ、**いちばん高いスコアだけ残す**
- 書き込みのたびに一覧全体を整理するので、**既存の重複も自動で片付く**。実際に適用したところ 33件 → 17名になった（`ｂｂｂｂ` の14件が1件になるなど）
- 最新のプレイ一覧はプレイ記録なのでまとめない

#### 確認

スコアに巨大な距離を入れて 1000000 で頭打ちになること、歌詞が9.5秒後にフェードを挟んで入れ替わること、ランキング画面からの登録が通ってボタンが再度押せること、ゲームオーバー画面の登録欄が登録後も残ること、同名で 500 → 1500 → 900 と登録して 1500 だけが1件残ることを確認した。

---

## v2.927.0 — 2026-09-08

### feat: DINO にハードモード（急降下なし）を追加

ヘッダーの `HARD` ボタンで切り替える。**急降下が使えなくなるだけで、障害物も速度もほかは通常と同じ。**

- 一度ジャンプすると滞空1.0秒を最後まで使い切るので、跳んだ直後に「跳ぶと当たる高い星」が来る場面がそのまま重くなる
- **配置の保証はそのまま効く。** 障害物の間隔計算はもともと急降下を考慮しておらず、通常のジャンプ軌道だけを前提にしている。つまりハードモードが計算どおりの難易度で、通常モードは急降下があるぶん楽、という関係
- ボットもハードモードでは急降下しない
- 選択は localStorage に覚える。**プレイ中に切り替えたら仕切り直す**（途中で難易度が変わるとスコアの意味が崩れるため）
- スタート画面に `HARD MODE — 急降下なし` を出し、急降下の説明行は薄くする

#### ランキング

- ハードモードで出したスコアには **🅗** を付ける
- `worker.js` の保存項目に `hard` を追加。手動登録・自動登録のどちらも送る
- 既存のスコアには `hard` が無いので、通常モード扱い（マークなし）になる

#### 確認

通常モードでは高さ144pxから0.068秒で着地、`HARD` に切り替えると同じ高さで押しても降下せず0.535秒かかることを実測。切り替え時にプレイ中の状態が `ready` に戻ること、ハードモードで死んだスコアが 🅗 付きで最新プレイの先頭に入ることも確認した。

---

## v2.926.0 — 2026-09-08

### change: 管理ウィンドウをセクションごとのアコーディオンに（既定は全部閉じる）

セクションが35個まで増えて縦スクロールが長すぎたため、見出しクリックで開閉できるようにした。

- **`public/admin.html`**: 読み込み時に全 `.section` へ `acc` / `closed` を付け、`.section-title` のクリックで `closed` をトグルする `initAccordion()` を追加。**ページを開いた直後は必ず全部閉じた状態**（開閉状態は保存しない）
- CSS: `.section.acc.closed > .section-title ~ *` を `display:none` にして中身を隠す。見出しの右端に `▾`／`▸`（閉じているとき）を出し、閉状態では見出しの下マージンも詰める
  - 1つのセクションに見出しが2つある箇所（「💬 コメント物理オブジェクト」内の「📊 配信サマリー」）があるため、`~` セレクタで**最初の見出しだけ残して以降を隠す**方式にした
- ヘッダー直下に **「▾ すべて開く / ▸ すべて閉じる」ボタン**（`accAllBtn`）を追加。全部閉じたままだとブラウザの Ctrl+F で探せなくなるため、一括展開できるようにした。ボタンの文言は現在の状態に追従する
- **動作確認**（Playwright で `admin.html` を実際に開いて確認）
  - 読み込み直後：35セクションすべて `closed`、ページ高さ **14,987px → 1,727px**（約1/9）
  - 見出しクリックで対象セクションだけ開き、中身の入力欄が表示されることを確認
  - 「すべて開く」→ closed 0件、「すべて閉じる」→ 35件に戻ることを確認
  - **閉じた状態でも入力値は読める**（`boJudgeSeconds=15` / `boPanelScaleSlider=145`）ため、既存の送信処理・初期同期には影響なし。JSエラーなし

---

## v2.925.0 — 2026-09-08

### change: BO（マジカルオプション）の勝敗履歴を件数指定可能に、下部の説明文を削除

- **勝敗履歴の表示件数を設定可能に**（管理ウィンドウ「📐 パネル外観」→「BO 履歴件数」）
  - `boResultMax`（0〜30件・既定8件）を追加。**0にすると履歴欄ごと非表示**になる
  - 内部の保持は上限30件にして、描画時に `results.slice(0, boResultMax)` で絞る方式。件数を増やしたときにすぐ埋まるようにした（`BO_RESULT_MAX` 定数は廃止）
  - `app-03` の変数・`app-01` の同期キー・`applyPanelSettings()`（`app-12`）・管理メッセージと `getState`（`app-13`）・`admin.html` のスライダー／設定マップ／初期同期に追加
- **パネル下部の説明文を削除**: 「『HIGH 10』『LOW 10』でいつでもエントリー…」の `.bo-hint` 行と、使わなくなった `.bo-hint` のCSSを削除
- **動作確認**（Playwright、実配信への投稿と共有設定の保存はスタブ化）
  - 履歴12件を積んだ状態で 8件（既定）→ 3件 → 12件 → 0件 → 8件 と切り替え、表示件数と `localStorage` 保存値が毎回一致することを確認
  - パネルから `.bo-hint` が消え、履歴行が最下部になることをスクリーンショットで確認。JSエラーなし

---

## v2.924.0 — 2026-09-08

### docs: キーワードサンプル28件を index.html に反映（1枚のみのキーワードに対応）

`chara/keyword-samples/` に追加された28件のサンプル画像を、ルート `index.html` の「🖼 キーワードサンプル」一覧に反映。**既存97件の表示は変更なし**。

- **画像の変換**: 追加されたPNG28枚を WebP 化（既存サンプルと同じ `sharp` の `webp({quality:85})`・元PNGは削除）。うち17件は前回変換ぶんと**バイト単位で同一**＝内容変更なし、11件（soi / waoo / 十字架 / 大神 / 掴み / 瓶 / 穴 / 肉食い / 虚式紫 / 足 / 鏡撮り）が新規
- **`index.html`**: `KW` 配列の末尾に28キーワードを追加
  - アリス / グラス / ジョジョ / スタイルF / スタイルG / スタイルH / スヌート / スプリット / スライム / デコピン / ネトフリ / ハートチャレンジ / ハートデュオ / フィギュ2 / ペルソナ / リファ / 塊 / soi / waoo / 十字架 / 大神 / 掴み / 瓶 / 穴 / 肉食い / 虚式紫 / 足 / 鏡撮り
  - いずれも `sdKeywordPrompts`（`data/settings.json`）に登録済みのキーワードであることを確認して追加
- **`index.html`**: `KW_SINGLE` を新設。**サンプルが1枚だけのキーワードは画像を1枚（カード幅いっぱい）で描く**ようにした。従来の2枚組（`{kw}.webp` ＝ ベース+キーワード、`{kw}_plain.webp` ＝ キーワードのみ）は分岐の else 側でそのまま
- **動作確認**（Playwright でルート `index.html` を実際に開いて確認）
  - カード数 97 → **125**、追加28件はすべて画像1枚で表示成功
  - 既存カードで画像が欠けたものは0件、`keyword-samples` への404も0件
  - `sdKeywordPrompts` 122件すべてにサンプル画像が揃い、`KW` 一覧からの漏れも0件になった

---

## v2.923.0 — 2026-09-08

### fix: タイマンのベットで確実にMPを2倍にできる抜け道を封鎖

必ず負ける相手を指名して挑み、全所持MPを相手側にベットすると、ベット分が所持MPから退避されて敗北時の没収対象が0になり、的中の2倍返しだけが残る＝**ノーリスクでMPが倍になる**状態だった。3方向から塞いだ。

- **A: 当事者のベットを禁止**（`app-06-comment-handler.js` のベットコマンド分岐）
  - `user.ipid` が `taimanState.challenger` / `taimanState.target` と一致する場合は受け付けず「⚔️ 対戦中の2人はベットできません」と吹き出し。自分の負けに賭ける八百長を根本から遮断
  - `showTaimanBetBanner()` の受付バナーにも注意書き（`.taiman-bet-note`）を追加
- **B: ベットで退避中のMPも没収対象に**（`endTaiman()`）
  - 没収額の母数を `loser.mp` → **`loser.mp + 敗者のベット元金`** に変更。手持ちから引ききれない分は**ベット元金から没収**（`_bets[loser.ipid].amount` を減額）し、元金が0になったベットは払い戻し対象から除外
  - 既存の「預金MPはタイマンで奪われない／タイマン中は預け入れ不可」と同じ趣旨の穴埋め。Aを迂回されても資産退避が成立しない
- **C: 配当を固定2倍 → パリミュチュエル方式**（馬券と同じ、`endTaiman()` の払い戻し）
  - 的中者で**負け側のプールを賭け金比で山分け**。`calcTaimanBetPools()` / `taimanBetOdds()` を新設し、配当計算と表示で共用
  - 片側に票が偏るほど配当が1倍に近づくので「強い方に賭けるだけ」の期待値プラスが消える。負け側が0のときのために最低倍率 `TAIMAN_BET_MIN_ODDS = 1.1` を保証
  - `renderTaimanHpBars()` の各サイドに**現在のオッズ**（`◯◯MP 4.0倍`）を表示。的中の吹き出しも `🎰 的中！ 1.3倍 MP+400` と倍率入りに
  - 受付バナーの「的中で2倍返し」を「的中者で負け側のMPを山分け（配当は賭け金の偏りで変動・最低1.1倍）」に変更
- **`style.css`**: `.taiman-bet-note`（注意書き・赤）と `.taiman-bet-odds-val`（オッズ強調・黄）を追加
- **`index.html`**: タイマンの機能解説に「🎰 ベット」「💪 応援」の説明を追加し、コマンド一覧に `ベット N N` と `（タイマン中に選手名）` のカードを追加（`/update-index`）
- **動作確認**（Playwright / localhost:3000）
  - ファイターAが `ベット 2 1000` → `taimanState.bets` に入らず所持MPも1000のまま（A）
  - 観客が 1に100MP・2に300MP → HPバーに `100MP 4.0倍` / `300MP 1.3倍` を表示。2の勝利で300MPベットが400MP払い戻し（700→1100）、100MP側は没収（C）
  - Aを迂回して挑戦者に1000MPの相手側ベットを直接注入 → 敗北後の所持MPは**0**（旧仕様なら2000）、勝者は2000MPを獲得（B）
  - JSエラーなし
...
### feat: BO（マジカルオプション）の表示設定を拡張＋賭けた値段から白い横線

- **重なり順をマイナスまで指定可能に**: `boPanelZSlider` の範囲を `0〜1000` → **`-1000〜1000`**（キャラより後ろに回せる）
- **横幅・縦幅・文字サイズを個別に設定可能に**（管理ウィンドウ「📐 パネル外観」）
  - `boPanelWidth`（300〜1200px・既定570）→ CSS変数 `--bo-w` で `.bo-panel` の幅
  - `boChartHeight`（60〜400px・既定110）→ `--bo-chart-h` で `.bo-chart-wrap` / `.bo-chart` の高さ。SVGの `viewBox` とマーカーのY計算も追従（`BO_CHART_H` 固定値の参照を `boChartHeight` に置換）
  - `boFontScale`（60〜200%・既定100）→ `--bo-fs` を掛けた `calc()` でパネル内の文字サイズを一括スケール（タイトル・レート・ラベル・結果一覧・ヒント）
  - 3つとも `app-01` の同期キー・`app-03` の変数・`applyPanelSettings()`・`app-13` の管理メッセージと `getState`・`admin.html` のスライダー／設定マップ／初期同期に追加。従来の「BO サイズ」（全体倍率）はそのまま併用できる
- **判定後もマーカーを残すように変更**: `settleBoEntry()` でマーカーを消さず `boState.ghosts` に移し、**チャートから流れて消えるまで表示し続ける**。ラベルの残り時間表示は判定後に結果（`+95MP` / `-30MP`）へ切り替わり、線と文字は少し淡くなる（`.bo-done`）
- **賭けた値段から白い横線を追加**: 各マーカーに `.bo-marker-line`（白 `rgba(255,255,255,0.85)`・1px）を追加し、エントリー地点から**右へ真っ直ぐ伸ばす**。判定待ちは現在レートの位置まで、判定済みは判定時点で止まる
- **ラベルの重なり回避を作り直し**: X方向の段ずらし（レーン方式）をやめ、**X範囲とY位置の両方**で判定して空いている高さへ逃がす方式に変更（`BO_LANE_PX` / `BO_LANE_MAX` は廃止）。ラベル高さは文字サイズ設定に連動
- **動作確認**（Playwright、実配信への投稿と共有設定の保存はスタブ化）
  - 横幅900px・縦幅220px・文字サイズ150% を指定 → パネル実測900px、チャート220px、`viewBox="0 0 450 220"`、タイトル15→22.5px、レート20→30px を確認
  - 重なり順 `-500` が `z-index: -500` として反映されることを確認
  - 4人が続けてエントリー → 白い横線が各エントリー地点から伸び、ラベルが重ならずに配置されることをスクリーンショットで確認
  - 判定後も4件のマーカーが残り、ラベルが `▲ あげる 30MP -30MP` のように結果表示へ切り替わることを確認。JSエラーなし
...
- **チャートの横幅を1.5倍**（`public/style.css` の `.bo-panel` 幅 380px → 570px）。あわせて SVG 座標系 `BO_CHART_W` も 300 → 450 にして、横方向に間延びしないようにした
- **表示する期間も1.5倍**（`app-12-features-minigames.js`）。`BO_WINDOW_MULT = 1.5` を新設し、表示窓を `(判定秒 + 5秒) × 1.5` に変更
  - 判定15秒なら **20秒ぶん → 30秒ぶん**の値動きが1画面に収まる
  - 幅と期間を同じ倍率で広げているので、**1秒あたりの細かさ（px/秒）は据え置き**。単に横に引き伸ばすのではなく、同じ密度のまま長い期間を確認できる
  - マーカーのX位置は `windowTicks` 基準で計算しているため、賭けた地点の表示も自動で追従する
- ラベルの重なり判定 `BO_LABEL_W` を 34% → 23% に調整（チャートが広がり、ラベル最大122px に対する比率が下がったため。不要な段ずらしが減る）
- **パネルのヘッダー表記を「📈 バイナリーオプション」→「📈 マジカルオプション」に変更**（管理ウィンドウのセクション名は従来どおり）
- **動作確認**（Playwright、実配信への投稿はスタブ化）
  - 判定15秒で起動 → 表示窓30秒・チャート実測幅840px（パネル倍率155%込み）＝**28.0px/秒**で、変更前の約17.8px/秒 × 1.5倍と一致
  - 4人が1.6秒間隔でエントリー → マーカーのX位置とラベルの段ずらしが正しく追従、はみ出しなし
  - 保持ティック数が窓（300ティック＝30秒）で頭打ちになり、以降は左へ流れることを確認。JSエラーなし

---

## v2.918.0 — 2026-09-08

### feat: バイナリーオプション（BO）ミニゲームを追加

「終了」を押すまでレートが動き続ける**常設パネル**。全員で同じチャートを見て、**いつでもエントリー**できる。判定は各自の**賭けた瞬間のレート**が基準で、そこから一定秒数後に上か下かで個別に精算される。レートは実レートではなく乱数（ランダムウォーク）で生成する。

- **`app-12-features-minigames.js`**（ファイル末尾に BO セクションを追加）
  - `startBo(judgeSeconds, payoutRate)`: 判定まで15秒・配当1.9倍が既定（管理ウィンドウで変更可。判定は5〜300秒、倍率は1.01〜5倍にクランプ）。設定値は `localStorage` に控えてコメント起動でも同じ条件を使う
  - `startBoByComment(user)`: コメント「BO」「BO開始」「BOスタート」で**視聴者もBOを開催できる**（消費 `BO_START_COST = 100`MP、masterは無料）。MP不足・稼働中は吹き出しで弾き、起動できなかった場合はMPを戻す
  - `boTick()`: `BO_TICK_MS = 100` ごとにレートを更新。`_boNextPrice()` は弱い慣性（momentum×0.82）＋乱数で、3%の確率で大きく跳ねてヒゲを作る。開始レートは1500
  - `handleBoEntry()`: 稼働中ならいつでも受付。エントリー時にMPを徴収し、**その瞬間のレート・ティック位置・判定ティック**を記録する。MP不足・1MP未満は弾く。1人が何件でも／両方向にもエントリー可
  - `settleBoEntry()`: エントリーごとに `judgeSeconds` 後のレートと比較して個別精算。的中は `賭けMP × 配当倍率`（四捨五入・最低1MP）、外れは没収、**同値は返金**。結果は吹き出し・キャラログ・パネルの「直近の結果」（`BO_RESULT_MAX = 8` 件）に出る
  - `stopBo()`: 終了時は判定待ちのエントリーを全額返金し、的中／ハズレ／引分の集計をシステムログに出す（旧名 `cancelBo()` も残置）
  - **コメント通知**: `_boPostComment()` を新設し、開催・エントリー・判定結果を配信コメントに流す（`postAIReply()` 経由）
    - 例: `📈 えんとり が LOW に 100MP賭けた！（1500.04 / 15秒後判定）` / `📈 結果: ゆーざ2 HIGH 1500.12→1503.28 的中 +76MP`
    - `BO_POST_MS = 1200` の間隔で1件ずつ投稿し、同じティックで判定された分は1コメントにまとめる。溜まりすぎた場合（12件超）は古い方から捨てる
  - `_boSyncMarkers()`: **エントリー地点に「▲/▼ 名前 金額MP ⏱残り秒数」のマーカー**をチャート上へ絶対配置。X はエントリー時のティック、Y はそのときのレート。現在レートが有利なら枠が光る（`bo-winning`）
    - ラベルが重ならないよう、X が近いものは段（レーン）をずらして積む（`BO_LABEL_W = 34%` / `BO_LANE_PX = 13px` / 4段を超えたら点のみ）。チャート上下にはみ出す場合は内側にクランプし、左端付近のラベルは右側に出す
    - 表示窓は「判定秒＋5秒」ぶん。時間が経つとマーカーごと左へ流れ、判定されると消える
  - `updateBoChart()`: ティックごとはポリライン・現在値・マーカーだけを差し替える軽量更新。パネル全体の再描画はエントリー時と判定時のみ
  - パネルはドラッグで移動でき、位置は `panelKey('boPanelX'/'boPanelY')` に保存（TDZ対策の前出しブロックに `boDragState` / `boState` を追加し、共通の mousemove / mouseup に分岐を追加）
- **パネルサイズ設定**: `boPanelScale`（50〜200%、既定100）を追加。CSS変数 `--bo-scale` で `.bo-panel` を `transform: scale()`（左上基準）する
  - `app-03-boss-pets.js` に変数、`app-01-core-characters.js` の同期キー一覧、`applyPanelSettings()`（`app-12`）、管理メッセージ処理と `getState`（`app-13`）、`admin.html` の「📐 パネル外観」スライダー・設定マップ・初期同期に追加
  - 拡大しても画面外へドラッグできないよう、BOパネルのドラッグ制限は `getBoundingClientRect()`（拡大率込みの実サイズ）で計算する
- **`app-06-comment-handler.js`**: 起動コマンド `BO` / `BO開始` / `BOスタート`（全角 `ＢＯ` 可）と、稼働中の `HIGH 10` / `LOW 10` / `ハイ 10` / `ロー 10` / `上 10` / `下 10`（大文字小文字・全角数字・全角スペース対応、スペース省略可）を追加。馬券判定の直後に配置
- **`app-13-race-admin-misc.js`**: `handleAdminMessage` に `boStart` / `boStop`（旧 `boCancel` も受ける）を追加
- **`public/admin.html`**: 競馬の下に「📈 バイナリーオプション」（判定まで秒・配当倍率＋「📈 BO開始」「■ 終了」）、「📐 パネル外観」に「BO サイズ」スライダーを追加。`adminSend()` 方式なので index.html 側への要素追加は不要
- **`public/style.css`**: `.bo-panel` 一式（水色テーマ、SVGチャート、エントリーマーカー、HIGH/LOW集計、直近の結果）
- **動作確認**（Playwright で実ページを操作。`/api/post-comment` と `postAIReply` はスタブ化して実配信には投稿していない）
  - 判定8秒で稼働し、1.5秒ずつずらして4人がエントリー → マーカーが「▲ あげる 30MP ⏱4秒」のように名前・金額・残り時間付きで別々のX位置に並ぶ
  - 最初のエントリーだけが先に判定され、残り3件は判定待ちのまま＝**個別判定**を確認。全件判定後もパネルとチャートは動き続ける（常設）
  - 起動コマンド：MP30では起動できずMPも減らない／MP250なら起動して150に減る／稼働中の再実行ではMPを取られない
  - コメント通知：開催・エントリー・判定結果がそれぞれ意図した文面で投稿キューに積まれることを確認
  - サイズ設定：スライダー150%で `--bo-scale: 1.50`・実測幅380→570px・`localStorage` に保存されることを確認
  - 精算：1.9倍配当（12MP→23MP）とハズレの没収、終了時の返金（+25MP）を確認。ラベルの重なり・チャート外へのはみ出しなし、JSエラーなし、`charSave.json` への副作用なし
...
- **歌詞の背景を出すスコアを 5000 → 2500 に引き下げ**（`LYRICS_AT`）。5000は遠すぎて、ほとんど誰も見られない位置だった
- **ボットが自分から急降下するようにした。** 空中にいるとき「今すぐ降りても当たらず、着地後も手が続く」なら降りる。早く地上に戻るほど次に自由が利くので詰みにくい
  - 降下中に重なる区間は、単調に下がるので**区間の両端の高さ**で判定できる。着地後のぶんはまとめて先読みで見る
  - 落下時間は重力を無視して `y / dropV` で見積もる。実際はもう少し速く着くので安全側
- 先読みで見る障害物を 3個/1.8秒 → **5個/2.2秒** に拡大
- **効果**: 45秒あたり4回死んでいたのが、150秒走らせて **0回・スコア8427・速度888** まで到達
- ただし判断が間に合わない場面は残るので、**ボット操作中は当たっても死なない**ようにした（見本が途切れないための保険）。手動プレイには影響しない
- 一度「この1回のジャンプで必ず越せること」まで求める判定にしたが、**待ちすぎて逆に悪化した**（180秒で13回死亡）ので戻した

#### 隠しコマンドの扱い

- 公開しているパッチノートにボットと無敵のコマンド文字列が載っていたので、**該当箇所から文字列を削除**した（v2.909.0 / v2.911.0）。実装は `cloudflare/pages/dino.html` にある

---

## v2.915.0 — 2026-09-06

### change: DINO のゲームオーバー画面で情報を右端に、既定名を「名無し」に

- **情報カラムを一番右へ。** 表示は「ハイスコア → 最新のプレイ → GAME OVER/スコア/名前入力/登録/リトライ」の並びになる
  - HTML の順序は変えず `flex-direction: row-reverse` で並びだけ入れ替えた。名前入力より先に表を読み上げ・タブ移動させたくないため、**タブ移動と読み上げの順序は従来どおり**
  - クラス名も実態に合わせて `.over-left` → `.over-info` に変更
  - 900px 以下の縦積みでは従来どおり情報が上（スコアを先に見せたいため）
- **自動登録の既定名を `きしょ犬` → `名無し` に変更**（`worker.js` の `DEFAULT_NAME`）。名前欄のプレースホルダも合わせて更新
  - すでに `きしょ犬` で登録済みの行はそのまま残す（実際に遊ばれた記録のため）。新しいプレイが入るたびに押し出されて消える
- **確認**: 1440x900 で情報カラムが最も右（x=1004、表は151/572）にあること、390x844 では情報が上（y=123）に来ることを確認。どちらも縦スクロール・はみ出しなし

---

## v2.914.0 — 2026-09-06

### change: DINO のゲームオーバー画面を縦スクロールなしで全部見えるように

要素が増えて中身が枠から 100〜217px はみ出し、縦スクロールが出ていた。全部を一画面に収めた。

- **横並びに再構成**。左に「GAME OVER / スコア / 名前入力 / 登録 / リトライ」、右に2つの表を並べる。3:1 の盤面の横幅を活かす形にした
- 表の行を詰めた（`padding` 4px→1px、文字サイズを `clamp()` で盤面幅に追従）。見出し・順位・スコア・日付も相対サイズに変更
- **狭い画面（900px以下）ではオーバーレイだけ画面全体に広げる**。盤面が 374x258 しかなく、その中に収めようとすると文字が読めない大きさになるため。`#wrap` に `transform` が無いので `position: fixed` が画面基準で効く
- 表が盤面いっぱいに広がると背景の絵が透けて読みにくかったので、ゲームオーバーとランキングの暗幕を `rgba(8,10,15,.88)` に濃くした
- **確認**: 1440x900 / 1280x720 / 1000x800 / 390x844 の4サイズで、縦スクロールが出ず、要素が枠外にはみ出さず、両方の表が全行表示されることを確認

---

## v2.913.0 — 2026-09-06

### feat: DINO ランキングに「最新のプレイ10件」を追加（自動登録）

ハイスコアとは別に、遊ばれたスコアを新しい順に10件表示するようにした。

#### 保存

- **`cloudflare/worker.js`**: `_dino_ranking.json` の中身を配列から `{ ranking, recent }` に変更。**以前の配列形式もそのまま読める**ようにして、既存のスコアを引き継いだ
- `POST /dino-ranking` に `type` を追加
  - `type: 'recent'` … 最新プレイに積む（`RECENT_MAX = 10` 件で古いものから落ちる）。**名前が空なら既定名 `きしょ犬`**
  - 省略時 … 従来どおりハイスコアに積む。名前が空なら 400 のまま
- `GET` は `{ ranking, recent }` を返す（どちらも IP は落とす）
- `DELETE /admin/dino-ranking` に **`index` 指定**を追加。`{ index: 1 }` で1位を、`{ index: 3, list: 'recent' }` で最新の3件目を消せる。文字化けした名前など、名前で指定しづらいものを片付けるため

#### 表示

- **`cloudflare/pages/dino.html`**: ゲームオーバー画面と Ranking パネルに表を2つ並べた（`High Score — Top 10` と `最新のプレイ — 10件`）。狭い画面では縦に折り返す
- 最新のプレイ側は順位ではなく新しい順なので、番号を出さない
- **ゲームオーバーのたびに自動で登録**する（`postRecent()`）。名前欄か localStorage の名前を使い、無ければ既定名。手動の「ランキングに登録」は従来どおりハイスコア側だけに入る
- 名前欄のプレースホルダを「なまえ（未入力なら きしょ犬）」に変更

#### 確認

API を直接叩いて、旧形式データの読み込み、名前なしで `きしょ犬` になること、11件入れても10件で新しい順に保たれること、手動登録が最新プレイ側を増やさないこと、手動で名前なしが従来どおり弾かれることを確認。ブラウザでも実際に死んでみて、今回のスコアが最新プレイの先頭に自動で入り、Ranking パネルにも両方の表が出ることを確認した。

---

## v2.912.0 — 2026-09-06

### change: DINO のスタート画面に急降下の操作説明を追加

急降下（v2.910.0）が説明のどこにも書かれておらず気づけなかったため、操作を明示した。

- **スタート画面**に1行追加: 「ジャンプ中に `SPACE` をもう一度で 急降下して着地」。キー名は Press Start 2P の枠付きで見せる
- **ヘッダーのヒント**も「SPACE / ↑ / タップ でジャンプ　・　ジャンプ中にもう一度で急降下」に更新
- 最初はキー一覧を2行の箇条書きにしたが、上の「〜でスタート」と内容が重複して詰まって見えたので1行に整理した
- 1440x900 と 390x844 で、盤面からはみ出さずスクロールも出ないことを確認

---

## v2.911.0 — 2026-09-06

### feat: DINO — デバッグ用の無敵モード（隠しコマンド）

- ゲーム画面で**所定の隠しコマンドをタイプすると無敵**になり、もう一度打つと解除。ヘッダーに緑の「無敵」バッジが出る（コマンド文字列は `cloudflare/pages/dino.html` を参照）
- 当たり判定はそのままで、`o.deadly` の衝突時に `die()` を呼ばず素通りさせる（カニの弾き飛ばしは無敵中も従来どおり動く）
- ボットの隠しコマンドとキー入力バッファを共用にした。長さが違うので `endsWith` で両方を判定する。名前入力欄にフォーカスがあるときは拾わない
...
- ゲーム画面で**所定の隠しコマンドをタイプするとボットが操作を代わる**。もう一度打つと解除。ヘッダーに `BOT` バッジが出る（コマンド文字列は `cloudflare/pages/dino.html` を参照）
- 待機中に有効化すると自動で開始する

...
ゲームは `/chara-s/:filename` エンドポイント経由でキャラ画像を配信している（`app-01-core-characters.js` の `imgSrc = /chara-s/...`）。

このエンドポイントは `server.js` で以下を行う：
1. 元画像を **半サイズ**にリサイズ（`sharp().resize(halfW)`）
2. PNG/JPEG/WebP を問わず **WebP（quality 85）で出力**（透過も保持）
3. サーバープロセス内メモリに **キャッシュ**（Map）

これにより `public/chara/` にある元ファイル（数MB の PNG 等）を変換・書き換えせずに、ブラウザには軽量な WebP を返せる。

### 新しいキャラ画像を追加したとき

`public/chara/` に PNG を置くだけでよい。`/chara-s/` が初回アクセス時に自動変換・キャッシュする。手動変換は不要。

### キャッシュをリセットしたいとき

サーバーを再起動すれば `_charaSCache`（Map）がクリアされる。
